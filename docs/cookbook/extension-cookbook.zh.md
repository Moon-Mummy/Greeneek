...
交付应用通过 `packages/bundle/*/cordis.patch.yml` 提供 profile 层，产品 `gnk` 启动器通过具名 profile 负责 Web、ACP、SDK 与一次性 headless 执行。可选的用户 overlay 位于 `apps/cli/config/examples/`；profile 集成测试位于 `apps/cli/tests/profiles/`，包专属 Loader 组合则留在对应包的测试目录中。
...
| 产品功能 | 插件机制 |
|---|---|
| 钩子系统（用户级 + 项目级） | `agent/session-start`、`agent/pre-step`、`agent/request`、`tools/pre-execute`、`tools/post-execute` 和 `agent/turn-stopping` 上的监听器；waterfall 返回类型化决策，`agent/turn-stopping` 则可通过 steering（中途引导）触发下一步；`gnk-hooks-claude-code` / `gnk-hooks-codex` 桥接器将钩子配置文件映射到这些扩展点上 |
| `/goal` | `ctx.goals` 管理持久状态，`gnk-goal-round-driver` 通过公共 `Agent` 调度同会话 Round，独立的命令/工具生产方分别提供人类/模型控制 |
...
| 上下文压缩（context compaction）（自动 + 手动） | `ctx.compaction` seam + `gnk-compaction-basic`；自动压力检查运行在串行 `agent/pre-step`，标准的溢出恢复机制运行在 `agent/request-error`，手动调用方使用同一个压缩服务（[压缩 Agent Note](../../.agents/notes/implemented/feature/2026-06-18-compaction-capability-seam.zh.md)） |
...
| 内置工具 | `ctx.tools.register()`；schema 自动流入装配——`gnk-tool-*` 系列（bash、fs、web、subagent、todo）是已交付的示例 |
...
| 子进程沙箱（landlock / sandbox-exec） | 通过 `gnk-bash-sandbox` 使用 `ctx.sandbox` 后端；能力级别的拒绝使用 `tools/pre-execute` |
| 权限系统 / AskUserQuestion | 从 `tools/pre-execute` 返回 `ask` 并通过 `ctx.approval` 应答；为普通用户提问注册一个独立的面向模型的 ask 工具 |
| Plan mode | [`@greeneek/gnk-plan-mode`](../../packages/plan/plan-mode/README.zh.md)：落日志的 `plan/mode` 状态、`plan:policy` 引导段、`/plan [message]` 入口、`/plan off` 直接退出，以及经用户评审的 `exit_plan_mode` 出口；强制约束留在独立的沙箱/审批轴上 |
| subagent 委派 | `ctx.subagents` 提供方注册表（`gnk-subagent-spawn-in-process`/`gnk-subagent-fork-in-process`/`gnk-subagent-acp`/`gnk-subagent-codex`/`gnk-subagent-claude-code`/`gnk-subagent-gnk-sdk`）+ `gnk-tool-subagent` 向模型暴露一个已配置的提供方 |
...
| 模型适配器 | 通过 `registerAdapter` 注册 `LlmAdapter` 子类（`gnk-llm-greeneek`、`gnk-llm-pi-ai`） |
