# Greeneek 官方 LLM API 协议扩展

[English](greeneek-llm-api-wire-extensions.md) | 中文

本参考文档定义 [`@greeneek/gnk-llm-greeneek`](../packages/llm/llm-greeneek/README.zh.md) 在 `greeneek-official` 聊天补全请求中发送的全部 Greeneek Harness 特有 HTTP 标头和附加 JSON 字段。本文不重复定义 Greeneek 上游 API 持有的字段。提供方无关的 LLM（大语言模型）接口与 `llm-pi-ai` 均不实现这些扩展。
...
| 位置 | 命名方式 | 示例 |
|---|---|---|
| HTTP 字段名 | 小写 kebab-case；HTTP 匹配仍不区分大小写 | `user-agent`, `x-greeneek-harness-session-id` |
| Greeneek 请求正文扩展字段 | 使用保留 `gnk_` 前缀的 snake case | `gnk_plugin_packages`, `gnk_session_log` |
| GNK 持有的嵌套 JSON 成员 | Camel case | `afterSeq`, `throughSeq`, `sessionId` |
| 带标签的值 | 使用 kebab-case 字符串；持久事件采用 `domain/action` | `session-log-greeneek/delivery-accepted` |

每个正文扩展独立持有自身的 `version`。版本仅适用于包含该字段的对象；不同字段的版本之间不存在兼容或排序关系。JSON 成员顺序不属于协议。

[`GreeneekLlmApiExtensionRegistry`](../packages/llm/greeneek-llm-api-extensions/README.zh.md) 为每个顶层扩展名保留一个提供方。空名称、两端带空白的名称、重复注册以及与 Greeneek 基础请求冲突的名称都会在 HTTP 分派前失败。

## 请求标头

| 标头 | 出现条件 | 值 |
|---|---|---|
| `user-agent` | 每个提供方 HTTP 请求，包括 Files API 操作 | 采用 `product/version (+url)` 形式的应用身份；默认产品为 `greeneek-harness` |
| `x-greeneek-harness-user-id` | 每个已授权的聊天补全请求 | 已解析 Harness home 的稳定匿名 UUID |
| `x-greeneek-harness-session-id` | 携带会话 id 的聊天补全请求 | 确切的请求 `sessionId` 字符串 |
| `x-greeneek-harness-compact` | 用途为 `compaction` 的聊天补全请求 | 字面字符串 `1` |

凭据失败发生在解析匿名用户 id 之前，因此未授权请求既不会发送这些标头，也不会创建身份文件。没有会话的直接请求会省略 `x-greeneek-harness-session-id`。会话标题请求没有额外的用途标头；请求携带 `sessionId` 时，仍然适用普通的会话 id 规则。
...
[`@greeneek/gnk-plugin-package-inventory-greeneek`](../packages/llm/plugin-package-inventory-greeneek/README.zh.md) 贡献完整存活的 Loader-backed 插件包清单。该字段默认启用。
...
| 成员 | 类型 | 含义 |
|---|---|---|
| `version` | `1` | `gnk_plugin_packages` 的 schema 版本 |
...
清单已启用但没有符合条件的配置项时，系统发送 `packages: []`；禁用贡献插件时，系统省略整个 `gnk_plugin_packages` 字段。包身份属于提供方元数据，绝不进入模型输入。

## `gnk_session_log`

[`@greeneek/gnk-session-log-greeneek`](../packages/session/session-log-greeneek/README.zh.md) 贡献权威会话日志的一段连续后缀。该字段默认禁用。启用后，它适用于携带存活会话且至少存在一个事件的请求；直接请求、陈旧会话 id 或空日志会省略该字段。
...
| 成员 | 类型 | 含义 |
|---|---|---|
| `version` | `1` | `gnk_session_log` 的 schema 版本 |
...
`session` 成员是确切的 `Session.header`，不是完整的运行时会话。外层 `gnk_session_log.version` 选择本扩展 schema，`session.version` 则选择权威磁盘会话格式；两个版本值相互独立演进。
...
请求标头会暴露 Harness 应用版本、一个匿名 Harness-home 身份和可选的会话身份。`gnk_plugin_packages` 会暴露存活 npm 包的名称与版本。启用后，`gnk_session_log` 可能暴露会话工作目录、系统提示词快照、用户与 assistant 内容、原始 assistant 分片、工具参数与结果、压缩摘要、反馈和插件持有的事件。适配器 API key 不是会话事件，因此不会进入该字段。通过 `baseURL` 选择的网关会收到与官方端点相同的值。
