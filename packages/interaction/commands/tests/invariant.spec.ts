...
import * as CommandInvariant from '@greeneek/gnk-commands/invariant'
import InvariantRegistry, { InvariantError } from '@greeneek/gnk-invariants'
import SessionStore, { SessionId, SessionSeq, type Session } from '@greeneek/gnk-session'
import { CommandId } from '@greeneek/gnk-commands'
...
describe('command lifecycle invariants', () => {
  ...
  it.each([-1, 1.5, 1])('rejects invalid or non-prior sourceEventSeq %s', async (sourceEventSeq) => {
    ...
    expect(() => {
      ...
      code: 'INVARIANT',
      packageName: '@greeneek/gnk-commands',
    }))
  })
