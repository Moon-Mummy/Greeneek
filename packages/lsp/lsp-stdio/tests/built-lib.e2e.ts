...
/**
 * Keyless built-artifact smoke: plain Node imports `@greeneek/gnk-lsp` and
 * `@greeneek/gnk-lsp-stdio` by name through their exports maps, spawns the fixture server, runs
...
describe.skipIf(!built)('built lib real load path (plain node)', () => {
  it('runs a query through lib/index.js and disposes cleanly, framing over the base protocol', async () => {
    ...
    const script = `
      ...
      const { default: Lsp } = await import('@greeneek/gnk-lsp')
      const LspLocal = await import('@greeneek/gnk-lsp-stdio')
      const { default: LocalFileSystem } = await import('@greeneek/gnk-fs-local')
