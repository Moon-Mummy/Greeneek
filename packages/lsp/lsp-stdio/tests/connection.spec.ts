...
import { LspConnection } from '@greeneek/gnk-lsp-stdio'
import type { ConnectionWriter } from '@greeneek/gnk-lsp-stdio/src/connection.ts'
...
describe('LspConnection', () => {
  ...
  it('forwards explicit GNK_* env entries to the child', async () => {
    // A configured GNK_* fact must reach the child: the seam scrubs only the
    // ambient namespace, and the explicit entry merges after that scrub. The
    // fixture echoes the named variable back as hover text.
    const conn = connect({ LSP_FAKE_ECHO_ENV: 'GNK_LSP_TEST_FACT', GNK_LSP_TEST_FACT: 'managed' })
