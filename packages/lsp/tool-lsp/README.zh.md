...
`gnk-tool-lsp` 通过 LSP seam 为模型提供单一的只读 `lsp` 工具，用于精确代码导航：转到符号的定义、查找其引用、跳转到其实现，或阅读悬停文档。该工具拥有模型看到的一切——名称、schema、提示词指引、结果格式化与 UI 呈现——并且绝不依赖哪个语言服务器应答查询。位置是从 1 开始的 UTF-16 光标坐标，工具会将其转换为 seam 从零开始的约定。结果是有边界的位置列表或规范化悬停文本，带有明确的空结果与截断标记。与 `gnk-lsp-stdio` 之类的提供方及 `gnk-lsp` seam 组合，即可启用导航。
...
| `timeoutMs` | `60000` | 由 `gnk-tool-call-timeout-policy` 强制执行的工具调用超时预算；覆盖完整的排队打开／查询／关闭生命周期，且模型不可配置 |

生成的[配置目录](../../../docs/config-catalog.zh.md#greeneekgnk-tool-lsp)是每个受支持字段的穷尽式真源。
...
- [gnk-lsp](../lsp/README.zh.md)——本工具查询的 seam。
- [gnk-lsp-stdio](../lsp-stdio/README.zh.md)——应答这些查询的 stdio 提供方。
...
模型会看到生成的 [`lsp` schema](../../../docs/tool-catalog.zh.md#greeneekgnk-tool-lsp)。
