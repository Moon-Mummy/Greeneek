 ...
 * @module @greeneek/gnk-agent-presets/mount
 */

import { pathToFileURL } from 'node:url'
import { Context, type Fiber } from '@greeneek/cordis'
import { Include } from '@greeneek/cordis-plugin-include'
import type { EntryTree } from '@greeneek/cordis-plugin-loader'
import { scopeOf, scopeParentOf, type ScopeKey } from '@greeneek/gnk-scope'
...
class PresetTree extends Include {
   ...
   * dependencies, so every `@greeneek/gnk-*` row would fail to import. The
 ...
 * invariant companion's service listener, and `gnk-invariants` is a
