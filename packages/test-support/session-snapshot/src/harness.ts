 ...
 *
 * @module @greeneek/gnk-session-snapshot/harness
 */
...
export interface RunOptions {
   ...
   * them to `gnk-llm-replay` via `$GNK_SNAPSHOT_CHILD_FILES` so each child
...
export async function runScenario(input: InputScript, opts: RunOptions): Promise<RunResult> {
  ...
  const outcome = await (async (): Promise<RunResult> => {
    ...
    const initialWorkspace = await captureWorkspaceSnapshot(cwd, {
      ignoredRootEntries: ['.agents', '.gnk', '.gnk-profile-patches', '.gnk-snapshot-stream-ready'],
    })
    const env: NodeJS.ProcessEnv = {
      ...opts.env,
      GNK_SNAPSHOT: opts.mode,
      GNK_SNAPSHOT_FILE: opts.fixtureFile,
      ...
      ...opts.overrideFile !== undefined ? { GNK_SNAPSHOT_OVERRIDE: opts.overrideFile } : {},
      ...opts.childFiles !== undefined && opts.childFiles.length > 0
        ? { GNK_SNAPSHOT_CHILD_FILES: opts.childFiles.join(delimiter) }
        : {},
    }
...
async function harvestSessionLogs(root: string): Promise<HarvestedLog[]> {
  ...
  // order must match the replay load order in gnk-llm-replay's loadSessionScripts
