/**
 * Scripted fake ACP agent bin for `gnk-session-snapshot`'s ACP adapter specs. Speaks
 * newline-delimited JSON-RPC on stdio like the real `gnk-acp-agent` bin, but
 ...
 * `behavior.json` sitting NEXT to the `$GNK_SNAPSHOT_FILE` fixture, so a spec
...
interface ScriptedLog {
  /** Path relative to `$GNK_SNAPSHOT_SESSIONS_ROOT`, e.g. `project/session/session.jsonl`. */
  file: string
  /**
...
interface Behavior {
  ...
  /** Echo the `GNK_SNAPSHOT_*` env the harness set as a chunk (spec-side env-plumbing assertions). */
  echoEnv?: boolean
...
const sessionsRoot = process.env.GNK_SNAPSHOT_SESSIONS_ROOT ?? ''
const fixtureFile = process.env.GNK_SNAPSHOT_FILE ?? ''
...
async function handlePrompt(id: number | string): Promise<void> {
  ...
  if (behavior.echoEnv === true) {
    chunk(`env:${JSON.stringify({
      mode: process.env.GNK_SNAPSHOT,
      override: process.env.GNK_SNAPSHOT_OVERRIDE ?? null,
      childFiles: process.env.GNK_SNAPSHOT_CHILD_FILES ?? null,
      spillRoot: process.env.GNK_SNAPSHOT_SPILL_ROOT ?? null,
      // Scenario-supplied deployment env (the `Scenario.env` layering hook).
      permissionMode: process.env.GNK_PERMISSION_MODE ?? null,
    })}`)
  }
