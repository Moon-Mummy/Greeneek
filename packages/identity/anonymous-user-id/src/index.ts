 ...
 * harness home resolved by {@link resolveGnkHome} (`$GNK_HOME` > `~/.gnk`),
 ...
 * machine: every process sharing one `$GNK_HOME` reports the same id, and
 ...
 *
 * @module @greeneek/gnk-anonymous-user-id
 */
...
export interface AnonymousUserIdOptions {
  /** Environment consulted for `GNK_HOME`; defaults to `process.env`. */
...
export function getOrCreateAnonymousUserId(options: AnonymousUserIdOptions = {}): AnonymousUserId {
  const file = join(resolveGnkHome(undefined, options.env ?? process.env), ANONYMOUS_USER_ID_FILE_NAME)
