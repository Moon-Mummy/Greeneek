...
import { credentialKey, credentialKeyScope, credentialRef, parseCredentialKey } from '@greeneek/gnk-credentials'
import type { CredentialKey, CredentialRecord } from '@greeneek/gnk-credentials'
...
async function tempDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'gnk-cred-records-'))
...
describe('record storage', () => {
  ...
  it('keeps references and records in one document without either disturbing the other', async () => {
    ...
    await ctx.credentials.set(credentialRef('GNK_RECORDS_KEY'), 'sk-live')
    await put(ctx, CODEX, { kind: 'grant', payload: { token: 't' } })

    const text = await readFile(path, 'utf8')
    expect(text).toBe(
      'version: 1\nrefs:\n  GNK_RECORDS_KEY: sk-live\nrecords:\n'
      + '  llm-pi-ai/openai-codex:\n    kind: grant\n    payload:\n      token: t\n',
    )
    expect(await ctx.credentials.resolve(credentialRef('GNK_RECORDS_KEY'))).toEqual({ value: 'sk-live', source: 'file' })
  })
