/**
 * File-backed credentials provider over `$GNK_HOME/.credentials.yaml`, layered
 ...
 * > $GNK_HOME/.env                   (read-only fallback)
 * ```
 *
 * The inherited environment wins because `GREENEEK_API_KEY=… gnk`, a CI
 ...
 * @module @greeneek/gnk-credentials-local
 */
...
import { CredentialProvider, credentialRef, parseCredentialKey } from '@greeneek/gnk-credentials'
import type {
  ApiKeyRecord,
  CredentialInfo,
  CredentialKey,
...
import type { LaunchEnvironmentEntry } from '@greeneek/gnk-launch-environment'
...
export function resolveSpec(config: Config): ResolvedSpec {
  return {
    filename: resolve(config.path ?? join(resolveGnkHome(config.gnkHome ?? legacyHomeConfig(config)), CREDENTIALS_FILENAME)),
 ...
 * holds it. Like the retry cadence in `gnk-atomic-write`, this is a
...
/** File-backed credentials provider (`$GNK_HOME/.credentials.yaml`). */
  ...
  private assertUnshadowed(ref: CredentialRef, verb: 'set' | 'unset'): void {
    if (this.inherited(ref) !== undefined) {
      throw new Error(
        ...
        + ' shadowed; unset it in the shell you start gnk from instead',
      )
    }
  }
