...
import { LAUNCHER_FAILURE_EXIT } from '@greeneek/node-addon-landlock-run'
import { SANDBOX_UNAVAILABLE, SandboxUnavailableError } from '@greeneek/gnk-sandbox'
import type { SandboxPolicy } from '@greeneek/gnk-sandbox'
import {
  LocalSandboxProvider,
} from '@greeneek/gnk-sandbox-local'
import type { Config } from '@greeneek/gnk-sandbox-local'
...
function absentRunnerEntry(): string {
  return join(mkdtempSync(join(tmpdir(), 'gnk-absent-acl-entry-')), 'runner.js')
}
...
function fakeLauncher(report = 'landlock: fully enforced'): string {
  const dir = mkdtempSync(join(tmpdir(), 'gnk-fake-landlock-'))
...
function fakeSeatbeltExec(status: number): string {
  const dir = mkdtempSync(join(tmpdir(), 'gnk-fake-seatbelt-'))
...
describe('the platform chains', () => {
  ...
  // live in @greeneek/gnk-sandbox-windows-acl/tests/provider-chain.spec.ts
...
describe('probeTimeoutMs config', () => {
  ...
  it('bounds the default probes: a launcher slower than the configured timeout reads as unusable', async () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), 'gnk-slow-landlock-'))
    const launcher = join(dir, 'landlock-run')
...
describe('the windows-acl probe (runner invocation contract)', () => {
  ...
  it('prefers the built lib/runner.js entry when the resolved file exists', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'gnk-fake-acl-entry-'))
