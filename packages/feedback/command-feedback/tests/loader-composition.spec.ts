...
describe('/feedback real Loader composition through cordis.yml', () => {
  it('boots cordis.yml and records feedback without model-visible output', async () => {
    root = await mkdtemp(join(tmpdir(), 'gnk-command-feedback-loader-'))
    ...
    const modules = new Map<string, unknown>([
      ...
      ['@greeneek/gnk-commands', CommandRuntime],
      ['@greeneek/gnk-command-feedback', CommandFeedback],
    ])
    ...
    const userId = getOrCreateAnonymousUserId({ env: { GNK_HOME: root } })
