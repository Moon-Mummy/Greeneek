 ...
 * @module @greeneek/gnk-tools/src/ptc
 */

import { brandString } from '@greeneek/gnk-brand'
import { createUserMessage, HarnessError } from '@greeneek/gnk-llm'
import type { ContentBlock, ToolCallId } from '@greeneek/gnk-llm'
import type { CodeBindingFunction, CodeRunResult, CodeRuntime } from '@greeneek/gnk-code-runtime'
import { snapshotJsonValue, type JsonValue } from '@greeneek/gnk-util-values'
...
function resolveFlavor(peekRuntime: () => CodeRuntime | undefined): RunCodeFlavor {
  ...
  if (!Object.hasOwn(RUN_CODE_FLAVORS, runtime.language) || flavor === undefined) {
    ...
    throw new Error(`gnk-tools: no run_code schema flavor registered for runtime language ${JSON.stringify(runtime.language)} (known: ${known})`)
  }
  return flavor
}
