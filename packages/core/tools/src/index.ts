 ...
 * execution pipeline.
 * @module @greeneek/gnk-tools
 */

import { Context, Service } from '@greeneek/cordis'
import z from '@greeneek/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeOf, scopeTarget } from '@greeneek/gnk-scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@greeneek/gnk-scope'
import type { ToolCallId, ContentBlock, ToolSchema } from '@greeneek/gnk-llm'
...
import { assertNever, deepFreeze, snapshotJsonValue, type JsonValue } from '@greeneek/gnk-util-values'
import type { ToolProviderResult } from '@greeneek/gnk-system-prompt'
import type { CodeRuntime } from '@greeneek/gnk-code-runtime'
 ...
 * instead of deriving them: the seam's `gnk-code-runtime` README pair, its
...
// lives in its own UI-facing module; re-export it so `@greeneek/gnk-tools`
 ...
 * @internal
 */
export const TOOL_RUNTIME_SCHEDULER: unique symbol = Symbol('@greeneek/gnk-tools.scheduler')
...
export class ToolRuntime extends Service {
  ...
  /** Internal staged view consumed by `gnk-agent-loop`'s parallel scheduler. */
  ...
  private sdkSection(): { name: string; order: number; text: (context: { scope?: ScopeKey }) => string } {
    return {
      ...
      text: (context) => {
        ...
        if (render === undefined) throw new Error(`gnk-tools: no SDK renderer for ${runtime.language}`)
  ...
  private requireCodeRuntime(mode: ToolPresentationMode): CodeRuntime {
    ...
    if (!runtime) {
      throw new Error(`gnk-tools: mode "${mode}" requires a code runtime — load a ctx.codeRuntime implementation (e.g. @greeneek/gnk-code-runtime-worker-thread) or set tools mode to "native"`)
    }
    if (!Object.hasOwn(SDK_RENDERERS, runtime.language)) {
      const known = Object.keys(SDK_RENDERERS).map(name => JSON.stringify(name)).join(', ')
      throw new Error(`gnk-tools: no SDK renderer registered for runtime language ${JSON.stringify(runtime.language)} (known: ${known})`)
    }
    return runtime
  }
   ...
   * `gnk-agent-tool-presentation` exists for, and reading the deployment default would
...
function createExecutionToken(): ToolExecutionToken {
  return Symbol('gnk.tool.execution') as ToolExecutionToken
}
