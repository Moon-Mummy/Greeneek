...
在必须存在会话的任何地方挂载 `gnk-session`。它在内存中创建并持有事件溯源的 `Session` 实例；持久存储由订阅 `session/event` 流的持久化插件叠加。
...
| [`gnk-util-values`](../../util/values/README.zh.md) | 共享无损 JSON 校验与分离式快照 |
