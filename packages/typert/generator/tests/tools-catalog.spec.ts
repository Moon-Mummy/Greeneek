...
import type { TypertContribution } from '@greeneek/gnk-typert-registry/types'
import { EVENT_API, SERVICE_API, TYPE_API } from '@greeneek/gnk-tool-cordis/src/api-catalog.ts'
...
describe('model-driven gnk-tools generation', () => {
  it('round-trips the complete service and event structure through the runtime registry', { timeout: 30_000 }, async () => {
    const workspace = new WorkspaceAnalyzer({
      root: workspaceRoot,
      faces: ['host'],
      packages: ['@greeneek/gnk-tools'],
    }).analyze()
    const host = workspace.faces.find(candidate => candidate.face === 'host')
    if (host === undefined) throw new Error('gnk-tools has no analyzed host face')
    const artifact = new FaceModelEmitter(host).emit('@greeneek/gnk-tools')
    ...
    const record = ctx.typert.getPackage('@greeneek/gnk-tools', 'host')
    ...
    await dispose()
    expect(ctx.typert.getPackage('@greeneek/gnk-tools', 'host')).toBeUndefined()
  })
})
