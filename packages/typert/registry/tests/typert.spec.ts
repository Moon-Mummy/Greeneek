...
} from '@greeneek/gnk-typert-registry'
import type {
...
describe('TypertRegistry', () => {
  it('registers and queries generated schemas separately from package reflection', async () => {
    ...
    expect(typertKey('@greeneek/gnk-tools', 'ToolInput')).toBe('@greeneek/gnk-tools#ToolInput')
    expect(typertPackageKey('@greeneek/gnk-tools', 'host')).toBe('@greeneek/gnk-tools#host')
    expect(ctx.typert.get('@greeneek/gnk-tools#ToolInput')).toMatchObject({
      package: '@greeneek/gnk-tools',
      face: 'host',
      name: 'ToolInput',
    })
    expect(ctx.typert.get('@greeneek/gnk-tools#ToolInput')?.schema).toBe(contribution.schemas[0]?.schema)
    expect(ctx.typert.getPackage('@greeneek/gnk-tools', 'host')).toMatchObject({
      key: '@greeneek/gnk-tools#host',
  ...
  it('withdraws schemas and package metadata through the exact contribution disposer', async () => {
    ...
    expect(ctx.typert.getPackage('@greeneek/gnk-tools')).toBeDefined()

    await dispose()

    expect(ctx.typert.get('@greeneek/gnk-tools#ToolInput')).toBeUndefined()
    expect(ctx.typert.getPackage('@greeneek/gnk-tools')).toBeUndefined()
  ...
  it('rejects duplicate package faces and schema keys before committing', async () => {
    ...
    expect(ctx.typert.get('@greeneek/gnk-tools#ToolInput')?.schema).toBe(original.schemas[0]?.schema)
  ...
  it('resolves required schemas and projects fresh JSON Schema documents', async () => {
    ...
    expect(ctx.typert.resolve('@greeneek/gnk-tools#ToolInput').name).toBe('ToolInput')
    expect(() => ctx.typert.resolve('@greeneek/gnk-tools#Missing')).toThrow('contributes no schema named "Missing"')
    ...
    const projected = ctx.typert.toJSONSchema('@greeneek/gnk-tools#ToolInput')
    expect(projected).toMatchObject({ type: 'object', properties: { name: { type: 'string' } } })
    expect(ctx.typert.toJSONSchema('@greeneek/gnk-tools#ToolInput')).not.toBe(projected)
  })
