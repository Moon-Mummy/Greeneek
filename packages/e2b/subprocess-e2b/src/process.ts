...
import type { CommandHandle, CommandResult, Sandbox } from '@greeneek/gnk-e2b'
import type {
  SubprocessCollect,
  SubprocessHandle,
  SubprocessOutcome,
...
function commandText(spec: SubprocessSpawnSpec, paths: RemotePaths): string {
  const encoder = `"$gnk_e2b_env_bin" -i "$gnk_e2b_node" -e ${quoteE2BShellArg(OUTPUT_ENCODER_SOURCE)}`
  const stdoutRedirect = hasSpill(spec.stdio.stdout)
    ? `> >("$gnk_e2b_tee" --output-error=warn-nopipe >("$gnk_e2b_head" -c ${spec.stdio.stdout.spill.maxBytes} > ${quoteE2BShellArg(paths.stdout)}) | ${encoder} 2>/dev/null)`
    : `> >(${encoder} 2>/dev/null)`
  const stderrRedirect = hasSpill(spec.stdio.stderr)
    ? `2> >("$gnk_e2b_tee" --output-error=warn-nopipe >("$gnk_e2b_head" -c ${spec.stdio.stderr.spill.maxBytes} > ${quoteE2BShellArg(paths.stderr)}) | ${encoder} >&2 2>/dev/null)`
  ...
  const inner = [
    ...
    'gnk_e2b_rm=$7',
    'shift 7',
    'gnk_e2b_pgid="$("$gnk_e2b_ps" -o pgid= -p "$$" | "$gnk_e2b_tr" -d " ")"',
    `printf '%s\\n' "$gnk_e2b_pgid" > ${quoteE2BShellArg(paths.pid)}`,
    `mapfile -d '' -t gnk_e2b_env < ${quoteE2BShellArg(paths.environment)}`,
    `"$gnk_e2b_rm" -f -- ${quoteE2BShellArg(paths.environment)}`,
    `"$gnk_e2b_env_bin" -i -- "\${gnk_e2b_env[@]}" "$@" ${stdoutRedirect} ${stderrRedirect}`.trimEnd(),
    'gnk_e2b_status=$?',
    `printf '%s\\n' "$gnk_e2b_status" > ${quoteE2BShellArg(paths.status)}`,
    'wait',
    'exit "$gnk_e2b_status"',
  ].join('\n')
  ...
  const bootstrap = [
    `mapfile -d '' -t gnk_e2b_env < ${quoteE2BShellArg(paths.environment)}`,
    'gnk_e2b_env_bin="$(command -v env)"',
    'gnk_e2b_setsid="$(command -v setsid)"',
    'gnk_e2b_bash="$(command -v bash)"',
    'gnk_e2b_node="$(command -v node)"',
    'gnk_e2b_ps="$(command -v ps)"',
    'gnk_e2b_tr="$(command -v tr)"',
    'gnk_e2b_tee="$(command -v tee)"',
    'gnk_e2b_head="$(command -v head)"',
    'gnk_e2b_rm="$(command -v rm)"',
    'for gnk_e2b_tool in "$gnk_e2b_env_bin" "$gnk_e2b_setsid" "$gnk_e2b_bash" "$gnk_e2b_node" "$gnk_e2b_ps" "$gnk_e2b_tr" "$gnk_e2b_tee" "$gnk_e2b_head" "$gnk_e2b_rm"; do',
    '  [[ "$gnk_e2b_tool" == /* && -x "$gnk_e2b_tool" ]] || exit 125',
    'done',
    `exec "$gnk_e2b_env_bin" -i -- "\${gnk_e2b_env[@]}" "$gnk_e2b_setsid" --wait -- "$gnk_e2b_bash" -c ${quoteE2BShellArg(inner)} gnk-e2b "$gnk_e2b_env_bin" "$gnk_e2b_node" "$gnk_e2b_ps" "$gnk_e2b_tr" "$gnk_e2b_tee" "$gnk_e2b_head" "$gnk_e2b_rm" ${argv}`,
  ].join('\n')
  return bootstrap
}
