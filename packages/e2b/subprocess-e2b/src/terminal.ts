...
const TERMINAL_RUNNER_SOURCE = [
  ...
  'set -euo pipefail',
  'gnk_state=$1',
  'mapfile -d \'\' -t gnk_env < "$gnk_state/environment"',
  'mapfile -d \'\' -t gnk_argv < "$gnk_state/argv"',
  'gnk_output_marker=$(<"$gnk_state/output-marker")',
  'rm -f -- "$gnk_state/environment" "$gnk_state/argv" "$gnk_state/output-marker" "$gnk_state/runner.bash"',
  'if (( ${#gnk_argv[@]} == 0 )); then',
  "  printf 'terminal runner received empty argv\\n' >&2",
  '  exit 125',
  'fi',
  'printf \'%s\' "$gnk_output_marker"',
  'exec env -i -- "${gnk_env[@]}" "${gnk_argv[@]}"',
  '',
].join('\n')
...
export async function spawnE2BTerminal(
  ...
  const outputMarker = Buffer.from(`gnk-e2b-bootstrap:${randomUUID()}`)
