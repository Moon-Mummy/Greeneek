...
import { FsTargetKey, FsVersion } from '@greeneek/gnk-fs'
import E2BFileSystem from '@greeneek/gnk-fs-e2b'
  ...
  readonly sandbox = {
    ...
    commands: {
      run: async (
        ...
        expect(home).toMatch(/^\/\.gnk-e2b-control-/)
...
describe('E2BFileSystem identity, metadata, and reads', () => {
  ...
  it('projects canonical process paths, file URLs, and containment', async () => {
    ...
    expect(fs.processPathFromHostPath('/Users/alice/.gnk/attachments/object')).toBeUndefined()
...
describe('E2BFileSystem atomic writes and edits', () => {
  it('creates owner-only files and returns metadata after the committed move', async () => {
    ...
    expect(remote.nodes.get('/workspace/new.txt')?.metadata?.['gnk-version']).toBeDefined()
