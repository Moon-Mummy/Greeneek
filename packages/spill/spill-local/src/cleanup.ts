...
/**
 * A backend-generated default root name: `gnk-spill-` plus the 6-character
 * suffix `mkdtemp` appends. Discovery matches this
 * EXACT shape, not the bare prefix, so an unrelated `gnk-spill-test-*` fixture
 * or a foreign tool's differently-shaped `gnk-spill-…` directory is never
...
export interface SweepRoot {
   ...
   * pruned. Set for DISCOVERED prior-default `gnk-spill-*` roots (one per past
...
/**
 * Discover prior default spill roots: the `gnk-spill-<6 chars>` directories
 ...
 * prefix, so an unrelated `gnk-spill-test-*` fixture or a foreign
