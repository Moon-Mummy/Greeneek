 ...
 *
 * `@greeneek/gnk-tools`' `ToolExecution` satisfies this shape, so the policy
 * reads `exec` straight through without importing `gnk-tools` or `gnk-agent`.
 * Only the session HEADER id is read — the same identity every other subsystem
 * keys off (see `gnk-tool-bash`'s owner derivation).
 *
 * @module @greeneek/gnk-spill-policy/types
 */
