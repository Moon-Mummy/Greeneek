/**
 * @greeneek/gnk-headless — one-shot direct Agent driver. The bundle patch
 * rides over gnk-base without Host, HTTP, or browser plugins; this runner
...
import { installModelSelection } from '@greeneek/gnk-agent'
import type { Agent, ModelSelectionRef } from '@greeneek/gnk-agent'
import type {} from '@greeneek/gnk-agent-default-model'
...
import type { Session, SessionEvent, SessionId, SessionLogOffset } from '@greeneek/gnk-session'
// Empty type imports carry the loader Context merge for the settlement await
// and the cmdline Context merge for the appExit host value.
import type {} from '@greeneek/cordis-plugin-loader'
import type {} from '@greeneek/gnk-cmdline'
...
function fail(io: HeadlessIo, error: unknown): void {
  io.stderr.write(`gnk: ${error instanceof Error ? error.message : String(error)}\n`)
  io.exit(1)
}
...
async function run(ctx: Context, task: string, io: HeadlessIo): Promise<void> {
  ...
  if (outcome.reason?.kind === 'error') {
    io.stderr.write(`gnk: ${outcome.reason.error.code}: ${outcome.reason.error.message}\n`)
  }
