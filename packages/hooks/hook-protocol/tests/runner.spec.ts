...
import type { ShellExecRequest, ShellExecSpec, ShellExecutor, ShellRunResult } from '@greeneek/gnk-shell'
import { DEFAULT_HOOK_TIMEOUT_MS, runHook } from '@greeneek/gnk-hook-protocol'
import type { RunHookOptions } from '@greeneek/gnk-hook-protocol'
 ...
 * executor (gnk-bash-local) is exercised end-to-end by the hook-bridge plugins
...
function recordingBash(run: (spec: ShellExecSpec) => Promise<ShellRunResult>): {
  ...
  const bash = {
    resolve(request: ShellExecRequest): ShellExecSpec {
      ...
      // exactly what gnk-bash-local's resolve does for the fields runHook sets.
      return {
