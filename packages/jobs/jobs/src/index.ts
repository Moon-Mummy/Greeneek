 ...
 * process-local registry lives in `@greeneek/gnk-jobs-local`.
 * @module @greeneek/gnk-jobs
 */
...
export abstract class JobRegistry extends Service {
  constructor(ctx: Context) {
    ...
    if (new.target === JobRegistry) {
      throw new Error('@greeneek/gnk-jobs is the abstract job registry seam; load an implementation such as @greeneek/gnk-jobs-local instead')
    }
    super(ctx, 'jobs')
  }
