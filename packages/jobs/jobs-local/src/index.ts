 ...
 * @module @greeneek/gnk-jobs-local
 */

import { Context } from '@greeneek/cordis'
import z from '@greeneek/schemastery'
import type { Agent } from '@greeneek/gnk-agent'
import { AnonymousEntries, ScopedLayers, scopeOf } from '@greeneek/gnk-scope'
import type { ScopeLayer } from '@greeneek/gnk-scope'
import { deadline, timeoutOf } from '@greeneek/gnk-timeout'
import { JobRegistry, JobId } from '@greeneek/gnk-jobs'
import type {
  JobDoneListener, JobKind, JobOutcome, JobRead, JobSnapshot, JobStart, JobStatus,
  JobsChangedListener,
} from '@greeneek/gnk-jobs'
 ...
 * `@greeneek/gnk-jobs` for the ownership, isolation, and lifecycle
...
export class LocalJobRegistry extends JobRegistry {
  ...
  start(spec: JobStart): JobId {
    if (!this.servesOwner(spec.owner)) {
      throw new Error('background jobs unavailable: no job controller serves this agent (load @greeneek/gnk-tool-jobs in its composition)')
    }
  ...
  private ensureOwnerCleanup(owner: Agent): void {
    ...
    if (agents === undefined) {
      throw new Error('background job ownership requires the agent registry (load @greeneek/gnk-agent)')
    }
