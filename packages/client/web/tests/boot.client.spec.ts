...
import * as modulesClient from '@greeneek/gnk-client-modules/client'
import type {
  ClientBundleRegistration, ClientModuleCreateOptions, ClientModuleLoaderTarget, GnkWindow,
  WebBootEntry,
...
const transportGlobal = globalThis as {
  __GNK_TRANSPORT__?: { loadBundle(url: string): Promise<void> }
}
...
afterEach(() => {
  ...
  delete transportGlobal.__GNK_TRANSPORT__
...
describe('bootstrap failure rendering', () => {
  ...
  it('renders a malformed boot manifest', async () => {
    await expectBootFailure(() => {
      installFacade()
      delete win.__GNK_BOOT__
    }, 'window.__GNK_BOOT__ is missing or not an object')
  })
...
describe('plugin activation', () => {
  it('prefetches a parser-loaded immediate row through the injected bundle transport', async () => {
    ...
    transportGlobal.__GNK_TRANSPORT__ = {
