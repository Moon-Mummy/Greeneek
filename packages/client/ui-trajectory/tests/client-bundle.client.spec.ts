 ...
 * not built (`pnpm --filter @greeneek/gnk-client-ui-trajectory bundle`).
 */
import { readFileSync } from 'node:fs'
import { resolve } from 'node:path'
import { Context } from '@greeneek/cordis'
import { stubSettingsScope } from '@greeneek/gnk-client-test-runtime'
import { afterEach, describe, expect, it } from 'vitest'
import { UiConversation } from '@greeneek/gnk-client-ui-conversation/client'
import { SlotRegistry } from '@greeneek/gnk-client-ui-renderer/client'

const PLUGIN_ID = '@greeneek/gnk-client-ui-trajectory'
...
describe('tsdown client artifact', () => {
  ...
  async function loadArtifact() {
    ...
    const modules = new Map<string, unknown>([
      ...
      ['@greeneek/gnk-client-store', await import('@greeneek/gnk-client-store')],
      ['@greeneek/gnk-client-ui-conversation/client', await import('@greeneek/gnk-client-ui-conversation/client')],
      ['@greeneek/gnk-client-ui-primitives', await import('@greeneek/gnk-client-ui-primitives')],
    ])
  ...
  it.skipIf(code === undefined)('mounted as an object plugin, apply registers the view tab on the real ring', async () => {
    ...
    const locale = await import('@greeneek/gnk-client-locale/client')
