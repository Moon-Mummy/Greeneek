/**
 * Package-owned invariant companion for `@greeneek/gnk-client-modules`.
 * @module @greeneek/gnk-client-modules/invariant
 */
...
const PACKAGE_NAME = '@greeneek/gnk-client-modules'
  ...
  ctx.on('internal/plugin', () => {
    ...
    for (const row of host.graph().entries) {
      if (host.clientPath(row.id) === undefined) {
        fail(`web plugin graph row "${row.id}" advertises ${row.url} but resolves no client bundle path — the served __GNK_BOOT__ would 404 on fetch`)
      }
    }
  }, { global: true })
}
