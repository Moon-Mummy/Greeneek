...
import { isChunkRow, packChunkRuns, type ChunkRow } from '@greeneek/gnk-session/chunk-rows'
...
import { inspectRequestPrompt } from '@greeneek/gnk-client-ui-conversation/client'
  ...
  it('orders the system field before the request messages while preserving message order', () => {
    const value = assembler([
      ...
      at(4, 'user/message', {
        ...
        source: { kind: 'plugin', plugin: '@greeneek/gnk-system-prompt', form: 'snapshot' },
      }, { surfaceOp: 'append' }),
      at(5, 'request/header', {
        reason: 'initial',
