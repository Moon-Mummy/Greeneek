 ...
 */
const CSS_VIRTUAL_PREFIX = '\0gnk-css:'
const GLOBAL_CSS_VIRTUAL_PREFIX = '\0gnk-global-css:'
const INLINE_CSS_VIRTUAL_PREFIX = '\0gnk-inline-css:'
 ...
 * Everything else under @greeneek/* is either a module-table entry
 * (external) or a leak the purity gate rejects.
 */
export const INLINE_SAFE = /^(?:@greeneek\/gnk-(?:file-reference|session|llm|tools|brand|deque|typert-protocol|util-crypto|util-values|util-workspace-path)(?:\/|$)|@greeneek\/gnk-token-meter\/client$|@greeneek\/gnk-agent-presets\/display$)/
...
const GENERATED_REMOTE = /^@greeneek\/gnk-[a-z0-9]+(?:-[a-z0-9]+)*\/remote$/
...
export function clientBundle(
  ...
  return ({ env }) => {
    const face = buildFace(env?.GNK_BUILD_FACE)
...
export function clientOnly(configs: readonly UserConfig[]): BuildFaceConfig {
  return ({ env }) => buildFace(env?.GNK_BUILD_FACE) === 'host'
...
function buildFace(value: unknown): BuildFace {
  ...
  throw new Error(`tsdown: --env.GNK_BUILD_FACE must be host or client, received ${String(value)}`)
}
...
interface WorkspaceManifest {
  ...
  readonly gnk?: { readonly client?: { readonly external?: unknown } }
}
...
/**
 * Module-table specifiers one `gnk.client` declaration requests. Matching is
 ...
 * @param declaration - the package's `gnk.client` object.
...
export function requestedExternals(
  ...
  return new Set(optionalStringArray(subject, 'gnk.client.external', declaration.external) ?? [])
}

/**
 * Module-table specifiers one package requests. The shell baseline is implicit
 * for every dynamic bundle; `gnk.client.external` only adds package-specific
...
function clientExternals(id: string): ReadonlySet<string> {
  ...
  const externals = new Set([
    ...
    ...requestedExternals(id, workspaceManifest(id).gnk?.client ?? {}),
  ])
...
function clientConfig(id: string, entry: string): UserConfig {
  ...
  return {
    ...
    plugins: [{
      ...
      // inline, and every other @greeneek value import is a build error — a
      ...
      name: 'gnk-client-bundle-purity',
      resolveId(source: string) {
        if (!source.startsWith('@greeneek/')) return null
        if (isRequested(source)) return null // requested module-table row: external wins
        if (VENDORED_LIBRARY.test(source)) return null // vendored library: inline, no shared identity
        if (INLINE_SAFE.test(source) || GENERATED_REMOTE.test(source)) return null // wire contribution: inline is the point
        throw new Error(
          `client bundle purity: "${source}" is not in the default client externals or ${id}'s gnk.client.external, an inline-safe wire layer, or a generated /remote contribution — `
...
const STATIC_LINKED_PLUGIN = 'gnk-static-linked-external'
