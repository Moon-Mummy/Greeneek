/**
 * Official-Greeneek first-run step. Readiness comes from the same
 ...
 * official Greeneek route. The step reuses that page's credential editor in
...
import styles from './GreeneekOnboardingDialog.module.css'

/** Registration-side dependencies of {@link GreeneekOnboardingDialog}. */
export interface GreeneekOnboardingInjected {
  hooks: {
...
export type GreeneekOnboardingDialogProps =
  PropsRuntime<'settings.onboarding'> & InjectFace<GreeneekOnboardingInjected>

/* v8 ignore next 3 -- closed-union defaults only defend future source widening */
function assertNever(_value: never): never {
  throw new Error('unexpected Greeneek onboarding state')
}

/**
 * Prompt a first-run user for the official Greeneek credential while no
 * provider can serve requests and that credential is writable.
 * @param props - settings-shell owner state and Models feature dependencies.
 * @returns the onboarding modal or null when onboarding needs no intervention.
 */
export function GreeneekOnboardingDialog(props: GreeneekOnboardingDialogProps): ReactNode {
  ...
  const namespace = state.namespaces.get('llm-greeneek')
