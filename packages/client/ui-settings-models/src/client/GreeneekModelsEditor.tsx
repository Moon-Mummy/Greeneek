/**
 * Curated editor for the direct Greeneek adapter's advisory model catalog.
...
export type GreeneekModelDraft = Record<string, unknown>
 ...
 * (rejected by {@link validateGreeneekModels} before any write).
 */
...
export interface GreeneekModelsValidationFailure {
...
export function modelDrafts(value: unknown): GreeneekModelDraft[] {
  if (!Array.isArray(value)) return []
  return value.map(entry =>
    typeof entry === 'object' && entry !== null && !Array.isArray(entry)
      ? entry as GreeneekModelDraft
      : {})
}
 ...
 */
export function validateGreeneekModels(value: unknown): GreeneekModelsValidationFailure | undefined {
...
/** Props of {@link GreeneekModelsEditor}. */
export interface GreeneekModelsEditorProps {
  /** Effective rows: inherited until the parent materializes an override. */
  models: readonly GreeneekModelDraft[]
  ...
  onChange: (models: GreeneekModelDraft[]) => void
  /** Remove the user-owned array and return to inheritance. */
  onReset: () => void
}

/**
 * Render the direct Greeneek adapter's model catalog: id and display name on
 * each row, capacities behind the row's own disclosure.
 * @param props - effective rows plus the array-level override actions.
 * @returns the catalog editor.
 */
export function GreeneekModelsEditor(props: GreeneekModelsEditorProps): ReactNode {
  ...
  const capacityText = (model: GreeneekModelDraft, index: number, field: CapacityField): string => {
