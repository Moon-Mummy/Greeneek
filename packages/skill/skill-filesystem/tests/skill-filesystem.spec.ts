...
import { FileSystem, FsError, FsVersion, type FsDirEntry, type FsEditOutcome, type FsEditRequest, type FsInfo, type FsPathInfo, type FsTarget, type FsWriteOutcome } from '@greeneek/gnk-fs'
import * as SkillFileSystem from '../src/index.ts'

async function tempDir(name: string): Promise<string> {
  return await import('node:fs/promises').then(fs => fs.mkdtemp(join(tmpdir(), `gnk-${name}-`)))
}
...
describe('gnk-skill-filesystem plugin exports', () => {
...
describe('FileSystemSkillProvider', () => {
  it('discovers project, custom, user, and agents skill roots in priority order', async () => {
    ...
    await writeSkill(join(home, '.gnk/skills'), 'same', 'user gnk skill')
    await writeSkill(custom, 'same', 'custom skill')
    await writeSkill(join(project, '.agents/skills'), 'same', 'project agents skill')
    await writeSkill(join(project, '.gnk/skills'), 'same', 'project gnk skill')
    await writeSkill(custom, 'custom-only', 'custom only')
    await writeSkill(join(home, '.gnk/skills/.system'), 'hidden-system', 'hidden system')
    ...
    expect(skills.find(skill => skill.name === 'same')?.description).toBe('project gnk skill')
    expect(skills.find(skill => skill.name === 'same')?.source).toBe('project-gnk')
    ...
    await writeSkill(join(noGit, '.gnk/skills'), 'fallback-root', 'Fallback root')
  ...
  it('lets project skills override runtime while runtime overrides custom and user skills', async () => {
    ...
    await writeSkill(join(project, '.gnk/skills'), 'project-name', 'Project wins')
    await writeSkill(custom, 'runtime-name', 'Custom loses')
    await writeSkill(join(home, '.gnk/skills'), 'runtime-name', 'User loses')
  ...
  it('parses flat skills and filters invalid skills from the invocation-neutral listing', async () => {
    ...
    const root = join(home, '.gnk/skills')
  ...
  it('discovers symlinked skill directories and flat files', async () => {
    ...
    await mkdir(join(home, '.gnk/skills'), { recursive: true })
    await symlink(join(external, 'linked-dir'), join(home, '.gnk/skills/linked-dir'))
    await symlink(join(external, 'linked-flat.md'), join(home, '.gnk/skills/linked-flat.md'))
    await symlink(join(external, 'missing'), join(home, '.gnk/skills/broken-link'))
    await symlink('/dev/null', join(home, '.gnk/skills/device-link'))
  ...
  it('uses the filesystem service for discovery, reads, and project-root lookup', async () => {
    ...
    await ctx.plugin(SkillRegistry)
    await ctx.plugin(SkillFileSystem, { gnkHome: join(home, '.gnk'), agentsHome: join(home, '.agents'), watch: false })
  ...
  it('forwards cancellation to filesystem reads while loading a skill', async () => {
    ...
    await writeSkill(join(home, '.gnk/skills'), 'abortable-skill', 'Abortable skill')
  ...
  it('uses fs/observed as a synchronous first-party invalidation path without a watcher', async () => {
    ...
    emitObserved(join(home, '.gnk/skills/.system/SKILL.md'), { name: 'write' })
  ...
  it('uses default home root resolution without exposing builtin skills', async () => {
    const previousGnkHome = process.env.GNK_HOME
    const previousAgentsHome = process.env.GNK_AGENTS_HOME
    const previousBundledSkillDir = process.env.GNK_BUNDLED_SKILL_DIR
    const envHome = await tempDir('skill-env-home')
    try {
      process.env.GNK_HOME = join(envHome, '.gnk')
      process.env.GNK_AGENTS_HOME = join(envHome, '.agents')
      const bundled = join(envHome, 'bundled-skills')
      process.env.GNK_BUNDLED_SKILL_DIR = bundled
      await writeSkill(join(envHome, '.gnk/skills'), 'env-skill', 'Env skill')
      ...
      process.env.GNK_HOME = join(envHome, 'empty-gnk')
      delete process.env.GNK_BUNDLED_SKILL_DIR
      process.env.GNK_AGENTS_HOME = join(envHome, 'empty-agents')
      const empty = new Context()
      await empty.plugin(SkillRegistry)
      SkillFileSystem.apply(empty, { watch: false })
      expect(await empty.skills.list()).toEqual([])

      delete process.env.GNK_AGENTS_HOME
      expect(new SkillFileSystem.FileSystemSkillProvider(empty, {
        signal: new AbortController().signal,
        invalidate() {},
      }, { gnkHome: join(envHome, 'empty-gnk') }).name).toBe('filesystem')
    } finally {
      if (previousGnkHome === undefined) {
        delete process.env.GNK_HOME
      } else {
        process.env.GNK_HOME = previousGnkHome
      }
      if (previousAgentsHome === undefined) {
        delete process.env.GNK_AGENTS_HOME
      } else {
        process.env.GNK_AGENTS_HOME = previousAgentsHome
      }
      if (previousBundledSkillDir === undefined) {
        delete process.env.GNK_BUNDLED_SKILL_DIR
      } else {
        process.env.GNK_BUNDLED_SKILL_DIR = previousBundledSkillDir
      }
    }
  })
})
