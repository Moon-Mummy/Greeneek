...
import { resolveConfigPath } from '@greeneek/gnk-app-boot'
import { recordFeedback } from '@greeneek/gnk-command-feedback'
import { runFixtureTurn } from '@greeneek/gnk-loader-smoke'
...
process.env.GNK_TELEMETRY_E2E_URL = `http://127.0.0.1:${address.port}/v1/logs`
process.env.GNK_TELEMETRY_OTLP_URL = process.env.GNK_TELEMETRY_E2E_URL
process.env.GNK_TELEMETRY_MODE = process.env.GNK_TELEMETRY_E2E_MODE ?? 'FULL'
...
try {
  ...
  const mode = process.env.GNK_TELEMETRY_E2E_MODE ?? 'FULL'
  if (mode !== 'FULL') {
