...
import { recordFeedback } from '@greeneek/gnk-command-feedback'
...
// pin GNK_HOME to a temp dir so the suite never touches the ambient ~/.gnk.
let tempHome: string
let previousGnkHome: string | undefined
beforeAll(() => {
  tempHome = mkdtempSync(join(tmpdir(), 'gnk-otel-home-'))
  previousGnkHome = process.env.GNK_HOME
  process.env.GNK_HOME = tempHome
})
afterAll(() => {
  if (previousGnkHome === undefined) delete process.env.GNK_HOME
  else process.env.GNK_HOME = previousGnkHome
...
describe('OpenTelemetrySessionBackend wire', () => {
  it('ships session records and the ops shutdown marker through the real SDK pipeline', async () => {
    ...
    expect(resource).toContainEqual({ key: 'service.name', value: { stringValue: 'greeneek-harness' } })
    expect(resource).toContainEqual({ key: 'user.id', value: { stringValue: getOrCreateAnonymousUserId() } })

    const records = allRecords(captures)
    const ledger = records.filter(r => r.scope === '@greeneek/gnk-session-telemetry-otel')
    const ops = records.filter(r => r.scope === '@greeneek/gnk-session-telemetry-otel/ops')
  ...
  it('drains records enqueued after a timer export began: dispose during an in-flight batch', async () => {
    ...
    const ops = allRecords(captures).filter(r => r.scope === '@greeneek/gnk-session-telemetry-otel/ops')
    expect(ops).toHaveLength(1)
    expect(ops[0]!.record.attributes).toContainEqual({ key: 'telemetry.op', value: { stringValue: 'shutdown' } })
  })
...
describe('gnk-session-telemetry-otel real-load-path guard', () => {
