...
import InvariantRegistry, { InvariantError } from '@greeneek/gnk-invariants'
...
describe('Greeneek session-log acceptance invariant', () => {
  it('accepts a watermark naming an earlier event in its containing Session', async () => {
    ...
    expect(() => session.append('session-log-greeneek/delivery-accepted', { sessionId: session.id, throughSeq: SessionSeq(0) }))
      .not.toThrow()
  })
  ...
  it('rejects a live watermark for another Session or a non-earlier sequence', async () => {
    ...
    expect(() => wrongId.append('session-log-greeneek/delivery-accepted', {
      sessionId: SessionId('other'),
      throughSeq: SessionSeq(0),
    })).toThrow(expect.objectContaining<Partial<InvariantError>>({
      code: 'INVARIANT',
      packageName: '@greeneek/gnk-session-log-greeneek',
    }))

    const wrongSeq = ctx.sessions.create(SessionId('wrong-seq'))
    wrongSeq.append('turn/start', { turn: 1 })
    expect(() => wrongSeq.append('session-log-greeneek/delivery-accepted', {
