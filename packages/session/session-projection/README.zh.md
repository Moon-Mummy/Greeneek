...
在客户端载体需要日志派生会话状态的当前值处挂载 `gnk-session-projection`。领域插件注册单元；载体读取快照并订阅变更流；两侧互不相识。
...
- **注册表 cell 只活在内存里**——重启后首次触达时靠折叠日志重建；挂载了 `gnk-session-projection-cache` 的组合改由持久行播种该折叠。
