...
import { normalizeTerminalText, TerminalSanitizer } from '@greeneek/gnk-terminal-bash/src/sanitize.ts'
...
describe('TerminalSanitizer', () => {
  it('removes split CSI and owned OSC prompt markers', () => {
    ...
    expect(sanitizer.push('D;0\x07gnk> ')).toEqual({ text: 'gnk> ', prompt: true, promptTail: 'gnk> ' })
  })
  ...
  it('reports printable prompt text that follows a marker in a later chunk', () => {
    ...
    expect(sanitizer.push('gnk> ')).toEqual({ text: 'gnk> ', prompt: false, promptTail: 'gnk> ' })
  })
