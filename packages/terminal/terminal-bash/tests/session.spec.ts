...
import { LocalPtySession } from '@greeneek/gnk-terminal-bash/src/session.ts'
import type { ResolvedConfig } from '@greeneek/gnk-terminal-bash/src/config.ts'
import type { TerminalSendOperation, TerminalSessionStatus, TerminalSignal } from '@greeneek/gnk-terminal'
import type {
  SubprocessOutcome,
...
import type {
  ProcessIdentity,
  ProcessInspector,
} from '@greeneek/gnk-subprocess-local/src/process-inspector.ts'
...
async function initialize(session: LocalPtySession, terminal: FakeTerminal): Promise<void> {
  ...
  terminal.emitData('\x1b]133;D;0\x07gnk> ')
...
describe('LocalPtySession readiness and output', () => {
  it('answers split cursor-position queries before publishing prompt readiness', async () => {
    ...
    terminal.emitData('\x1b]133;D;0\x07gnk> \x1b[')
  ...
  it('cancels with foreground-group SIGINT, observes AbortSignal, and contains write failures', async () => {
    ...
    terminal.emitData('\x1b]133;D;130\x07gnk> ')
    await vi.advanceTimersByTimeAsync(10)
  ...
  it('does not attribute a delayed prior prompt to the current send', async () => {
    ...
    terminal.emitData('\x1b]133;D;0\x07gnk> printf \'PID=%s\\n\' "$!"\r\n')
    await vi.advanceTimersByTimeAsync(20)
    expect(settled).toBe(false)
