...
import AgentRegistry, { Inbox, type Agent } from '@greeneek/gnk-agent'
...
import TerminalSessionService, { TerminalBackendCleanupError, TerminalSessionId } from '@greeneek/gnk-terminal'
import type { TerminalSendRequest, TerminalWaitReason } from '@greeneek/gnk-terminal'
import { BashTerminalBackend, PWSH_PROMPT_SETUP } from '@greeneek/gnk-terminal-bash'
import { ENCODING_PREAMBLE } from '@greeneek/gnk-pwsh-local'
import * as ptyLocal from '@greeneek/gnk-terminal-bash'
import type { ResolvedConfig } from '@greeneek/gnk-terminal-bash/src/config.ts'
import type { LocalPtySession } from '@greeneek/gnk-terminal-bash/src/session.ts'
...
describe('BashTerminalBackend startup rollback', () => {
  ...
  it('wraps confined argv, scrubs the environment, and returns initialized sessions', async () => {
    ...
    expect(spawned).toMatchObject({
      ...
      graceMs: 10,
      env: {
        TERM: 'dumb', PAGER: 'cat', GIT_PAGER: 'cat', PS1: 'gnk> ', BASH_SILENCE_DEPRECATION_WARNING: '1',
        PROMPT_COMMAND: 'printf "\\033]133;D;%s\\007" "$?"; PS1=\'gnk> \'',
        GNK_SHELL: '1', GNK_SESSION_ID: 'agent', GNK_PTY_SESSION_ID: 'pty-1',
      },
    })
  ...
  it('composes the default local session around a spawned terminal', async () => {
    ...
    queueMicrotask(() => { output.write(Buffer.from('\x1b]133;D;0\x07gnk> ')) })
    const backend = new BashTerminalBackend(
      ctx,
      config(),
      async () => terminal,
  ...
  it('bootstraps a pwsh dialect through the prompt function and scrubs bash-only env', async () => {
    ...
    const session = {
      ...
      startSend: (request: TerminalSendRequest) => {
        ...
        return {
            ...
            viewport: 'setup-echo gnk> ', waitReason: 'stdin_read' as const,
    ...
    expect(session.motd).toBe('setup-echo gnk> ')
    expect(spawned?.env).toMatchObject({
      TERM: 'dumb', NO_COLOR: '1', GNK_SHELL: '1', GNK_SESSION_ID: 'agent', GNK_PTY_SESSION_ID: 'pty-1',
    })
    ...
    const session = {
      ...
      startSend: (request: TerminalSendRequest) => {
        ...
        return {
            ...
            viewport: second ? 'gnk> ' : "function prompt { 'gnk> ' }\n",
    ...
    const session = {
      ...
      startSend: (request: TerminalSendRequest) => {
        ...
        return {
            ...
            viewport: 'gnk> ', waitReason: 'stdin_read' as const,
            sessionStatus: { kind: 'running' as const }, truncated: false,
          }),
          readOutput: () => ({ delta: '', truncated: false }),
          cancel: () => false,
