...
import GreeneekLlmApiExtensionRegistry from '../src/index.ts'
...
describe('GreeneekLlmApiExtensionRegistry', () => {
  it('prepares detached fields and accepts every provider exactly once', async () => {
    const ctx = await harness()
    const first = vi.fn()
    const second = vi.fn()
    const mutable = { value: 'original' }
    ctx.greeneekLlmApiExtensions.register('test_alpha', {
      prepare: () => ({
        value: mutable,
        accept: first,
      }),
    })
    ctx.greeneekLlmApiExtensions.register('test_beta', {
    ...
    const prepared = await ctx.greeneekLlmApiExtensions.prepare({ body: { messages: [] }, signal: SIGNAL, sessionId: 's' })
  ...
  it('preserves the prepared result as an acceptance method receiver', async () => {
    ...
    ctx.greeneekLlmApiExtensions.register('test_alpha', { prepare: () => result })

    const prepared = await ctx.greeneekLlmApiExtensions.prepare({ body: {}, signal: SIGNAL })
  ...
  it('rejects duplicate fields and releases ownership with the registering fiber', async () => {
    ...
    const dispose = owner.greeneekLlmApiExtensions.register('test_alpha', {
      prepare: () => ({ value: { value: 'one' } }),
    })
    expect(() => ctx.greeneekLlmApiExtensions.register('test_alpha', {
    ...
    await expect(ctx.greeneekLlmApiExtensions.prepare({ body: {}, signal: SIGNAL }))
  ...
  it('reports a single acceptance failure verbatim and omits an undefined contribution', async () => {
    ...
    ctx.greeneekLlmApiExtensions.register('test_beta', { prepare: () => undefined })
    const prepared = await ctx.greeneekLlmApiExtensions.prepare({ body: {}, signal: SIGNAL })
  ...
  it('rejects invalid field names and preparation failures before returning fields', async () => {
    const ctx = await harness()
    expect(() => ctx.greeneekLlmApiExtensions.register('' as 'test_alpha', {
      prepare: () => ({ value: { value: 'x' } }),
    })).toThrow(/non-blank trimmed/)
    ctx.greeneekLlmApiExtensions.register('test_alpha', {
      prepare: () => { throw new Error('prepare failed') },
    })
    await expect(ctx.greeneekLlmApiExtensions.prepare({ body: {}, signal: SIGNAL })).rejects.toThrow('prepare failed')
  })
  ...
  it('stops waiting for a provider that ignores request cancellation', async () => {
    ...
    const pending = ctx.greeneekLlmApiExtensions.prepare({
      body: {},
