 ...
 *
 * @module gnk-llm-pi-ai/replay
 */

import { LlmError } from '@greeneek/gnk-llm'
import type { Message, ModelMessageSource, ReplayEnvelope } from '@greeneek/gnk-llm'
...
function foreignAssistant(message: Message): AssistantMessage {
  ...
  return {
    ...
    api: 'gnk-foreign',
    provider: source?.provider ?? 'gnk-foreign',
    model: source?.model ?? 'gnk-foreign',
    usage: emptyPiUsage(),
