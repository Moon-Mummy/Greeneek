/**
 * Active Loader-backed plugin package inventory for official Greeneek requests.
 ...
 * @module @greeneek/gnk-plugin-package-inventory-greeneek
 */
...
import type { Entry, EntryTree } from '@greeneek/cordis-plugin-loader'
...
import type { GreeneekPluginPackageIdentity, GreeneekPluginPackageInventoryExtension } from './types.ts'
import type {} from './types.ts'

export type * from './types.ts'

/** Cordis plugin name. */
export const name = 'plugin-package-inventory-greeneek'
/** Services required to locate host/requesting-agent entries and contribute the field. */
export const inject = ['agents', 'greeneekLlmApiExtensions', 'loader']
...
function identityFromManifest(path: string, allowAnonymous: boolean): GreeneekPluginPackageIdentity | undefined {
  const manifest = JSON.parse(readFileSync(path, 'utf8')) as PackageManifest
  if (allowAnonymous && manifest.name === undefined) return undefined
  if (typeof manifest.name !== 'string' || manifest.name.length === 0
    || typeof manifest.version !== 'string' || manifest.version.length === 0) {
    throw new Error(`plugin-package-inventory-greeneek: ${path} must declare non-empty name and version`)
  }
...
class PackageIdentityResolver {
  ...
  private readonly cache = new Map<string, GreeneekPluginPackageIdentity | undefined>()
  ...
  resolve({ entry, bareBaseUrl }: ActiveEntry): GreeneekPluginPackageIdentity | undefined {
    ...
    if (packageName !== undefined) {
      ...
      if (manifest === undefined) {
        throw new Error(`plugin-package-inventory-greeneek: cannot resolve active package ${JSON.stringify(packageName)}`)
      }
...
async function collectActivePluginPackages(
  ...
  hostBaseUrl: string,
  sessionId?: string,
): Promise<GreeneekPluginPackageIdentity[]> {
  ...
  if (sessionId !== undefined && ctx.get('agentPresets') !== undefined) {
    ...
    if (agent !== undefined) {
      ...
      const { standingMountFor } = await import('@greeneek/gnk-agent-presets')
  ...
  const unique = new Map<string, GreeneekPluginPackageIdentity>()
...
/**
 * Register the complete `gnk_plugin_packages` request contribution when enabled.
 * @param ctx - plugin context carrying Loader provenance and the Greeneek request-extension registry.
...
export function apply(ctx: Context, config: Config): void {
  ...
  ctx.greeneekLlmApiExtensions.register('gnk_plugin_packages', {
    prepare: async (request) => {
      const value: GreeneekPluginPackageInventoryExtension = {
        version: 1,
