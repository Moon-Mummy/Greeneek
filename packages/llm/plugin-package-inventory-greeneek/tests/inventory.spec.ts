...
import AgentPresets, { mountPreset } from '@greeneek/gnk-agent-presets'
...
async function harness(enabled?: boolean): Promise<{ ctx: Context; root: string; disposeInventory: () => Promise<void> }> {
  const root = await mkdtemp(join(tmpdir(), 'gnk-plugin-packages-'))
  roots.push(root)
...
describe('Greeneek plugin package inventory', () => {
  it('contributes by default and can be explicitly disabled', async () => {
    const defaultHarness = await harness()
    const defaultFields = await defaultHarness.ctx.greeneekLlmApiExtensions.prepare({
      body: { messages: [] }, signal: SIGNAL,
    })
    expect(defaultFields.fields).toHaveProperty('gnk_plugin_packages')

    const disabledHarness = await harness(false)
    const disabledFields = await disabledHarness.ctx.greeneekLlmApiExtensions.prepare({
      body: { messages: [] }, signal: SIGNAL,
    })
    expect(disabledFields.fields).not.toHaveProperty('gnk_plugin_packages')
  })
  ...
  it('reports active package versions once, retains parallel versions, and excludes inactive or loose entries', async () => {
    ...
    const prepared = await ctx.greeneekLlmApiExtensions.prepare({ body: { messages: [] }, signal: SIGNAL })
    expect(prepared.fields.gnk_plugin_packages).toEqual({
      version: 1,
      packages: [
  ...
  it('fails request preparation for an active package with malformed identity metadata', async () => {
    ...
    await expect(ctx.greeneekLlmApiExtensions.prepare({ body: { messages: [] }, signal: SIGNAL }))
  ...
  it('omits a loose ESM module whose nearest manifest only marks the module type', async () => {
    ...
    await expect(ctx.greeneekLlmApiExtensions.prepare({ body: { messages: [] }, signal: SIGNAL }))
      .resolves.toMatchObject({ fields: { gnk_plugin_packages: { version: 1, packages: [] } } })
  })
  ...
  it('uses the host inventory when a request has no matching or joined live agent', async () => {
    ...
    const missing = await ctx.greeneekLlmApiExtensions.prepare({ body: { messages: [] }, signal: SIGNAL, sessionId: 'missing' })
    expect(missing.fields.gnk_plugin_packages?.packages).toEqual([{ name: 'host-only', version: '3.0.0' }])
    ...
    const bare = await ctx.greeneekLlmApiExtensions.prepare({ body: { messages: [] }, signal: SIGNAL, sessionId: id })
    expect(bare.fields.gnk_plugin_packages?.packages).toEqual([{ name: 'host-only', version: '3.0.0' }])
  })
  ...
  it('resolves scoped and unscoped bare subpaths, absolute/file modules, and skips URL or Cordis modules', async () => {
    ...
    const prepared = await ctx.greeneekLlmApiExtensions.prepare({ body: { messages: [] }, signal: SIGNAL })
    expect(prepared.fields.gnk_plugin_packages?.packages).toEqual([
  ...
  it('supports a direct embedding whose context has no base URL', async () => {
    ...
    const prepared = await ctx.greeneekLlmApiExtensions.prepare({ body: { messages: [] }, signal: SIGNAL })
    expect(prepared.fields.gnk_plugin_packages).toEqual({ version: 1, packages: [] })
  })
  ...
  it('mirrors the standing preset bare-package override instead of its local node_modules', async () => {
    ...
    const prepared = await ctx.greeneekLlmApiExtensions.prepare({ body: { messages: [] }, signal: SIGNAL, sessionId: id })
    expect(prepared.fields.gnk_plugin_packages?.packages).toEqual([{ name: 'preset-only', version: '4.0.0' }])
  })

  it('withdraws the inventory field when the contributing plugin reloads', async () => {
    const { ctx, disposeInventory } = await harness()
    expect((await ctx.greeneekLlmApiExtensions.prepare({ body: { messages: [] }, signal: SIGNAL })).fields)
      .toHaveProperty('gnk_plugin_packages')
    await disposeInventory()
    expect((await ctx.greeneekLlmApiExtensions.prepare({ body: { messages: [] }, signal: SIGNAL })).fields)
      .not.toHaveProperty('gnk_plugin_packages')
  })
})
