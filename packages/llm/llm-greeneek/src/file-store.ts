/** Greeneek Files API upload reuse, invalidation, and quota recovery. @module gnk-llm-greeneek/file-store */

import type { RequestImageAttachment } from '@greeneek/gnk-attachment'
import { LlmError } from '@greeneek/gnk-llm'
import { GreeneekFilesClient, isFilesQuotaError } from './files-api.ts'
import type { GreeneekFileId } from './file-id.ts'
import { greeneekFileScope, GreeneekUploadIndex } from './upload-index.ts'
import type { GreeneekUploadRecord } from './upload-index.ts'

/** Greeneek chat accepts at most 32 MiB per image even when it is referenced by file id. */
export const MAX_CHAT_IMAGE_BYTES = 32 * 1024 * 1024
const OWNED_FILE_PREFIX = 'gnk-'

/** Resolved file-store policy from the plugin configuration. */
export interface GreeneekFilePolicy {
...
export interface GreeneekFileConnection {
  baseURL: string
  apiKey: string
}

/** Result of one file-id resolution. */
export interface GreeneekFileReference {
  record: GreeneekUploadRecord
  uploaded: boolean
}

interface FileStoreOptions {
  index?: GreeneekUploadIndex
  now?: () => number
  fetch?: typeof fetch
}
...
interface SharedUpload {
  ...
  promise: Promise<GreeneekFileReference>
  settled: boolean
  waiters: number
}
...
function abortReason(signal: AbortSignal): Error {
  ...
  return reason instanceof Error
    ? reason
    : new Error('Greeneek file upload cancelled with a non-Error reason.', { cause: reason })
}

function uploadFailure(error: unknown): Error {
  return error instanceof Error
    ? error
    : new Error('Greeneek file upload failed with a non-Error reason.', { cause: error })
}

function waitForUpload(operation: SharedUpload, signal: AbortSignal | undefined): Promise<GreeneekFileReference> {
  ...
  return new Promise<GreeneekFileReference>((resolve, reject) => {
    const abort = (): void => {
...
/** User-scoped durable file-id reuse for the Greeneek route. */
export class GreeneekFileStore {
  private readonly index: GreeneekUploadIndex
  ...
  constructor(options: FileStoreOptions = {}) {
    this.index = options.index ?? new GreeneekUploadIndex()
    this.now = options.now ?? Date.now
    this.fetchImpl = options.fetch
  }

  private client(connection: GreeneekFileConnection): GreeneekFilesClient {
    return new GreeneekFilesClient({
  ...
  ensureUploaded(
    ...
    connection: GreeneekFileConnection,
    policy: GreeneekFilePolicy,
    signal?: AbortSignal,
  ): Promise<GreeneekFileReference> {
    signal?.throwIfAborted()
    const scope = greeneekFileScope(connection.baseURL, connection.apiKey)
  ...
  private async ensureUploadedOnce(
    ...
    if (version.bytes > MAX_CHAT_IMAGE_BYTES) {
      throw new LlmError('Greeneek chat image exceeds the 32 MiB per-image limit.', 'INVALID_REQUEST')
    }
    const scope = greeneekFileScope(connection.baseURL, connection.apiKey)
    const now = this.now()
    ...
    const upload = async (): Promise<GreeneekUploadRecord> => {
      ...
      if (remote.bytes !== version.data.byteLength) {
        throw new LlmError('Greeneek Files API upload response does not match the submitted image.', 'INVALID_RESPONSE')
      }
      return {
        scope,
    ...
    let candidate: GreeneekUploadRecord
    try {
  ...
  async invalidate(
    ...
    await this.index.remove(
      greeneekFileScope(connection.baseURL, connection.apiKey),
      version.variantId,
      fileId,
    )
  }
  ...
  async reclaimOldestOwned(
    ...
    const client = this.client(connection)
    let after: GreeneekFileId | undefined
    const owned: GreeneekFileId[] = []
   ...
   */
  async releaseAll(connection: GreeneekFileConnection, signal?: AbortSignal): Promise<number> {
    let total = 0
    for (;;) {
    ...
    await this.index.clear(greeneekFileScope(connection.baseURL, connection.apiKey))
    return total
  }
}
