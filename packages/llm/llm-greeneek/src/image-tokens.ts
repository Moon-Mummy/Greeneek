/**
 * Greeneek v4 vision-token accounting: the provider's published image-token
 * calculator (api-docs.greeneek.dev, Token & Token Usage) ported verbatim.
 ...
 *
 * @module gnk-llm-greeneek/image-tokens
 */
...
function solveResizeRatio(height: number, width: number, budget: number): GridResize {
  ...
  /* v8 ignore start -- unreachable at the provider budget: idealGridWidth >= 1
    ...
    if (!(solvedGridWidth > 1)) throw new Error('greeneek image tokens: no grid fits the token budget')
    bestWidth = solvedGridWidth * PATCH_SIZE * DOWNSAMPLE_RATIO
    bestHeight = solvedGridHeight * PATCH_SIZE * DOWNSAMPLE_RATIO
  /* v8 ignore stop */
  } else {
...
/**
 * Vision tokens Greeneek v4 charges for one request image of the given
 ...
 */
export function greeneekImageTokens(width: number, height: number): number {
  ...
  throw new Error(`greeneek image tokens: resize did not converge for ${width}x${height}`)
}
