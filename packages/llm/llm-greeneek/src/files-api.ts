/** OpenAI-compatible Greeneek Files API transport. @module gnk-llm-greeneek/files-api */

import { attributionHeaders, LlmError } from '@greeneek/gnk-llm'
import type { ImageMediaType } from '@greeneek/gnk-attachment'
import { GreeneekFileId } from './file-id.ts'
import type { GreeneekFileId as GreeneekFileIdType } from './file-id.ts'
...
export interface GreeneekFileObject {
  id: GreeneekFileIdType
  bytes: number
  createdAt: number
  filename: string
  purpose: 'user_data'
...
export interface GreeneekFilePage {
  data: GreeneekFileObject[]
  firstId?: GreeneekFileIdType
  lastId?: GreeneekFileIdType
  hasMore: boolean
}

/** Files API operation failure with its HTTP status retained for recovery policy. */
export class GreeneekFilesError extends LlmError {
 ...
 */
export function isFilesQuotaError(error: unknown): error is GreeneekFilesError {
  return error instanceof GreeneekFilesError
...
function invalidResponse(operation: string): LlmError {
  return new LlmError(`Greeneek Files API returned an invalid ${operation} response.`, 'INVALID_RESPONSE')
}

function parseFileObject(value: unknown, operation: string): GreeneekFileObject {
...
export class GreeneekFilesClient {
  ...
  private async request(path: string, init: RequestInit, signal?: AbortSignal): Promise<Response> {
    ...
    try {
      ...
      throw new LlmError(`Greeneek Files API request to ${this.baseURL} failed`, 'TRANSPORT', { cause: error })
    }
    ...
    throw new GreeneekFilesError(
      message ?? `Greeneek Files API error (HTTP ${response.status})`,
      response.status,
      detail,
    )
  }
  ...
  async upload(input: {
    ...
    signal?: AbortSignal
  }): Promise<GreeneekFileObject & { expiresAt: number }> {
    if (input.data.byteLength > MAX_FILE_UPLOAD_BYTES) {
      throw new LlmError('Greeneek Files API upload exceeds 128 MiB.', 'INVALID_REQUEST')
    }
    if (!Number.isSafeInteger(input.expiresAfterSeconds)
      ...
      throw new LlmError('Greeneek file expiry must be between 3600 and 2592000 seconds.', 'INVALID_REQUEST')
    }
   ...
   */
  async list(options: {
    after?: GreeneekFileIdType
    limit?: number
    order?: 'asc' | 'desc'
    signal?: AbortSignal
  } = {}): Promise<GreeneekFilePage> {
    ...
    return {
      ...
      ...typeof wire.first_id === 'string' ? { firstId: GreeneekFileId(wire.first_id) } : {},
      ...typeof wire.last_id === 'string' ? { lastId: GreeneekFileId(wire.last_id) } : {},
   ...
   */
  async retrieve(fileId: GreeneekFileIdType, signal?: AbortSignal): Promise<GreeneekFileObject> {
   ...
   * @param signal - request cancellation.
   */
  async delete(fileId: GreeneekFileIdType, signal?: AbortSignal): Promise<void> {
