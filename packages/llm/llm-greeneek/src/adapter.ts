/**
 * `GreeneekAdapter`: fetch + SSE against a Greeneek (OpenAI-compatible)
 * chat-completions endpoint, emitting harness StreamChunks. The adapter is
 * transport-only: connection facts arrive through a thunk resolved once per
 * operation and the bearer token through a per-request resolver, so the
 * registering plugin owns validation, layering, and credential policy.
 *
 * @module gnk-llm-greeneek/adapter
 */

import { attributionHeaders, contentHasImage, CONTEXT_WINDOW_EXCEEDED_CODE, isContextWindowExceededError, isQuotaExceededError, LlmAdapter, LlmError, offloadedImageText, offloadRequestImagesWithPolicy, ProviderRequestId, QUOTA_EXCEEDED_CODE, ReasoningEffortId } from '@greeneek/gnk-llm'
import type {
  ContentBlock,
  GenerateOptions,
...
import { deadline, idleWatchdog, timeoutOf } from '@greeneek/gnk-timeout'
import type { AnonymousUserId } from '@greeneek/gnk-anonymous-user-id'
import type {
  GreeneekLlmApiExtensionRequest,
  GreeneekLlmApiJson,
  PreparedGreeneekLlmApiExtensions,
} from '@greeneek/gnk-greeneek-llm-api-extensions'
import { serializeRequest, serializeRequestWithImages } from './serialize.ts'
import type { ImageWireLocation, RequestDefaults } from './serialize.ts'
import { greeneekImageRequestPricing, resolveRequestImagePolicy } from './request-pricing.ts'
import { GreeneekFileStore } from './file-store.ts'
import type { GreeneekFilePolicy } from './file-store.ts'
import type { GreeneekFileId } from './file-id.ts'
 ...
 */
export interface GreeneekConnectionOptions {
  ...
  models: readonly GreeneekCatalogModel[]
  ...
  filePolicy: GreeneekFilePolicy
  /** Provider-owned model-request retry policy, already resolved. */
  retryPolicy: ResolvedRetryPolicy
}

/** Constructor options for {@link GreeneekAdapter}: the operation-local resolution hooks the plugin owns. */
export interface GreeneekAdapterOptions {
  /** Current validated connection facts; called once per operation. */
  options: () => GreeneekConnectionOptions
  /**
   ...
   */
  resolveApiKey: (connection: GreeneekConnectionOptions) => Promise<string>
  ...
  resolveFiles?: () => GreeneekFileStore
  /** Prepare the official API's plugin-contributed top-level fields for one exact wire request. */
  prepareExtensions: (request: GreeneekLlmApiExtensionRequest) => Promise<PreparedGreeneekLlmApiExtensions>
}
...
const FILES_API_TIMEOUT_CODE = 'GREENEEK_FILES_API_TIMEOUT'
...
class FileResolutionFailure extends Error {
  constructor(cause: unknown) {
    super('Greeneek Files API could not resolve a request image.', { cause })
...
function detailNamesFileId(detail: string, fileId: GreeneekFileId): boolean {
...
function normalizedImageDiagnostic(
  ...
  if (target !== undefined) {
    return `Greeneek rejected normalized image ${normalizedImageFacts(target)}: ${providerMessage}. `
  ...
  return `Greeneek rejected a normalized request image: ${providerMessage}. Candidate images: `
    + `${candidates.map(normalizedImageFacts).join('; ')}. `
    + 'The provider rejected bytes already normalized by the harness; PNG, JPEG, WebP, and GIF remain supported input formats.'
}

function modelInfo(provider: string, model: GreeneekCatalogModel): LlmModelInfo {
  return {
    provider,
    id: model.id,
...
function requestId(headers: Headers): ReturnType<typeof ProviderRequestId> | undefined {
  const value = headers.get('x-request-id') ?? headers.get('x-greeneek-request-id')
 ...
 */
export class GreeneekAdapter extends LlmAdapter {
  private readonly files: GreeneekFileStore

  constructor(private readonly config: GreeneekAdapterOptions) {
    super()
    this.files = config.resolveFiles?.() ?? new GreeneekFileStore()
  }

  override providerInfo(provider: string): LlmProviderInfo {
    return { id: provider, name: 'Greeneek' }
  }
  ...
  override imageRequestPricing(_provider: string, model: string): ReturnType<LlmAdapter['imageRequestPricing']> {
    ...
    return greeneekImageRequestPricing(this.config.options(), model, resolveAccess)
  }
  ...
  private modelInfoFor(
    connection: GreeneekConnectionOptions,
    provider: string,
    model: string,
  ): LlmResolvedModelInfo {
  ...
  private async * streamWithConnection(
    ...
    if (hasImages) {
      ...
      if (model?.inputModalities?.includes('image') !== true) {
        throw new LlmError(
          `Greeneek model "${options.model}" does not accept image input.`,
          'UNSUPPORTED_CONTENT',
        )
      }
      ...
      if (attachments === undefined) {
        throw new LlmError(
          'Greeneek image conversion requires the durable attachment service.',
          'UNSUPPORTED_CONTENT',
        )
      }
    }
    ...
    try {
      ...
      if (timeoutOf(watchdog.signal, STREAM_IDLE_TIMEOUT_CODE) !== undefined) {
        throw new LlmError(
          `Greeneek stream idle timeout after ${connection.streamIdleTimeoutMs}ms`,
          'TIMEOUT',
          { cause: error },
        )
      }
      if (options.signal?.aborted) {
        throw new LlmError('Greeneek request aborted by caller', 'ABORTED', { cause: error })
      }
      if (error instanceof LlmError) throw error
      throw new LlmError(`Greeneek API stream from ${connection.baseURL} failed`, 'TRANSPORT', { cause: error })
    } finally {
      consumer.abort('Greeneek stream consumer stopped')
  ...
  private async * request(
    ...
    const headers = {
      ...
      ...attributionHeaders(),
      'x-greeneek-harness-user-id': String(userId),
      ...options.sessionId !== undefined
        ? { 'x-greeneek-harness-session-id': String(options.sessionId) }
        : {},
      ...options.purpose === 'compaction'
        ? { 'x-greeneek-harness-compact': '1' }
        : {},
    }
    ...
    while (true) {
          ...
          body = await serializeRequestWithImages(requestOptions, {
            representation: {
              ...
              resolveFileId: async (version, _block, location) => {
                ...
                let resolved: Awaited<ReturnType<GreeneekFileStore['ensureUploaded']>>
                try {
      ...
      let extensions: PreparedGreeneekLlmApiExtensions
      try {
        extensions = await this.config.prepareExtensions({
          body: body as unknown as Readonly<Record<string, GreeneekLlmApiJson>>,
          signal,
          ...options.sessionId === undefined ? {} : { sessionId: String(options.sessionId) },
          ...options.purpose === undefined ? {} : { purpose: options.purpose },
        })
      } catch (error) {
        throw new LlmError('Greeneek request extension preparation failed', 'REQUEST_EXTENSION', { cause: error })
      }
      for (const field of Object.keys(extensions.fields)) {
        if (Object.hasOwn(body, field)) {
          throw new LlmError(`Greeneek request extension field ${JSON.stringify(field)} collides with the base request`, 'REQUEST_EXTENSION')
        }
      }
      ...
      try {
        ...
        if (signal.aborted) throw error
        throw new LlmError(
          `Greeneek API request to ${connection.baseURL} failed`,
          'TRANSPORT',
          { cause: error },
        )
      }
      ...
      if (!response.ok) {
        let message = `Greeneek API error (HTTP ${response.status})`
        ...
        throw new LlmError(message, httpErrorCode(response.status, providerError), {
          cause: new Error(rawResponse.length > 0 ? rawResponse : `Greeneek HTTP ${response.status}`),
      ...
      try {
        ...
        throw new LlmError('Greeneek request extension acceptance failed', 'REQUEST_EXTENSION', { cause: error })
      }
      if (!response.body) {
        throw new LlmError('Greeneek API returned no response body', 'EMPTY_RESPONSE')
      }
