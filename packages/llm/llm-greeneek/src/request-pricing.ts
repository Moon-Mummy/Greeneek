/**
 * Provider-side request-image pricing for Greeneek routes: reproduces the
 ...
 *
 * @module gnk-llm-greeneek/request-pricing
 */

import { offloadedImageText, offloadedImagePrefixCount, requestImageHandleText, textOnlyImageText } from '@greeneek/gnk-llm'
import type { ImageAttachmentAccessResolver, LlmImageRequestPrice, LlmImageRequestPricing } from '@greeneek/gnk-llm'
import { requestImageDimensions } from '@greeneek/gnk-attachment'
import type { ImageAttachmentRef, ImageRequestPolicy } from '@greeneek/gnk-attachment'
import { greeneekImageTokens } from './image-tokens.ts'
...
/** Total-pixel budget matching Greeneek's normal vision projection. */
...
/**
 * Resolve the request-image budgets owned by one Greeneek model route.
 * @param model - Advertised model route and its optional image overrides.
 * @returns Complete pixel and encoded-byte budgets.
 * @internal
 */
export function resolveRequestImagePolicy(model: GreeneekCatalogModel): ImageRequestPolicy {
...
/**
 * Build the request-image pricing for one Greeneek route from a validated
 ...
 */
export function greeneekImageRequestPricing(
  ...
  return {
    priceImages: (images) => {
      ...
      return images.map((ref, index) => {
        ...
        return {
          visualTokens: greeneekImageTokens(dimensions.width, dimensions.height),
