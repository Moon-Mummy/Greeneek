/** Durable Greeneek attachment-to-file-id index. @module gnk-llm-greeneek/upload-index */
...
import { ImageVariantId } from '@greeneek/gnk-attachment'
import type { AttachmentId, ImageVariantId as ImageVariantIdType } from '@greeneek/gnk-attachment'
import { GreeneekFileId, GreeneekFileScope } from './file-id.ts'
import type { GreeneekFileId as GreeneekFileIdType, GreeneekFileScope as GreeneekFileScopeType } from './file-id.ts'

/** One durable remote upload mapping. Unix times are milliseconds. */
export interface GreeneekUploadRecord {
  scope: GreeneekFileScopeType
 ...
 */
export function greeneekFileScope(baseURL: string, apiKey: string): GreeneekFileScopeType {
  const digest = createHash('sha256')
    .update(baseURL.replace(/\/+$/u, ''))
    .update('\0')
    .update(apiKey)
    .digest('hex')
  return GreeneekFileScope(digest)
}
...
function parseRecord(value: unknown): GreeneekUploadRecord {
  if (value === null || typeof value !== 'object' || Array.isArray(value)) {
    throw new InvalidUploadIndexError('llm-greeneek: upload index contains a non-object record')
  }
  ...
  if (typeof record.scope !== 'string' || !/^[0-9a-f]{64}$/u.test(record.scope)
    ...
    throw new InvalidUploadIndexError('llm-greeneek: upload index contains an invalid record')
  }
  return {
    scope: GreeneekFileScope(record.scope),
    attachmentId: record.attachmentId as AttachmentId,
    variantId: ImageVariantId(record.variantId),
    fileId: GreeneekFileId(record.fileId),
...
function parseIndex(text: string): StoredIndex {
  ...
  try {
    ...
    throw new InvalidUploadIndexError('llm-greeneek: upload index is not valid JSON', { cause: error })
  }
  if (value === null || typeof value !== 'object' || Array.isArray(value)) {
    throw new InvalidUploadIndexError('llm-greeneek: upload index is not an object')
  }
  const index = value as { formatVersion?: unknown; records?: unknown }
  if (index.formatVersion !== 3 || !Array.isArray(index.records)) {
    throw new InvalidUploadIndexError('llm-greeneek: unsupported upload index format')
  }
  const records = index.records.map(parseRecord)
  const keys = new Set<string>()
  for (const record of records) {
    const key = `${record.scope}\0${record.variantId}`
    if (keys.has(key)) throw new InvalidUploadIndexError('llm-greeneek: upload index contains duplicate mappings')
    keys.add(key)
  }
  return { formatVersion: 3, records }
}

function reusable(record: GreeneekUploadRecord, now: number, refreshMarginMs: number): boolean {
  return record.expiresAt - now > refreshMarginMs
}

/** Atomic local index shared by every Greeneek session in this GNK home. */
export class GreeneekUploadIndex {
  /** Absolute owner-private JSON index path. */
  readonly path: string

  /**
   * @param path - explicit test path; omission uses `GNK_HOME/llm-greeneek/files-v3.json`.
   */
  constructor(path = join(resolveGnkHome(), 'llm-greeneek', 'files-v3.json')) {
    this.path = path
  }
  ...
  ): Promise<GreeneekUploadRecord | undefined> {
   ...
   */
  async commit(
    candidate: GreeneekUploadRecord,
    now: number,
    refreshMarginMs: number,
   ...
   * @param scope - endpoint/API-key namespace.
   */
  async clear(scope: GreeneekFileScopeType): Promise<void> {
    await mkdir(dirname(this.path), { recursive: true, mode: 0o700 })
    await withFileLock(this.path, async () => {
      const index = await this.load()
