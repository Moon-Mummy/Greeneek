...
import LlmRuntime, { createUserMessage, INVALID_CREDENTIAL_CODE } from '@greeneek/gnk-llm'
...
async function home(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'gnk-llm-dynamic-'))
 ...
 * llm-greeneek over one temp harness home. `watch: false` keeps every change
...
function prompt(ctx: Context) {
  return assemble(ctx, { model: 'greeneek-v4-flash', messages: [] })
}
...
describe('request-level dynamic configuration', () => {
  ...
  it('advertises a live settings catalog without re-registration', async () => {
    ...
    await expect(ctx.llm.listModels('greeneek-official')).resolves.toEqual([
      { provider: 'greeneek-official', id: 'settings-model', name: 'From Settings', inputModalities: ['text', 'image'] },
    ])
  })
  ...
  it('applies changed request file limits to the next request', async () => {
    ...
    await assemble(ctx, { model: 'greeneek-v4-flash-vision-exp', messages })
  ...
  it('re-registers the route in place when the captured retry policy changes, without an empty-registry window', async () => {
    ...
    expect(observed).toEqual([['greeneek-official']])
  })
  ...
  it('keeps the last good options when a settings snapshot fails beyond-schema validation', async () => {
    ...
    await expect(ctx.llm.listModels('greeneek-official')).resolves.toEqual([
      { provider: 'greeneek-official', id: 'recovered', name: 'recovered', inputModalities: ['text'] },
    ])
  })
  ...
  it('keeps the whole last-good snapshot when a rejected one changed the URL', async () => {
    ...
    vi.stubEnv('GREENEEK_API_KEY', 'good-key')
