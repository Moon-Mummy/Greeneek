...
import type { ImageAttachmentRef, ImageMediaType, RequestImageAttachment } from '@greeneek/gnk-attachment'
import { createUserMessage, ToolCallId, ReasoningEffortId, createMessage } from '@greeneek/gnk-llm'
import type { ContentBlock, GenerateOptions, Message } from '@greeneek/gnk-llm'
import {
  serializeMessages,
...
function request(overrides: Partial<GenerateOptions> = {}): GenerateOptions {
  return { provider: 'greeneek-official', model: 'greeneek-v4-flash', messages: [], ...overrides }
}
...
describe('serializeRequest', () => {
  ...
  it('rejects an effort outside the Greeneek capability', () => {
...
describe('image serialization', () => {
  ...
  it('includes provider-resolved normalized access in a retained image handle', async () => {
    ...
    version.width = 1130
    version.height = 565
    images.resolveImageAccess = () => ({ readonlyPath: '/tmp/gnk/objects/aa/object' })
    const wire = await serializeRequestWithImages(request({
      model: 'greeneek-v4-flash-vision-exp',
      messages: [createUserMessage({
        content: [{ type: 'image', attachment: ref }],
    ...
    expect(JSON.stringify(wire.messages[0])).toContain('/tmp/gnk/objects/aa/object')
  ...
  it('offloads oldest images before reads and keeps the newest image', async () => {
    ...
    images.resolveImageAccess = ref => ref.mediaType === 'image/png'
      ? { readonlyPath: '/tmp/gnk/objects/png' }
      : undefined
    const wire = await serializeRequestWithImages(request({
      model: 'greeneek-v4-flash-vision-exp',
      messages: [createUserMessage({
    ...
    expect(wire.messages[0]).toMatchObject({
      ...
      content: [
        {
          type: 'text',
          text: expect.stringContaining(`image omitted to fit request image limits; ${png.attachmentId}. Normalized copy (read-only; may be resized or re-encoded): "/tmp/gnk/objects/png"`) as string,
        },
