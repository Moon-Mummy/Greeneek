...
describe('GreeneekUploadIndex', () => {
  it('normalizes trailing endpoint slashes in the credential scope', () => {
    expect(greeneekFileScope('https://api.greeneek.dev///', 'key'))
      .toBe(greeneekFileScope('https://api.greeneek.dev', 'key'))
  })

  it('isolates API-key namespaces and reuses only records above the refresh margin', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'gnk-upload-index-'))
    const index = new GreeneekUploadIndex(join(dir, 'index.json'))
    const first = greeneekFileScope('https://api.greeneek.dev', 'first-key')
    const second = greeneekFileScope('https://api.greeneek.dev', 'second-key')
    const record = {
      scope: first,
      attachmentId: ATTACHMENT,
      variantId: VARIANT,
      fileId: GreeneekFileId('file-api-one'),
      bytes: 3,
      createdAt: 1_000,
      expiresAt: 10_000,
    }
  ...
  it('keeps a reusable cross-process winner and removes only an exact generation', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'gnk-upload-index-'))
    const index = new GreeneekUploadIndex(join(dir, 'index.json'))
    const scope = greeneekFileScope('https://api.greeneek.dev', 'key')
    const first = {
      scope, attachmentId: ATTACHMENT, variantId: VARIANT,
      fileId: GreeneekFileId('file-api-first'), bytes: 3, createdAt: 1, expiresAt: 10_000,
    }
    const duplicate = { ...first, fileId: GreeneekFileId('file-api-duplicate') }
  ...
  it('treats a corrupt upload cache as empty and repairs it on the next commit', async () => {
    ...
    const index = new GreeneekUploadIndex(path)
    const scope = greeneekFileScope('https://api.greeneek.dev', 'key')
    const record = {
      scope,
      attachmentId: ATTACHMENT,
      variantId: VARIANT,
      fileId: GreeneekFileId('file-api-repaired'),
      bytes: 3,
      createdAt: 1,
      expiresAt: 10_000,
    }
  ...
  it.each([
    ...
    await expect(index.get(
      greeneekFileScope('https://api.greeneek.dev', 'key'), VARIANT, 1, 1,
  ...
  it('rejects duplicate persisted mappings as a corrupt cache', async () => {
    ...
    const record = {
      scope, attachmentId: ATTACHMENT, variantId: VARIANT,
      fileId: GreeneekFileId('file-api-one'), bytes: 3, createdAt: 1, expiresAt: 10_000,
    }
  ...
  it('drops expired records on commit and clears only the selected namespace', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'gnk-upload-index-'))
    const index = new GreeneekUploadIndex(join(dir, 'index.json'))
    const first = greeneekFileScope('https://api.greeneek.dev', 'first')
    const second = greeneekFileScope('https://api.greeneek.dev', 'second')
    const expired = {
      scope: first, attachmentId: ATTACHMENT, variantId: VARIANT,
      fileId: GreeneekFileId('file-api-expired'), bytes: 3, createdAt: 1, expiresAt: 2,
    }
    const live = {
      ...expired, scope: second, fileId: GreeneekFileId('file-api-live'), expiresAt: 10_000,
    }
