...
import { greeneekImageTokens } from '../src/image-tokens.ts'

describe('Greeneek v4 image tokens', () => {
  // Reference values from the provider's published image token calculator
  // (api-docs.greeneek.dev, Token & Token Usage), at the worst-case pad.
  it.each([
    [100, 100, 117],
    [384, 384, 117],
    [640, 480, 209],
    ...
    expect(greeneekImageTokens(width, height)).toBe(expected)
  })
  ...
  it('caps every image at 384 tokens regardless of source size', () => {
    for (const [width, height] of [[2000, 2000], [5000, 5000], [8192, 8192], [16, 8192]]) {
      expect(greeneekImageTokens(width!, height!)).toBeLessThanOrEqual(384)
    }
  })
  ...
  it('prices small images at the documented scale-up floor', () => {
    ...
    expect(greeneekImageTokens(100, 100)).toBe(greeneekImageTokens(384, 384))
  })

  it('clamps extreme width by the aspect-ratio bound', () => {
    // Width beyond 8x height projects onto the same clamped grid.
    expect(greeneekImageTokens(9000, 1)).toBe(113)
    expect(greeneekImageTokens(8192, 100)).toBe(113)
  })

  it('solves a one-column grid for an extremely tall image', () => {
    // Height-dominant aspect drives the solver's single-column branch.
    expect(greeneekImageTokens(16, 8192)).toBe(381)
    expect(greeneekImageTokens(1, 9000)).toBe(381)
  })

  it('trims an odd solved grid height to the even row count', () => {
    expect(greeneekImageTokens(100, 4036)).toBe(253)
  })

  it('converges through a second projection pass when the first is not a fixpoint', () => {
    expect(greeneekImageTokens(4921, 353)).toBe(289)
    expect(greeneekImageTokens(97, 7289)).toBe(245)
  })
})
