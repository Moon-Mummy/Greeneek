...
import type { AttachmentStore, ImageAttachmentRef, RequestImageAttachment } from '@greeneek/gnk-attachment'
...
import type { PreparedGreeneekLlmApiExtensions } from '@greeneek/gnk-greeneek-llm-api-extensions'
import * as LlmGreeneek from '@greeneek/gnk-llm-greeneek'
import { GreeneekAdapter, resolveAdapterOptions } from '@greeneek/gnk-llm-greeneek'
...
beforeEach(() => {
  testHome = mkdtempSync(join(tmpdir(), 'gnk-llm-greeneek-'))
  vi.stubEnv('GNK_HOME', testHome)
})
...
async function harness(baseURL: string, config: object = {}) {
  ...
  vi.stubEnv('GREENEEK_API_KEY', 'test-key')
  const ctx = new Context()
  await ctx.plugin(LlmRuntime)
  await ctx.plugin(GreeneekLlmApiExtensionRegistry)
  await ctx.plugin(LlmGreeneek, { baseURL, ...config })
  return ctx
}

/** Direct adapter over the plugin's real resolve step, with a static key. */
function noExtensions(): Promise<PreparedGreeneekLlmApiExtensions> {
  return Promise.resolve({ fields: {}, accept: () => Promise.resolve() })
}

/** Direct adapter over the plugin's real resolve step, with a static key. */
function adapterOf(
  config: Partial<LlmGreeneek.Config> & { apiKey?: string } = {},
  attachments?: AttachmentStore,
  files?: LlmGreeneek.GreeneekFileStore,
): GreeneekAdapter {
  const { apiKey, ...rest } = config
  return new GreeneekAdapter({
...
function fileStoreOf(
  implementation: (...args: Parameters<LlmGreeneek.GreeneekFileStore['ensureUploaded']>) => ReturnType<LlmGreeneek.GreeneekFileStore['ensureUploaded']>,
) {
  const ensureUploaded = vi.fn(implementation)
  const invalidate = vi.fn(() => Promise.resolve())
  return {
    store: { ensureUploaded, invalidate } as unknown as LlmGreeneek.GreeneekFileStore,
    ensureUploaded,
    invalidate,
  }
}

function fileReference(fileId: string): Awaited<ReturnType<LlmGreeneek.GreeneekFileStore['ensureUploaded']>> {
  return {
    record: { fileId: LlmGreeneek.GreeneekFileId(fileId) },
    uploaded: true,
  } as Awaited<ReturnType<LlmGreeneek.GreeneekFileStore['ensureUploaded']>>
}
...
describe('request image policy', () => {
  ...
  it('answers image request pricing from the current connection snapshot', () => {
    ...
    const priced = adapter.imageRequestPricing('greeneek-official', 'vision')?.priceImages([imageRef])
    expect(priced).toHaveLength(1)
    expect(priced?.[0]!.visualTokens).toBeGreaterThan(0)
    const textOnly = adapter.imageRequestPricing('greeneek-official', 'unlisted')?.priceImages([imageRef])
...
describe('GreeneekAdapter against a mock server', () => {
  it('merges prepared extension fields and accepts them once after HTTP 2xx', async () => {
    ...
    await drain(adapter.stream({ provider: 'greeneek-official', model: 'm', messages: [], sessionId: SessionId('s') }))
    expect(server.requests[0]).toMatchObject({ gnk_test: { version: 1 } })
  ...
  it('fails before fetch on extension preparation or base-field collision', async () => {
    ...
    const failed = new GreeneekAdapter({
      ...base,
      prepareExtensions: () => Promise.reject(new Error('metadata unavailable')),
    })
    await expect(drain(failed.stream({ provider: 'greeneek-official', model: 'm', messages: [] })))
      .rejects.toMatchObject({ code: 'REQUEST_EXTENSION' })

    const collision = new GreeneekAdapter({
      ...base,
      prepareExtensions: (() => Promise.resolve({ fields: { model: 'replacement' }, accept: () => Promise.resolve() })) as never,
    })
    await expect(drain(collision.stream({ provider: 'greeneek-official', model: 'm', messages: [] })))
      .rejects.toMatchObject({ code: 'REQUEST_EXTENSION' })
  ...
  it('does not accept extensions on non-2xx and does accept before a later stream failure', async () => {
    ...
    const adapter = new GreeneekAdapter({
      ...
      resolveApiKey: () => Promise.resolve('k'),
      resolveUserId: () => TEST_USER_ID,
      prepareExtensions: () => Promise.resolve({ fields: { gnk_test: 1 }, accept: async () => { accept() } }) as never,
    })
  ...
  it('reports a post-2xx extension acceptance failure without relabelling it as transport', async () => {
    ...
    await expect(drain(adapter.stream({ provider: 'greeneek-official', model: 'm', messages: [] })))
  ...
  it('streams a text generation end to end through the assembler', async () => {
    ...
    // App attribution and Greeneek request identity are independent wire facts.
    expect(server.headers[0]?.['user-agent']).toBe(userAgent())
    expect(server.headers[0]?.['x-greeneek-harness-user-id']).toBe(getOrCreateAnonymousUserId())
    expect(server.headers[0]).not.toHaveProperty('x-greeneek-harness-session-id')
    ...
    expect(server.headers[0]).not.toHaveProperty('x-greeneek-harness-compact')
  })
  ...
  it('uploads a durable image once and sends only its Files API id to the vision model', async () => {
    ...
    await drain(adapter.stream({
      provider: 'greeneek-official',
      model: 'greeneek-v4-flash-vision-exp',
      messages: [createUserMessage({
        content: [
    ...
    expect(server.fileRequests).toEqual([{
      method: 'POST',
      path: '/files',
      filename: `gnk-${'a'.repeat(16)}-${'b'.repeat(8)}.png`,
      bytes: 3,
    }])
  ...
  it('falls back to one all-base64 request when Files API resolution fails', async () => {
    ...
    const adapter = adapterOf({
      baseURL: server.url,
      models: [{ id: 'greeneek-v4-flash-vision-exp', inputModalities: ['text', 'image'] }],
  ...
  it('falls back after the configured Files API deadline without aborting chat', async () => {
    ...
    const adapter = adapterOf({
      baseURL: 'https://greeneek.invalid',
  ...
  it('projects nested tool-result images with route-owned request budgets', async () => {
    ...
    await drain(adapter.stream({ provider: 'greeneek-official', model: 'vision-low', messages: [nested] }))
    await drain(adapter.stream({ provider: 'greeneek-official', model: 'vision-custom', messages: [nested] }))
  ...
  it('reuses the exact request version between agent and compaction calls', async () => {
    ...
    await drain(adapter.stream({ provider: 'greeneek-official', model: 'greeneek-v4-flash-vision-exp', messages }))
    await drain(adapter.stream({
      provider: 'greeneek-official',
      model: 'greeneek-v4-flash-vision-exp',
      messages,
    ...
    expect(server.headers[1]?.['x-greeneek-harness-compact']).toBe('1')
  })
  ...
  it.each(['greeneek-v4-flash', 'unlisted-pass-through'])(
  ...
  it('forwards the harness user and session ids for host-side trajectory routing', async () => {
    ...
    expect(server.headers[0]?.['x-greeneek-harness-session-id']).toBe('child-session')
    expect(server.headers[0]?.['x-greeneek-harness-user-id']).toBe(getOrCreateAnonymousUserId())
  })
  ...
  it('marks the auxiliary compaction call on the wire', async () => {
    ...
    expect(server.headers[0]?.['x-greeneek-harness-compact']).toBe('1')
  })
  ...
  it('uses the configured maxTokens default and preserves an explicit request cap', async () => {
    ...
    await assemble(ctx, { model: 'greeneek-v4-flash', messages: [], maxTokens: 8_192 })
  ...
  it('publishes only off and omits the wire effort when thinking is disabled', async () => {
    ...
    await expect(ctx.llm.resolveModelInfo('greeneek-official', 'greeneek-v4-flash'))
  ...
  it.each([
    ...
    const ctx = await harness(server.url)
    const result = await assemble(ctx,{ model: 'greeneek-v4-flash', messages: [] })
  ...
  it('uses the HTTP status as the cause when an error response has no body', async () => {
    ...
    await expect(drain(adapter.stream({
      ...
      code: 'SERVER',
      cause: { message: 'Greeneek HTTP 500' },
    })
  })
  ...
  it('parses a future Retry-After HTTP date and the Greeneek request-id fallback', async () => {
    ...
    try {
      const server = await mockServer([{
        ...
        headers: {
          ...
          'x-greeneek-request-id': 'greeneek-503',
        },
      }])
      const ctx = await harness(server.url)
      const result = await assemble(ctx, { model: 'greeneek-v4-flash', messages: [] })
      expect(result.finish).toEqual({
        ...
        failure: {
          ...
          requestId: ProviderRequestId('greeneek-503'),
        },
      })
    } finally {
      dateNow.mockRestore()
  ...
  it('reports a transport failure with the endpoint in the message', async () => {
    ...
    expect(result.finish).toMatchObject({
      ...
      failure: {
        code: 'TRANSPORT',
        message: 'Greeneek API request to http://127.0.0.1:1 failed',
      },
    })
  })
  ...
  it('throws EMPTY_RESPONSE when the response has no body', async () => {
    ...
    try {
      const iterate = async (): Promise<void> => {
        for await (const _chunk of adapter.stream({ provider: 'greeneek-official', model: 'm', messages: [] })) { /* drain */ }
      }
  ...
  it('classifies an abrupt body close as TRANSPORT', async () => {
    ...
    expect(result.finish.failure.message).toMatch(/^Greeneek API stream from .* failed$/)
  })
  ...
  it('renders a non-Error transport rejection without losing its cause', async () => {
    ...
    try {
      ...
      await expect(drain()).rejects.toMatchObject({
        message: 'Greeneek API request to https://example.invalid failed',
        code: 'TRANSPORT',
        cause: 'offline',
      })
    } finally {
  ...
  it('keeps an idle provider read alive through SSE comments', async () => {
    ...
    try {
      const chunks: string[] = []
      const drain = (async () => {
        for await (const chunk of adapter.stream({ provider: 'greeneek-official', model: 'm', messages: [] })) {
...
describe('plugin registration and config', () => {
  it('keeps wire helpers off the package root', () => {
    for (const helper of [
      ...
      'mapUsage',
      'translate',
    ]) expect(LlmGreeneek).not.toHaveProperty(helper)
  })

  it('registers the greeneek provider and unregisters on dispose (HMR safety)', async () => {
    const server = await mockServer([])
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    const fiber = await ctx.plugin(LlmGreeneek, {
      baseURL: server.url,
    })
    expect(ctx.llm.listProviders()).toEqual([{ id: 'greeneek-official', name: 'Greeneek' }])
  ...
  it('registers retryPolicy from the provider config', async () => {
    ...
    expect(ctx.llm.providerRetryPolicy('greeneek-official')).toEqual({
      mode: 'always',
      initialDelayMs: 25,
      maxDelayMs: 100,
      jitterRatio: 0.2,
  ...
  it('owns the greeneek provider and advertises the default models', async () => {
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    await ctx.plugin(LlmGreeneek, { baseURL: 'http://127.0.0.1:1' })
    expect(ctx.llm.listProviders()).toEqual([{ id: 'greeneek-official', name: 'Greeneek' }])
    await expect(ctx.llm.listModels('greeneek-official')).resolves.toEqual([
      {
        provider: 'greeneek-official',
      ...
      { provider: 'greeneek-official', id: 'greeneek-v4-flash-vision-exp', name: 'Greeneek-V4-Flash-Vision-Exp', inputModalities: ['text', 'image'] },
    ])
    await expect(ctx.llm.resolveModelInfo('greeneek-official', 'greeneek-v4-flash'))
      .resolves.toMatchObject({
        provider: 'greeneek-official',
    ...
    await expect(ctx.llm.resolveModelInfo('greeneek-official', 'greeneek-v4-flash-vision-exp'))
      .resolves.toMatchObject({
        provider: 'greeneek-official',
  ...
  it.each(['off', 'low', 'max'] as const)('uses the configured %s reasoning default', async (effort) => {
    ...
    await expect(ctx.llm.resolveModelInfo('greeneek-official', 'unlisted-pass-through'))
      .resolves.toMatchObject({
        reasoning: {
          efforts: [
            { id: ReasoningEffortId('off'), name: 'Off', description: 'Use for simple tasks that do not need reasoning.' },
  ...
  it.each(['low', 'high', 'max'] as const)(
    ...
    async (reasoningEffort) => {
      const ctx = new Context()
      await ctx.plugin(LlmRuntime)
      await expect(ctx.plugin(LlmGreeneek, {
        baseURL: 'http://127.0.0.1:1',
        thinking: 'disabled',
        reasoningEffort,
  ...
  it('accepts disabled thinking with off at the resolver boundary', async () => {
    ...
    await expect(adapter.resolveModel('greeneek-official', 'pass-through')).resolves.toMatchObject({
      reasoning: {
        efforts: [{
          id: ReasoningEffortId('off'),
          name: 'Off',
  ...
  it('uses the default model catalog when apply is called directly', async () => {
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    LlmGreeneek.apply(ctx, { baseURL: 'http://127.0.0.1:1' })
    await expect(ctx.llm.listModels('greeneek-official')).resolves.toEqual([
      {
        provider: 'greeneek-official',
        id: 'greeneek-v4-flash',
  ...
  it('defaults an adapter-supplied catalog entry to text input', async () => {
    ...
    await expect(adapter.listModels('greeneek-official')).resolves.toEqual([{
      provider: 'greeneek-official',
      id: 'adapter-model',
      name: 'adapter-model',
      inputModalities: ['text'],
  ...
  it('advertises configured models without restricting arbitrary request ids', async () => {
    ...
    await expect(ctx.llm.listModels('greeneek-official')).resolves.toEqual([
      { provider: 'greeneek-official', id: 'private-fast', name: 'private-fast', inputModalities: ['text'] },
      { provider: 'greeneek-official', id: 'private-reasoner', name: 'Private Reasoner', description: 'Higher reasoning budget', inputModalities: ['text', 'image'] },
    ])
    await expect(ctx.llm.resolveModelInfo('greeneek-official', 'private-fast'))
      .resolves.toMatchObject({ context: { contextWindow: 32_000 } })
    await expect(ctx.llm.resolveModelInfo('greeneek-official', 'private-reasoner'))
      .resolves.toMatchObject({
        name: 'Private Reasoner',
        description: 'Higher reasoning budget',
        inputModalities: ['text', 'image'],
      })
    await expect(ctx.llm.resolveModelInfo('greeneek-official', 'arbitrary-unlisted'))
      .resolves.toMatchObject({
        context: { contextWindow: 1_000_000 },
        defaultMaxTokens: 256_000,
      })
  ...
  it('uses exact model capacity before the adapter-wide default', async () => {
    ...
    await expect(ctx.llm.resolveModelInfo('greeneek-official', 'inherits-default'))
      .resolves.toMatchObject({ context: { contextWindow: 256_000 } })
    await expect(ctx.llm.resolveModelInfo('greeneek-official', 'exact-override'))
  ...
  const invalidModels: Array<[LlmGreeneek.GreeneekCatalogModel[], RegExp]> = [
    ...
    [[{
      id: 'm',
      inputModalities: ['audio'] as unknown as NonNullable<LlmGreeneek.GreeneekCatalogModel['inputModalities']>,
    }], /expected "text" \| "image"/],
  ...
  const invalidProgrammaticModalities: Array<[LlmGreeneek.GreeneekCatalogModel[], RegExp]> = [
  ...
  it('rejects the removed imageDetail model setting through schema and direct construction', async () => {
    const legacyModel = { id: 'vision', inputModalities: ['image'], imageDetail: 'low' } as unknown as
      LlmGreeneek.GreeneekCatalogModel
  ...
  it('prefers a model\'s own output cap over the profile default', async () => {
    ...
    await expect(adapter.resolveModel('greeneek-official', 'capped'))
      .resolves.toMatchObject({ defaultMaxTokens: 512 })
    await expect(adapter.resolveModel('greeneek-official', 'uncapped'))
      .resolves.toMatchObject({ defaultMaxTokens: 4096 })
    await expect(adapter.resolveModel('greeneek-official', 'not-in-catalog'))
      .resolves.toMatchObject({ defaultMaxTokens: 4096 })
  })
  ...
  it('falls back to GREENEEK_API_KEY and GREENEEK_BASE_URL env vars', async () => {
    vi.stubEnv('GREENEEK_API_KEY', 'env-key')
    vi.stubEnv('GREENEEK_BASE_URL', 'http://127.0.0.1:1')
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    await ctx.plugin(LlmGreeneek, {})
    expect(ctx.llm.listProviders()).toEqual([{ id: 'greeneek-official', name: 'Greeneek' }])
  ...
  it('loads keyless, keeps the catalog browsable, and fails the request actionably', async () => {
    ...
    expect(ctx.llm.listProviders()).toEqual([{ id: 'greeneek-official', name: 'Greeneek' }])
    await expect(ctx.llm.listModels('greeneek-official')).resolves.toHaveLength(3)
    const first = await assemble(ctx, { model: 'greeneek-v4-flash', messages: [] })
    expect(first.finish).toMatchObject({ kind: 'error', failure: { code: 'MISSING_CREDENTIAL' } })
    // The guidance leads with the managed credential store.
    const second = await assemble(ctx, { model: 'greeneek-v4-flash', messages: [] })
    ...
    expect(second.finish.failure.message)
      .toMatch(/store GREENEEK_API_KEY through the credentials service.*export GREENEEK_API_KEY/s)
  })
  ...
  it('reads the ambient variable when no credentials seam is mounted', async () => {
    ...
    vi.stubEnv('GREENEEK_API_KEY', 'ambient-key')
    const server = await mockServer([{ kind: 'sse', events: textEvents }])
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    await ctx.plugin(LlmGreeneek, { baseURL: server.url })
    await assemble(ctx, { model: 'greeneek-v4-flash', messages: [] })
  ...
  it('prefers explicit config over env for key and base URL', async () => {
    vi.stubEnv('GREENEEK_API_KEY', 'env-key')
    vi.stubEnv('GREENEEK_BASE_URL', 'http://env-host:1')
    const server = await mockServer([{ kind: 'sse', events: textEvents }])
  ...
  it('uses GREENEEK_BASE_URL when config omits baseURL', async () => {
    const server = await mockServer([{ kind: 'sse', events: textEvents }])
    vi.stubEnv('GREENEEK_BASE_URL', server.url)
    vi.stubEnv('GREENEEK_API_KEY', 'test-key')
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    await ctx.plugin(LlmGreeneek, {})
  ...
  it('takes GREENEEK_BASE_URL from any environment layer, with explicit config still on top', () => {
    const trusted = createLaunchEnvironmentSnapshot([
      { source: 'user-env', path: '/home/.gnk/.env', values: { GREENEEK_BASE_URL: 'https://user.example' } },
    ])
    ...
    const project = createLaunchEnvironmentSnapshot([
      { source: 'project-env', path: '/work/.env', values: { GREENEEK_BASE_URL: 'https://project.example' } },
    ])
    ...
    const shell = createLaunchEnvironmentSnapshot([
      { source: 'process', values: { GREENEEK_BASE_URL: 'https://stale.example' } },
    ])
  ...
  it('defaults to the public base URL without config or env', async () => {
    vi.stubEnv('GREENEEK_API_KEY', 'k')
    vi.stubEnv('GREENEEK_BASE_URL', undefined)
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    // Registration succeeds; no call is made (would hit api.greeneek.dev).
    await ctx.plugin(LlmGreeneek, {})
    expect(ctx.llm.listProviders()).toEqual([{ id: 'greeneek-official', name: 'Greeneek' }])
  })
  ...
  it('adapter is constructible directly for embedding over the shared resolver', async () => {
    const adapter = adapterOf()
    expect(adapter).toBeInstanceOf(GreeneekAdapter)
    ...
    await expect(adapter.listModels('greeneek-official')).resolves.toHaveLength(3)
  })
  ...
  it('resolves connection facts and the credential exactly once per stream call', async () => {
    ...
    const adapter = new GreeneekAdapter({ options, resolveApiKey, resolveUserId, prepareExtensions: noExtensions })
