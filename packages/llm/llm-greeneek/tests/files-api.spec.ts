...
import { userAgent } from '@greeneek/gnk-llm'
...
describe('GreeneekFilesClient', () => {
  it('uploads multipart bytes with the required purpose and explicit expiry', async () => {
    ...
    const client = new GreeneekFilesClient({ baseURL: 'https://api.greeneek.dev/', apiKey: 'key', fetch: fetchImpl })
    ...
    await expect(client.upload({
      ...
      id: GreeneekFileId('file-api-one'),
      bytes: 3,
  ...
  it('validates list, retrieve, and delete responses', async () => {
    ...
    await expect(client.list({ after: GreeneekFileId('file-api-before'), limit: 20, order: 'desc' })).resolves.toMatchObject({
      data: [{ id: 'file-api-one' }], firstId: 'file-api-one', lastId: 'file-api-one', hasMore: false,
    })
    await expect(client.retrieve(GreeneekFileId('file-api-one'))).resolves.toMatchObject({ id: 'file-api-one' })
    await expect(client.delete(GreeneekFileId('file-api-one'))).resolves.toBeUndefined()
  })
  ...
  it.each([
    ...
    const client = new GreeneekFilesClient({
      baseURL: 'https://api.greeneek.dev',
      apiKey: 'key',
      fetch: vi.fn(() => Promise.resolve(new Response('not-json', { status }))),
    })
    await expect(client.retrieve(GreeneekFileId('missing'))).rejects.toMatchObject({
      name: 'GreeneekFilesError',
      code,
      detail: '',
    })
  })
  ...
  it.each([
    ...
    const error = await client.retrieve(GreeneekFileId('missing')).catch((caught: unknown) => caught)
    expect(error).toBeInstanceOf(GreeneekFilesError)
    expect(error).toMatchObject({ message: 'Greeneek Files API error (HTTP 400)', detail: '' })
  })
  ...
  it('wraps transport failures but preserves an aborted request reason', async () => {
    ...
    await expect(client.retrieve(GreeneekFileId('one'))).rejects.toMatchObject({
      code: 'TRANSPORT',
      cause: transport,
    })
    ...
    await expect(client.retrieve(GreeneekFileId('one'), controller.signal)).rejects.toBe(transport)
  })
  ...
  it.each([
    ...
    await expect(client.retrieve(GreeneekFileId('one'))).rejects.toMatchObject({ code: 'INVALID_RESPONSE' })
  })
  ...
  it('accepts a list without cursors and uses the global fetch default', async () => {
    ...
    try {
      const client = new GreeneekFilesClient({ baseURL: 'https://api.greeneek.dev///', apiKey: 'key' })
  ...
  it.each([
    ...
    await expect(client.delete(GreeneekFileId('file-api-one'))).rejects.toMatchObject({ code: 'INVALID_RESPONSE' })
  })
})
