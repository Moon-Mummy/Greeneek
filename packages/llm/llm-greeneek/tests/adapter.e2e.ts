...
import LlmRuntime, { createUserMessage, ToolCallId, ReasoningEffortId, createMessage } from '@greeneek/gnk-llm'
import type { Message, ToolSchema } from '@greeneek/gnk-llm'
import AttachmentStore, { AttachmentId, ImageVariantId } from '@greeneek/gnk-attachment'
import type {
...
import * as PluginPackageInventoryGreeneek from '@greeneek/gnk-plugin-package-inventory-greeneek'
import * as SessionLogGreeneek from '@greeneek/gnk-session-log-greeneek'
import * as LlmGreeneek from '@greeneek/gnk-llm-greeneek'
import type { Config } from '@greeneek/gnk-llm-greeneek'
 ...
 * entirely without $GREENEEK_API_KEY; the pre-release vision smoke additionally
 * requires $GREENEEK_VISION_E2E=1 (see vitest.e2e.config.ts).
 */

const FLASH = 'greeneek-v4-flash'
const VISION = 'greeneek-v4-flash-vision-exp'
const VISION_E2E_ENABLED = process.env.GREENEEK_VISION_E2E === '1'
...
beforeEach(async () => {
  identityHome = await mkdtemp(join(tmpdir(), 'gnk-e2e-user-id-'))
  vi.stubEnv('GNK_HOME', identityHome)
})
...
async function harness(_model: string, config: Partial<Config> = {}) {
  ...
  await ctx.plugin(LlmGreeneek, config)
  return ctx
}
...
describe.skipIf(!process.env.GREENEEK_API_KEY)('llm-greeneek e2e (real API)', () => {
  it.skipIf(!VISION_E2E_ENABLED)('uses the built-in official route to upload, reference, and delete one image', async () => {
    const key = process.env.GREENEEK_API_KEY
    if (key === undefined) throw new Error('e2e ran without GREENEEK_API_KEY')
    const baseURL = process.env.GREENEEK_BASE_URL ?? LlmGreeneek.PUBLIC_BASE_URL
    const ctx = await harness(VISION, { baseURL })
    await ctx.plugin(E2eAttachmentStore)
    const attachments = ctx.attachments as E2eAttachmentStore
    let uploadedFile: LlmGreeneek.GreeneekFileIdType | undefined
    ...
    const observedFetch: typeof fetch = async (input, init) => {
      ...
      if (method === 'POST' && url.pathname.endsWith('/files') && response.ok) {
        ...
        if (typeof value.id === 'string') uploadedFile = LlmGreeneek.GreeneekFileId(value.id)
      }
      return response
    }
    vi.stubGlobal('fetch', observedFetch)
    const files = new LlmGreeneek.GreeneekFilesClient({ baseURL, apiKey: key })
    ...
    try {
      ...
      expect(
        result.finish.kind,
        `Greeneek vision result: ${JSON.stringify(result.finish)}`,
      ).toBe('stop')
  ...
  it('accepts the session-log and plugin-package request extension fields', async () => {
    ...
    await ctx.plugin(SessionLogGreeneek, { enabled: true })
    await ctx.plugin(PluginPackageInventoryGreeneek)
  ...
  it(
    ...
    async () => {
      ...
      expect(
        first.finish.kind,
        `Greeneek Flash tool-call turn finished as ${JSON.stringify(first.finish)}`,
      ).toBe('tool-calls')
      ...
      expect(
        second.finish.kind,
        `Greeneek Flash tool-result turn finished as ${JSON.stringify(second.finish)}`,
      ).toBe('stop')
