...
import { offloadedImageText, requestImageHandleText, textOnlyImageText } from '@greeneek/gnk-llm'
import { AttachmentId } from '@greeneek/gnk-attachment'
import type { ImageAttachmentRef } from '@greeneek/gnk-attachment'
import { greeneekImageRequestPricing } from '../src/request-pricing.ts'
...
describe('Greeneek request-image pricing', () => {
  it('prices an uncatalogued model as its text-only substitution', () => {
    const image = ref('photo', 1920, 1080)
    const prices = greeneekImageRequestPricing(connection(), 'unlisted').priceImages([image])
  ...
  it('prices a catalogued text-only model as its text-only substitution', () => {
    ...
    const prices = greeneekImageRequestPricing(options, 'text-only').priceImages([image])
    expect(prices).toEqual([{ visualTokens: 0, text: textOnlyImageText(image) }])
  })

  it('prices a retained image by its projected request dimensions plus its handle text', () => {
    const image = ref('photo', 1920, 1080)
    const prices = greeneekImageRequestPricing(connection(), 'vision').priceImages([image])
  ...
  it('honors the low-detail pixel budget preset', () => {
    ...
    const prices = greeneekImageRequestPricing(options, 'vision').priceImages([image])
  ...
  it('builds handle and placeholder text through the supplied access resolution', () => {
    ...
    const prices = greeneekImageRequestPricing(
