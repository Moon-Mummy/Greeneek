...
const NAME = 'gnk-test-bin'

const tmp = (): string => mkdtempSync(join(tmpdir(), 'gnk-app-boot-'))
...
describe('loadEnv', () => {
  it('loads variables from .env in the given dir', () => {
    const dir = tmp()
    writeFileSync(join(dir, '.env'), 'GNK_APP_BOOT_SPEC_VAR=loaded\n')
    const warn = vi.fn()
    loadEnv(NAME, dir, warn)
    expect(process.env['GNK_APP_BOOT_SPEC_VAR']).toBe('loaded')
    expect(warn).not.toHaveBeenCalled()
    delete process.env['GNK_APP_BOOT_SPEC_VAR']
  })
  ...
  it('defaults dir to the process cwd and warn to a stderr write', () => {
    const dir = tmp()
    writeFileSync(join(dir, '.env'), 'GNK_APP_BOOT_SPEC_DEFAULTS=yes\n')
    ...
    expect(process.env['GNK_APP_BOOT_SPEC_DEFAULTS']).toBe('yes')
    delete process.env['GNK_APP_BOOT_SPEC_DEFAULTS']
...
describe('loadLayeredEnv', () => {
  ...
  it.each([
    ['a harness switch', 'GNK_PERMISSION_MODE=danger-full-access\n'],
    ['the executable search path', 'PATH=/tmp/evil\n'],
    ['a module preload', 'NODE_OPTIONS=--require /tmp/evil.js\n'],
    ['a skill root', 'GNK_AGENTS_HOME=/tmp/injected\n'],
...
describe('boot', () => {
  ...
  it('can resolve bare plugins from the harness when the config project shadows their package name', async () => {
    ...
    const shadow = join(dir, 'node_modules', '@greeneek', 'gnk-system-prompt')
    const harnessPlugin = join(harness, 'node_modules', '@greeneek', 'gnk-system-prompt')
  ...
  it('exposes gnkHomePath to Loader config expressions', async () => {
    const dir = tmp()
    const gnkHome = join(dir, 'home')
    vi.stubEnv('GNK_HOME', gnkHome)
    ...
    writeFileSync(join(dir, 'cordis.yml'), [
      ...
      '  name: ./capture.mjs',
      '  config:',
      "    path: !!js gnkHomePath('sessions')",
      '',
    ].join('\n'))
    let ctx: Context | undefined
    try {
      ctx = await boot(NAME, join(dir, 'cordis.yml'))
      expect(ctx.get('capturedPath')).toBe(join(gnkHome, 'sessions'))
    } finally {
      await ctx?.fiber.dispose()
      vi.unstubAllEnvs()
    }
...
describe('addHarnessSourceSection', () => {
  ...
  const EXPECTED = `The Greeneek implementation checkout is at ${SOURCE_ROOT}. The checkout location and current working directory are separate values and may differ; never infer the working directory from this path. Use pwd to determine the current working directory. Use this checkout only to inspect or extend Greeneek itself.`
  ...
  it('distinguishes the source path from the current workdir between identity and persona', async () => {
    ...
    try {
      ...
      // through `-1 < n`.
      const identityAt = rendered.indexOf('You are an AI agent powered by Greeneek.')
