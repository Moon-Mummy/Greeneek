...
export function registerWebTools(registry: { register(t: HarnessTool): void }): void {
  ...
  registry.register({
    definition: {
      name: "web.search",
      description: "Search the web. Provider is chosen by WEB_SEARCH_PROVIDER (mock, exa, perplexity, deepseek).",
    ...
    async execute(args, ctx) {
      ...
      if (provider === "mock") {
        return [
          ...
          "- The DeepSeek-Harness-derived MIT base — LICENSE",
          "- 13 new capabilities in this release. Set WEB_SEARCH_PROVIDER=exa, perplexity, or deepseek with the matching API key for live results.",
        ].join("\n");
      }
