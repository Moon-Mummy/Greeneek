 ...
 * @module @greeneek/gnk-subprocess
 */

import { Context, Service } from '@greeneek/cordis'
import { GNK_ENV_PREFIX, LEGACY_ENV_PREFIX } from './types.ts'
import type { SubprocessHandle, SubprocessSpawnSpec } from './types.ts'
import type { SubprocessTerminalHandle, SubprocessTerminalSpawnSpec } from './types.ts'

export { GNK_ENV_PREFIX, LEGACY_ENV_PREFIX } from './types.ts'
export type {
  CollectedOutput,
  GnkEnvironment,
  GnkEnvironmentKey,
 ...
 * harness's own `GREENEEK_API_KEY`/secrets must not leak into a spawned
 ...
 * `GNK_*` names — the canonical base every harness child starts from. `PATH`,
 ...
 * credential or current `GNK_*` fact goes through the spec's explicit `env`,
 * which merges after this scrub). Both scrubs match case-insensitively:
 * Windows environment names are case-insensitive, so a parent `gnk_*` entry
 * would otherwise survive and read back as `$env:GNK_*` in the child;
 * deliberate lowercase `gnk_*` names on POSIX are implausible. Exported as a plain function so spawners
...
export function scrubbedParentEnv(): Record<string, string> {
  ...
  for (const [key, value] of Object.entries(process.env)) {
    if (value !== undefined && !SENSITIVE_ENV_PATTERN.test(key)
      && !key.toUpperCase().startsWith(GNK_ENV_PREFIX) && !key.toUpperCase().startsWith(LEGACY_ENV_PREFIX)) env[key] = value
  }
  return env
}
