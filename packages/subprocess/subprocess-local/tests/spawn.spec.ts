...
import type { SubprocessHandle, SubprocessOutputReader } from '@greeneek/gnk-subprocess'
...
function shellArgv(command: string): string[] {
  ...
  switch (command) {
    ...
    case 'echo "[${GNK_STALE:-absent}|$GNK_SHELL|$GNK_SESSION_ID]"':
      return node('console.log("[" + [process.env.GNK_STALE ?? "absent", process.env.GNK_SHELL, process.env.GNK_SESSION_ID].join("|") + "]")')
    case 'echo "[${GNK_TEST_API_KEY:-absent}|${GNK_TEST_TOKEN:-absent}|${SUBPROCESS_TEST_PASSWORD:-absent}|${GNK_TEST_PLAIN:-absent}]"':
      return node('console.log("[" + [process.env.GNK_TEST_API_KEY ?? "absent", process.env.GNK_TEST_TOKEN ?? "absent", process.env.SUBPROCESS_TEST_PASSWORD ?? "absent", process.env.GNK_TEST_PLAIN ?? "absent"].join("|") + "]")')
...
const spillDir = mkdtempSync(join(tmpdir(), 'gnk-subprocess-spec-'))
...
describe('spawnSubprocess', () => {
  ...
  it('rejects with a spawn error for a nonexistent cwd', async () => {
    await expect(spawnSubprocess(spec('echo hi', { cwd: '/nonexistent-dir-gnk-test' })).done)
...
describe.skipIf(process.platform === 'win32')('tree-survivor escalation (terminate and bounded waits reach helpers the leader left behind)', () => {
  ...
  it('service teardown awaits tree survivors, not just handle settlement', async () => {
    const { Context } = await import('@greeneek/cordis')
    const { default: LocalSubprocessRuntime } = await import('@greeneek/gnk-subprocess-local')
...
describe('coverage seams', () => {
  ...
  it('childEnv keeps the POSIX spread on non-Windows hosts', () => {
    ...
    try {
      expect(childEnv({ GNK_X: '1' }).GNK_X).toBe('1')
    } finally {
  ...
  it('a spawn-failed handle rejects done while waitForExit reports gone', async () => {
    const running = spawnSubprocess(spec('true', { cwd: '/nonexistent-dir-gnk-dispose-test' }))
  ...
  it('waitForExit on a failed spawn reports exited immediately', async () => {
    const running = spawnSubprocess(spec('true', { cwd: '/nonexistent-dir-gnk-spawn-test' }))
    await expect(running.done).rejects.toThrow()
    await expect(running.waitForExit()).resolves.toBe(true)
  })
...
describe('environment and spill-file hardening', () => {
  it('scrubs credential-shaped and ambient GNK env vars from child processes', async () => {
    process.env.GNK_TEST_API_KEY = 'super-secret'
    process.env.GNK_TEST_TOKEN = 'also-secret'
    process.env.SUBPROCESS_TEST_PASSWORD = 'password-secret'
    process.env.GNK_TEST_PLAIN = 'visible'
    try {
      const result = await finish(spawnSubprocess(spec(
        'echo "[${GNK_TEST_API_KEY:-absent}|${GNK_TEST_TOKEN:-absent}|${SUBPROCESS_TEST_PASSWORD:-absent}|${GNK_TEST_PLAIN:-absent}]"',
      )))
      expect(result.stdout.text.trim()).toBe('[absent|absent|absent|absent]')
    } finally {
      delete process.env.GNK_TEST_API_KEY
      delete process.env.GNK_TEST_TOKEN
      delete process.env.SUBPROCESS_TEST_PASSWORD
      delete process.env.GNK_TEST_PLAIN
    }
  })

  it('forwards explicit GNK_* env entries while scrubbing ambient ones', async () => {
    // Both facts through one explicit map: the ambient GNK_STALE is dropped by
    // the scrub, and the deliberately supplied current values merge after it.
    process.env.GNK_STALE = 'old-value'
    try {
      const result = await finish(spawnSubprocess(spec('echo "[${GNK_STALE:-absent}|$GNK_SHELL|$GNK_SESSION_ID]"', {
        env: { GNK_SHELL: '1', GNK_SESSION_ID: 'current-session' },
      })))
      expect(result.stdout.text.trim()).toBe('[absent|1|current-session]')
    } finally {
      delete process.env.GNK_STALE
    }
  })
  ...
  it.skipIf(process.platform === 'win32')('creates spill files with owner-only permissions and random names', async () => {
    ...
    expect(path).toMatch(/gnk-subprocess-\d+-\d+-[0-9a-f]{12}-stdout\.log$/)
  ...
  it.skipIf(process.platform === 'win32')('defaults spills into a private per-process directory', async () => {
    ...
    expect(dir).toMatch(/gnk-subprocess-/)
