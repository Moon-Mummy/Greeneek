 ...
 * model-facing render consumes ({@link module:@greeneek/gnk-tool-fs-search/search-core}
 ...
 * final output budget (`gnk-spill-policy`) only shrinks `content`, never `meta`,
 * so this projection owns keeping `meta` bounded.
 *
 * @module @greeneek/gnk-tool-fs-search/presentation
 */
 ...
 * ({@link module:@greeneek/gnk-tool-fs-search/grep} `formatGrepMatches`), so
