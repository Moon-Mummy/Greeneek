 ...
 *
 * @module @greeneek/gnk-tool-fs-search/search-core
 */
...
import { ItemRetainer, TextRetainer } from '@greeneek/gnk-output-retention'
import type { RetainedItems } from '@greeneek/gnk-output-retention'
import type { SubprocessHandle, SubprocessOutcome, SubprocessOutputRead, SubprocessSpawnSpec } from '@greeneek/gnk-subprocess'
import type { SaveTextSpill, SpillRef } from '@greeneek/gnk-spill'
 ...
 * `@greeneek/gnk-tool-call-timeout-policy` to enforce through `exec.signal`.
 */
 ...
 * (`gnk-spill-policy`) only shrinks a result's `content`, never its `meta`, so the
 ...
 * (`@greeneek/gnk-tool-call-timeout-policy`) and caller cancellation terminate the
 * process tree.
 *
 ...
 * retention pass both the model-facing render ({@link module:@greeneek/gnk-tool-fs-search/grep}
 * `formatGrepOutput`) and the search-card projection
 * ({@link module:@greeneek/gnk-tool-fs-search/presentation} `grepSearchMeta`)
