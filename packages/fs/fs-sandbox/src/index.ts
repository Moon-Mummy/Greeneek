 ...
 * `@greeneek/gnk-fs` Service Definition. It extends `LocalFileSystem` so all
 ...
 * (`@greeneek/gnk-bash-sandbox`). The residual
 ...
 * `FS_SANDBOX_DENIED`.
 *
 * @module @greeneek/gnk-fs-sandbox
 */

import { Context } from '@greeneek/cordis'
import { LocalFileSystem } from '@greeneek/gnk-fs-local'
import type { Config as LocalConfig } from '@greeneek/gnk-fs-local'
import { FsError } from '@greeneek/gnk-fs'
import type { FsEditOutcome, FsEditRequest, FsTarget, FsVersion, FsWriteIntent, FsWriteOutcome } from '@greeneek/gnk-fs'
import { writableRoots } from '@greeneek/gnk-sandbox'
 ...
 * INSTEAD OF `gnk-fs-local`, together with a `ctx.sandboxPolicy`, is the whole
 * swap — the model-facing tools are untouched). Its configured default mode is
 * the capability fact exposed by {@link sandboxMode}; `gnk-tool-fs` resolves
