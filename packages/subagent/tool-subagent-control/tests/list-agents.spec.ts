...
import type { SubagentListEntry } from '@greeneek/gnk-subagent'
...
async function setupWith(adapter: MockAdapter | GatedAdapter) {
  ...
  const root = mkdtempSync(join(tmpdir(), 'gnk-tool-list-agents-'))
  roots.push(root)
...
describe('gnk-tool-subagent-control/list-agents', () => {
