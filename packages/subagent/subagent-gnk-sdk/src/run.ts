/**
 * Fresh-process SDK subagent client. Drives one child Greeneek Harness
 * runtime over stdio JSON-RPC through `@greeneek/gnk-sdk-client` and owns
 ...
 *
 * @module @greeneek/gnk-subagent-gnk-sdk/run
 */
...
import {
  GreeneekHarness,
  type GreeneekHarnessOptions,
  type HarnessNotification,
  JsonRpcResponseError,
  SdkProtocolError,
  TransportClosedError,
} from '@greeneek/gnk-sdk-client'
import type { ContentBlock, ReasoningEffortId } from '@greeneek/gnk-llm'
import type { SessionEvent, SessionId, TurnEndReason } from '@greeneek/gnk-session'
...
export interface SdkRunSpec {
  /** Explicit gnk CLI module; omission resolves the SDK client's same-version dependency. */
  gnkBin?: string
  /** Named child profile. */
  profile: string
   ...
   * runtime's own `GREENEEK_API_KEY`). Merged after
   * the seam's `scrubbedParentEnv()` base, so an explicit credential or
   * current `GNK_*` fact survives while ambient namesakes never leak.
   */
...
class SdkRunFailure extends Error {
  constructor(readonly facts: SdkFailureFacts, cause: unknown) {
    super(`subagent-gnk-sdk: ${failureDiagnostic(facts)}`, { cause })
    this.name = 'SdkRunFailure'
  }
}

/** Runtime constructor seam replaced only by package-local fake-runtime tests. */
export const internals: { createHarness(options: GreeneekHarnessOptions): GreeneekHarness } = {
  createHarness: options => new GreeneekHarness(options),
}
 ...
 * @returns an Error whose message contains only the fixed GNK SDK failure line.
 */
...
export async function startSdkRun(request: SubagentStartRequest, spec: SdkRunSpec): Promise<SubagentRun> {
  ...
  const harness = internals.createHarness({
    ...spec.gnkBin === undefined ? {} : { gnkBin: spec.gnkBin },
    profile: spec.profile,
    patches: spec.patches,
    gnkHome: spec.gnkHome,
    processCwd: spec.cwd,
