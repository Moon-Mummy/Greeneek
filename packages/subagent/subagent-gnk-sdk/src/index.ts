/**
 * Out-of-process SDK subagent backend. Each child is a complete Greeneek
 ...
 * @module @greeneek/gnk-subagent-gnk-sdk
 */
...
import type { AgentOptions } from '@greeneek/gnk-agent'
import type { SubagentCapabilities, SubagentProvider, SubagentStartRequest } from '@greeneek/gnk-subagent'
import { assertPositiveFinite, NO_START_CAPABILITIES, resolveChildCwd, validateConfiguredCwd } from '@greeneek/gnk-subagent'
import {
...
export const name = 'subagent-gnk-sdk'
...
export const Config: z<Config> = z.object({
  providerName: z.string().default('gnk-sdk'),
  gnkBin: z.string(),
  profile: z.string().default('sdk'),
  patches: z.array(z.string()).default([]),
  gnkHome: z.string().required(),
  cwd: z.string(),
  provider: z.string().default('greeneek-official'),
  model: z.string().default('greeneek-v4-flash'),
...
type ResolvedConfig = Required<Omit<Config, 'cwd' | 'maxTokens' | 'gnkBin'>> & Pick<Config, 'cwd' | 'maxTokens' | 'gnkBin'>
...
function resolveConfiguredFile(field: string, value: string): string {
  ...
  throw new TypeError(`subagent-gnk-sdk ${field} must name an existing file: ${path}`)
}

/** GNK SDK can apply Agent route options while the other start features remain child-owned. */
...
class SdkSubagentProvider implements SubagentProvider {
  ...
  start(request: SubagentStartRequest) {
    ...
    let cwd: string
    try {
      cwd = resolveChildCwd('subagent-gnk-sdk', this.config.cwd, request.parent.session.header.cwd)
    } catch (error: unknown) {
      const failure = sdkConfigurationFailure(error)
      this.ctx.logger.warn(`subagent-gnk-sdk "${this.name}": child start failed: %o`, error)
      throw failure
    }
    const route = resolveSdkRoute(this.config, request.agentOptions)
    const spec: SdkRunSpec = {
      ...this.config.gnkBin === undefined ? {} : { gnkBin: this.config.gnkBin },
      profile: this.config.profile,
      patches: this.config.patches,
      gnkHome: this.config.gnkHome,
      cwd,
      ...route,
      env: this.config.env,
      ...
      onError: (error, stopReason) => {
        ...
        this.ctx.logger.warn(`subagent-gnk-sdk "${this.name}": child run failed (${stopReason}): ${error.message}`)
      },
    }
...
export function apply(ctx: Context, config: Config): void {
  ...
  assertPositiveFinite('subagent-gnk-sdk', 'shutdownTimeoutMs', resolved.shutdownTimeoutMs)
  assertPositiveFinite('subagent-gnk-sdk', 'disposeEofGraceMs', resolved.disposeEofGraceMs)
  assertPositiveFinite('subagent-gnk-sdk', 'disposeGraceMs', resolved.disposeGraceMs)
  if (resolved.maxTokens !== undefined && (!Number.isSafeInteger(resolved.maxTokens) || resolved.maxTokens <= 0)) {
    throw new TypeError('subagent-gnk-sdk maxTokens must be a positive safe integer')
  }
  if (!isAbsolute(resolved.gnkHome)) throw new TypeError('subagent-gnk-sdk gnkHome must be an absolute path')
  const launchPaths: ResolvedConfig = {
    ...resolved,
    patches: resolved.patches.map((path, index) => resolveConfiguredFile(`patches[${String(index)}]`, path)),
    ...resolved.gnkBin === undefined ? {} : { gnkBin: resolveConfiguredFile('gnkBin', resolved.gnkBin) },
  }
  ...
  const configuredCwd = validateConfiguredCwd('subagent-gnk-sdk', resolved.cwd)
