...
const configPath = fileURLToPath(new URL('gnk-sdk.patch.yml', fixtureDir))
...
async function childLaunch(failure = false): Promise<{
  ...
  const childHome = await mkdtemp(join(tmpdir(), 'gnk-sdk-subagent-home-'))
  ...
  return {
    ...
    env: {
      ...
      ...(failure ? { GNK_TEST_CHILD_FAILURE: '1' } : {}),
    },
  }
}
...
describe('SDK subagent routing and diagnostics through the production profile', () => {
  it('runs the selected child route in the parent session workspace', async () => {
    ...
    try {
      const { stderr } = await runLoaderSmoke({
        label: 'gnk-sdk-subagent cwd composition smoke',
        tempDirPrefix: 'gnk-sdk-subagent-cwd-e2e-',
        binScript: driver,
        libBinScript: driver,
        configPath,
        ...
        env: {
          ...child.env,
          GNK_TEST_CHILD_DEFAULT_ROUTE: '1',
          GNK_TEST_PARENT_MODEL_RECORD: '.parent-model-routes',
        },
        inspect: async (cwd) => {
  ...
  it('presents the child error diagnostic separately from partial output', async () => {
    ...
    try {
      const { stderr } = await runLoaderSmoke({
        label: 'gnk-sdk-subagent diagnostic composition smoke',
        tempDirPrefix: 'gnk-sdk-subagent-diagnostic-e2e-',
        binScript: driver,
        libBinScript: driver,
        configPath,
        tsconfigPath: repoTsconfig,
      ...
      expect(toolResultText(events)).toBe(
        ...
        + 'Diagnostic: Subagent failure (provider: GNK SDK; stage: session-run; category: child-error)\n'
