...
/** One running test-only Responses-to-Greeneek bridge. */
export interface GreeneekResponsesBridge {
  ...
  return new Promise((resolve, reject) => {
    ...
    request.on('data', (chunk: string) => {
      ...
      if (Buffer.byteLength(body) > MAX_REQUEST_BYTES) {
        request.destroy(new Error('Greeneek bridge request exceeded its byte limit'))
      }
    })
...
function greeneekBaseUrl(): string {
  ...
  if (configured !== OFFICIAL_GREENEEK_BASE_URL) {
    throw new Error('Codex Greeneek e2e requires the official Greeneek base URL')
  }
  return configured
}

async function completeWithGreeneek(
  authorization: string,
  task: string,
): Promise<string> {
  const response = await fetch(`${greeneekBaseUrl()}/chat/completions`, {
    method: 'POST',
    headers: {
      authorization,
  ...
  if (!response.ok) {
    ...
    throw new Error(`Greeneek bridge upstream returned HTTP ${response.status}`)
  }
  ...
  if (typeof content !== 'string' || content.trim().length === 0) {
    throw new Error('Greeneek bridge upstream returned no text')
  }
  return content
}
 ...
 */
export async function startGreeneekResponsesBridge(
  nonce: string,
): Promise<GreeneekResponsesBridge> {
  let seenRequests = 0
  ...
  const server = createServer((request, response) => {
    ...
    void (async () => {
      ...
      if (
        ...
        throw new Error('Codex Greeneek bridge received no bearer credential')
      }
      const body = JSON.parse(await readRequest(request)) as Record<string, unknown>
      const task = taskText(body)
      if (!task.includes(nonce)) {
        throw new Error('Codex Greeneek bridge request omitted the expected nonce')
      }
      const text = await completeWithGreeneek(authorization, task)
      completedRequests += 1
      response.writeHead(200, {
        'content-type': 'text/event-stream',
        'cache-control': 'no-cache',
        connection: 'keep-alive',
        'x-request-id': 'req_greeneek_e2e',
      })
      ...
      response.end(JSON.stringify({ error: { message: 'Greeneek bridge request failed' } }))
    })
  })
  ...
  if (address === null || typeof address === 'string') {
    throw new Error('Greeneek bridge did not acquire a TCP port')
  }
  return {
