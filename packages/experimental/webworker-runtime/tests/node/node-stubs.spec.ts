...
import { GNK_HOME, GNK_TMP } from '../../src/storage/paths.ts'
...
describe('replaced external packages', () => {
  it('lists the packages the loader serves from the bundle', () => {
    ...
    expect(REPLACED_EXTERNAL_PACKAGES).not.toContain('@greeneek/node-addon-landlock-run')
...
describe('node:os', () => {
  it('reports the virtual platform identity and the VFS directories', () => {
    ...
    expect([os.release(), os.hostname()]).toEqual(['0.0.0-gnk-worker', 'gnk-worker'])
  })
