...
import { GNK_TMP } from '../../storage/paths.ts'
...
function vfsPath(path: string, cwd: string): string {
  ...
  if (absolute === '/tmp') return GNK_TMP
  if (absolute.startsWith('/tmp/')) return `${GNK_TMP}${absolute.slice('/tmp'.length)}`
  return absolute
}
...
/** Throw the denial dialect consumed by `gnk-bash-sandbox`. */
