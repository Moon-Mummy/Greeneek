 ...
 *
 * Usage: gnk-pack-vfs-image --out <file> [--profile web] [--root /gnk]
 *        node --import tsx/esm src/bin.ts --out ../../apps/web/dist/preview/vfs-image.tar.gz
 * @module @greeneek/gnk-experimental-webworker-packer/src/bin
 */
...
function flag(name: string, fallback?: string): string {
  ...
  if (index === -1) {
    ...
    throw new Error(`gnk-pack-vfs-image: --${name} is required`)
  }
  const value = process.argv[index + 1]
  if (value === undefined || value.startsWith('--')) {
    throw new Error(`gnk-pack-vfs-image: --${name} needs a value`)
  }
  return value
}
