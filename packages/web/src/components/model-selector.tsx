  ...
  const Group = ({ title, list }: { title: string; list: ModelInfo[] }) => {
    ...
    return (
      <div style={{ marginBottom: 14 }}>
        <h4 style={{ fontSize: 11, textTransform: "uppercase", letterSpacing: 0.6, color: "var(--dsw-alias-state-business-primary)", margin: "12px 0 6px 0" }}>{title}</h4>
        {Object.entries(byProvider).map(([prov, arr]) => (
          <div key={prov} style={{ marginBottom: 8 }}>
            <div style={{ fontSize: 11, color: "var(--dsw-alias-label-secondary)", textTransform: "uppercase", margin: "6px 0 4px 0" }}>{prov}</div>
            {arr.map((m) => (
              <div key={m.id} className="row" style={{ padding: "6px 0", borderBottom: "1px solid var(--dsw-alias-border-l2)", alignItems: "center", gap: 8 }}>
                ...
                <div style={{ flex: 1, cursor: "pointer" }} onClick={() => onPick(m.id, m.provider)}>
                  ...
                  <div className="row" style={{ gap: 6, marginTop: 2, flexWrap: "wrap" }}>
                    ...
                    {m.pricing?.isFree ? <span className="chip" style={{ fontSize: 10, background: "var(--dsw-alias-state-business-tertiary)" }}>FREE</span> : null}
