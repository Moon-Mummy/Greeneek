...
`gnk-bash-local` 是 POSIX 上的默认 Bash 执行器：每条命令都以全新的非登录 `bash -c` 进程运行，不读取 rc 文件，因此调用之间不会残留任何 shell 状态。它会为每条命令应用已配置的预算——工作目录、超时、输出上限——对超时与取消进行分类，并在流溢出时返回有界输出与 spill 文件恢复。命令以 harness 进程自身的权限运行：本执行器不做任何隔离，需要沙箱能力时请组合 `gnk-bash-sandbox`。挂载后，面向模型的 `bash` 工具会与它对接。
...
生成的[配置目录](../../../docs/config-catalog.zh.md#greeneekgnk-bash-local)是每个受支持字段及其 JSDoc 的穷尽式真源。
...
通过 `gnk-tool-bash` 间接影响；该工具会渲染本执行器有界的 stdout/stderr 尾部、后台进程增量、spill 文件路径与基础设施失败。
...
- **自身不提供隔离**——命令以 harness 进程的权限运行；需要隔离的部署组合 `gnk-bash-sandbox`，每次调用的 allow/deny/ask 策略则属于工具的 `pre-execute` waterfall。
