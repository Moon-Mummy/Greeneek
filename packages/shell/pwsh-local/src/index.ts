 ...
 *
 * @module @greeneek/gnk-pwsh-local
 */

/* jscpd:ignore-start -- this executor mirrors gnk-bash-local call-for-call by
   design (see this package's README), so the two import the same seam surface */
import { Context } from '@greeneek/cordis'
import z from '@greeneek/schemastery'
import { SHELL_SETTINGS_NAMESPACE, ShellExecutor } from '@greeneek/gnk-shell'
import type { ShellExecRequest, ShellExecSpec, ShellProcess, ShellProcessRead, ShellRunResult, CollectedOutput } from '@greeneek/gnk-shell'
import type { SubprocessCollect, SubprocessHandle, SubprocessOutputReader, SubprocessSpawnSpec } from '@greeneek/gnk-subprocess'
import type {} from '@greeneek/gnk-settings'
import { clampTimeout, deadline, MAX_TIMER_DELAY_MS, timeoutOf } from '@greeneek/gnk-timeout'
/* jscpd:ignore-end */
import { resolvePwshPath } from './resolve.ts'

/* jscpd:ignore-start -- deliberate call-for-call mirror of gnk-bash-local (Agent Note: pwsh-tool-and-executor). */
/**
...
export class PwshLocalExecutor extends ShellExecutor {
   ...
   * `gnk-bash-local`'s `runArgv`/`startArgv` hooks; see
  ...
  private spawnSpec(
    ...
    return {
      ...
      signal,
      env: { ...ENV_OVERRIDES, ...spec.env, ...spec.gnkEnv },
    }
  }
   ...
   * `gnk-bash-local` (whose sandboxing subclass consumes the same hook); the
   * pwsh-confining consumer is `@greeneek/gnk-pwsh-sandbox`.
