 ...
 * `@greeneek/gnk-pwsh-local`) backs `ctx.shell`; the tool contract is
 * PowerShell-dialect: native `C:\...` paths and `$env:NAME` variables.
 *
 * Behavior mirrors `gnk-tool-bash` call-for-call: foreground and
 * `run_in_background` execution (background handles register with the
 * generic `ctx.jobs` runtime), the managed `GNK_*` environment through the
 ...
 * exit-status parse from `@greeneek/gnk-shell`.
 *
 * @module @greeneek/gnk-tool-pwsh
 */

import { isAbsolute, resolve as resolvePath } from 'node:path'
import type { Context } from '@greeneek/cordis'
import z from '@greeneek/schemastery'
import { defineTool, TOOL_ABORTED } from '@greeneek/gnk-tools'
import type { GenericCallView, TerminalCallView, ToolExecution, ToolResult, ToolResultView } from '@greeneek/gnk-tools'
import { HarnessError } from '@greeneek/gnk-llm'
import type { Agent } from '@greeneek/gnk-agent'
import type {} from '@greeneek/gnk-jobs'
...
import { ESCALATION_TARGETS, approveEscalation, validateEscalationArgs } from '@greeneek/gnk-sandbox'
import type { SandboxPolicyService } from '@greeneek/gnk-sandbox-policy'
import type { ShellRunResult } from '@greeneek/gnk-shell'
import { parseExitStatus } from '@greeneek/gnk-shell'
import { processOutcome } from './background.ts'
import { renderPwshProcessRead, renderPwshResult } from './render.ts'
import type { RenderablePwshResult } from './render.ts'

declare module '@greeneek/gnk-jobs' {
...
/* jscpd:ignore-start -- minimal mirror of gnk-tool-bash's validation and execute plumbing (Agent Note). */
...
function pwshDescription(backgroundEnabled: boolean, escalationModes: readonly SandboxMode[]): string {
  ...
  const base = 'Execute a PowerShell command (`pwsh -Command`) and return its stdout/stderr. '
    ...
    + 'Current harness environment facts are exposed through managed `$env:GNK_*` variables; inspect them when needed. '
...
function canonicalPwshResult(result: ShellRunResult): PwshForegroundResult {
  ...
  return {
    ...
    /* jscpd:ignore-start -- the canonical projection and background-handle shape mirror gnk-tool-bash's by design (Agent Note). */
...
/* jscpd:ignore-start -- deliberate mirror of gnk-tool-bash's apply() preamble (pwsh-tool-and-executor Agent Note). */
export function apply(ctx: Context, config: Config = {}): void {
  ...
  /* jscpd:ignore-start -- deliberate mirror of gnk-tool-bash's escalation resolver (pwsh-tool-and-executor Agent Note). */
  /**
  ...
  ctx.tools.register(defineTool({
    ...
    /* jscpd:ignore-start -- deliberate mirror of gnk-tool-bash's parameter surface (pwsh-tool-and-executor Agent Note). */
    parameters: {
    ...
    /* jscpd:ignore-end */
    output: {
      // The foreground result wire shape mirrors gnk-tool-bash's by contract —
      // consumers of one must accept the other (see the pwsh-tool-and-executor
      // Agent Note).
      /* jscpd:ignore-start -- deliberate result-schema symmetry with gnk-tool-bash. */
      schema: {
        oneOf: [
          {
            type: 'object',
    ...
    /* jscpd:ignore-start -- the execute path mirrors gnk-tool-bash's by design (see the pwsh-tool-and-executor Agent Note). */
    async execute(args: PwshToolArgs, exec) {
      ...
      const request = {
        ...
        gnkEnv: ctx.shellEnv.collect(exec),
      ...
      if (args.run_in_background === true) {
        ...
        if (jobs === undefined) {
          throw new Error('background jobs unavailable: load @greeneek/gnk-jobs and @greeneek/gnk-tool-jobs')
        }
