/**
 * Integration tests: the REAL `@greeneek/gnk-pwsh-local` executor plus the
...
import ToolRuntime, { TOOL_ABORTED } from '@greeneek/gnk-tools'
...
import { PwshLocalExecutor, resolvePwshPath } from '@greeneek/gnk-pwsh-local'
...
describe.skipIf(!hasPwsh)('pwsh tool over the real pwsh executor', () => {
  beforeEach(async () => {
    dir = await mkdtemp(join(tmpdir(), 'gnk-tool-pwsh-'))
