 ...
 * managed `GNK_*` collection, abort translation, canonical result projection,
...
import type { ShellExecRequest, ShellExecSpec, ShellProcess, ShellRunResult } from '@greeneek/gnk-shell'
...
import type { ShellProcessRead } from '@greeneek/gnk-shell'
...
async function setup(toolConfig: Partial<ToolPwsh.Config> = {}, gnkHome?: string) {
...
async function setupWithTasks(toolConfig: Partial<ToolPwsh.Config> = {}, gnkHome?: string) {
  const ctx = new Context()
  await ctx.plugin(SystemPrompt)
  await ctx.plugin(ToolRuntime)
  await ctx.plugin(AgentRegistry)
...
describe('execution through the bash seam', () => {
  it('forwards command, session cwd, timeout, and managed GNK_* environment', async () => {
    const gnkHome = mkdtempSync(join(tmpdir(), 'gnk-tool-pwsh-home-'))
    const { ctx, bash } = await setup({}, gnkHome)
    ...
    expect(request?.gnkEnv).toEqual({
      GNK_HOME: gnkHome,
      GNK_SHELL: '1',
      GNK_SESSION_ID: 'session-1',
    })
  ...
  it('omits workdir and the session id without an agent, so executor defaulting applies', async () => {
    ...
    const gnkEnv = bash.requests[0]?.gnkEnv
    expect(gnkEnv).toBeDefined()
    expect(gnkEnv?.['GNK_SHELL']).toBe('1')
    expect(gnkEnv?.['GNK_HOME']).toEqual(expect.any(String))
    expect(gnkEnv).not.toHaveProperty('GNK_SESSION_ID')
  })
...
describe('per-call sandbox policy resolution', () => {
  it('stamps the CALLING SESSION\'s resolved policy onto the request (session cwd, not the server launch dir)', async () => {
    ...
    const sessionCwd = mkdtempSync(join(tmpdir(), 'gnk-tool-pwsh-policy-'))
