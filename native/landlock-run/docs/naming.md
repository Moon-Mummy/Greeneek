...
The public package family belongs to the `@greeneek` scope and uses the `node-addon-landlock-run` package prefix; platform packages append platform information only:

```text
@greeneek/node-addon-landlock-run
@greeneek/node-addon-landlock-run-<platform>
```
