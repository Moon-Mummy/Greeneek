# Agent Note: gnk web 组合默认挂载会话遥测（OTel 上报）
...
- 开发者运行没有遥测配置的 `gnk web` 时，在记录 `/feedback` 之前不会发出遥测网络请求。内部部署需设置 `GNK_TELEMETRY_MODE`，并可让 `GNK_TELEMETRY_OTLP_URL` 指向其他 collector。
