...
仓库不存在顶层 `examples/` 树。具名 `gnk` profile 是唯一的 Node 应用组合。跨包 ACP、headless 与 SDK profile 测试位于 `apps/cli/tests/profiles/`；包专属 Loader 配置与 driver 位于该包的 `tests/fixtures/`。录制会话测试继续位于顶层 `snapshots/`，非会话预期输出继续归属方本地保存。
...
仓库不存在 `demo:acp` 与 `demo:cordis` 脚本。ACP 通过 `gnk --profile acp` 启动；Cordis 指南使用显式 overlay 启动 `gnk web`。`demo:ptc` 继续作为薄 wrapper，以 `GNK_TOOLS_MODE=ptc` 运行 `gnk --profile headless`。
