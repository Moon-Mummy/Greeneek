...
- 配置值 `tools.mode: 'code'` → `'ptc'`（`ToolPresentationMode` 以及 `gnk-tools`、`gnk-agent-tool-presentation` 中的 zod union）
...
保持不变：`run_code` 及其 `code` 参数（它们描述程序载荷，而非模式）、`CodeSdkLanguage`、`CodeRunFailedError`、`gnk-code-runtime*` 包族、第三方二进制名 `codex-code-mode-host`，以及所有冻结的 archived Note。
