...
可复用的 TUI 包（package）仍然保留实现，但 [`gnk` 不再将其作为应用入口交付](../simplification/2026-08-03-explicit-config-gnk-entrypoint.md)。本记录继续负责包边界和终端行为；后续记录负责产品组合。
...
Greeneek Harness 将 [`@greeneek/gnk-tui`](../../../../packages/ui/tui/README.md) 作为独立的 Cordis 插件交付。该插件只负责终端输入与呈现；agent 生命周期、会话持久化、工具执行以及模型可见的提问工具仍由不同组合项负责。插件要求 stdin 和 stdout 均为 TTY；条件不满足时会失败，不会静默切换为逐行输出。

该包是终端入口，而不是完整应用。宿主在已配置 agent 之前挂载 `@greeneek/gnk-tui`，并围绕它组合后端、工具和策略。产品 CLI 目前不交付终端组合；非交互任务使用 headless 模式，Web 是已安装产品中面向人的界面，而 ACP 仍是独立的自动化协议。
...
- **把 readline 与全屏模式都保留在 `@greeneek/gnk-stdio` 中**：不予采纳，因为逐行输出和差分 TTY 渲染具有不同的依赖、输入规则、日志所有权和资源清理义务。拆分为独立包可以让管道安全契约保持精简、明确。
