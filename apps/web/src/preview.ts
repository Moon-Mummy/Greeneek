 ...
 */
import GnkWorker from '@greeneek/gnk-experimental-webworker-runtime/worker?worker'
import {
  chooseWorkerHostSource, connectWorkerHost, IMAGE_FILE_NAME,
} from '@greeneek/gnk-experimental-webworker-runtime/client'

const image = `preview/${IMAGE_FILE_NAME}`
const source = await chooseWorkerHostSource({ image })
await connectWorkerHost(new GnkWorker({ name: 'gnk-host' }), { image, overlays: source.overlays })
