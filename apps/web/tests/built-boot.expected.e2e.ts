...
it('boots the built plugin graph and renders a fixture session end to end', async () => {
  ...
  if (clientBuildValue('GNK_CLIENT_BUILD_PROFILE') === 'official') {
    expect(document.querySelector('svg[viewBox="26 0 156 24"]')).not.toBeNull()
    expect(screen.queryByText('GNK Local Build')).toBeNull()
  } else {
    expect(document.querySelector('svg[viewBox="0 0 23.16 17.04"]')).not.toBeNull()
    const version = clientBuildValue('GNK_CLIENT_VERSION')
    if (version === undefined) throw new Error('default client build record must carry GNK_CLIENT_VERSION')
    const commit = clientBuildValue('GNK_CLIENT_COMMIT_HASH')
    const buildVersion = version
      + (commit === undefined ? '' : `-${commit}`)
      + (clientBuildValue('GNK_CLIENT_GIT_DIRTY') === 'true' ? '-dirty' : '')
    screen.getByText('GNK Local Build')
  ...
  for (const plugin of ['@greeneek/gnk-client-ui-layout', '@greeneek/gnk-client-ui-sidebar', '@greeneek/gnk-client-ui-conversation', '@greeneek/gnk-client-ui-tool']) {
...
it('boots without ui-chat and does not select another conversation view implicitly', async () => {
  mountAssembledApp('?fixture', { exclude: ['@greeneek/gnk-client-ui-chat'] })

  const tree = await screen.findByRole('tree', { name: 'Sessions' }, { timeout: 10_000 })
  const boot = Reflect.get(window, '__GNK_BOOT__') as { entries: Array<{ id: string }> } | undefined
  expect(boot?.entries.some(entry => entry.id === '@greeneek/gnk-client-ui-chat')).toBe(false)
