...
it('isolates replay skill discovery from every ambient host root', async () => {
  const ambient = await mkdtemp(join(tmpdir(), 'gnk-web-ambient-skills-'))
  const gnkHome = join(ambient, 'gnk-home')
  const agentsHome = join(ambient, 'agents-home')
  const bundled = join(ambient, 'bundled')
  await Promise.all([
    writeSkill(join(gnkHome, 'skills'), 'ambient-gnk'),
    writeSkill(join(agentsHome, 'skills'), 'ambient-agents'),
    writeSkill(bundled, 'ambient-bundled'),
  ])

  const originalGnkHome = process.env.GNK_HOME
  const originalAgentsHome = process.env.GNK_AGENTS_HOME
  const originalBundled = process.env.GNK_BUNDLED_SKILL_DIR
  process.env.GNK_HOME = gnkHome
  process.env.GNK_AGENTS_HOME = agentsHome
  ...
  try {
    ...
    try {
      ...
      expect(names).not.toContain('ambient-gnk')
    ...
    try {
      ...
      if (originalGnkHome === undefined) delete process.env.GNK_HOME
      else process.env.GNK_HOME = originalGnkHome
      if (originalAgentsHome === undefined) delete process.env.GNK_AGENTS_HOME
      else process.env.GNK_AGENTS_HOME = originalAgentsHome
      if (originalBundled === undefined) delete process.env.GNK_BUNDLED_SKILL_DIR
      else process.env.GNK_BUNDLED_SKILL_DIR = originalBundled
