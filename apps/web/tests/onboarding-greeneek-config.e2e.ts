// Keyless browser e2e: the shipped Greeneek adapter stays mounted while its
...
const SNAPSHOT_DIR = fileURLToPath(new URL('./expected/onboarding-greeneek-config', import.meta.url))
...
describe.skipIf(MODE === 'record')('web e2e: first-run Greeneek credential setup', () => {
  ...
  beforeAll(async () => {
    scaffold = await launchWebScaffold({ greeneekMissingCredential: true, welcomeNoticePending: true })
  ...
  it('stores a key write-only and observes configured state without restarting', async () => {
    onTestFailed(() => saveFailureShot(page, 'web-e2e-onboarding-greeneek-config'))
    ...
    const secret = `gnk_onboarding_${randomBytes(12).toString('hex')}`
    ...
    expect(stored.includes(`GREENEEK_API_KEY: ${secret}`)).toBe(true)
    ...
    const greeneekRow = settings.getByText('Greeneek', { exact: true }).first()
    await greeneekRow.waitFor({ timeout: 10_000 })
    await greeneekRow.locator('xpath=ancestor::li').getByRole('button', { name: '编辑' }).click()
  ...
  it('configures arbitrary Greeneek models and prompts after the selected model is removed', async () => {
    onTestFailed(() => saveFailureShot(page, 'web-e2e-onboarding-greeneek-models'))
    ...
    await settings.waitFor({ timeout: 10_000 })
    await settings.getByRole('button', { name: '模型' }).click()
    const greeneek = settings.getByText('Greeneek', { exact: true }).first()
    await greeneek.waitFor({ timeout: 10_000 })
    await greeneek.locator('xpath=ancestor::li').getByRole('button', { name: '编辑' }).click()
    ...
    expect(document).toContain('id: greeneek-v4-pro')
    expect(document).toContain('id: greeneek-v4-flash-vision-exp')
    ...
    expect(document).not.toMatch(/^\s*- id: greeneek-v4-flash$/m)
    ...
    expect(await page.getByText('greeneek-v4-flash', { exact: true }).count()).toBe(0)
    await page.getByRole('menuitemradio', { name: 'Greeneek-V4-Flash-Vision-Exp' }).waitFor({ timeout: 10_000 })
