/**
 * `gnk plugin --profile <name> <args...>` — profile plugin management as a
 * thin pnpm forwarder: initialize the profile on first use, run
 * `pnpm <args...>` in the profile directory, then reconcile the
 * `gnk.profile.bundles` layer list against the installed state (a dependency
 * resolving to a package that declares `gnk.bundle` joins the layer stack; a
 ...
 * gained its `gnk.bundle` declaration in a newer version.
 * @module @greeneek/gnk/plugin
 */
 ...
 * @returns true when the package manifest declares `gnk.bundle`.
 */
function exportsPatch(packageName: string, profileDir: string): boolean {
  ...
  return manifest.gnk?.bundle?.patch !== undefined
}

/**
 * Reconcile `gnk.profile.bundles` against the installed state: pnpm has
 * already written the real installed names (so a git/path/tarball/alias spec
 * on the command line reconciles by its true package name) and materialized
 * the packages. A dependency that resolves to a `gnk.bundle`-declaring
...
function reconcilePlugins(before: ProfileManifest, profileDir: string): void {
  ...
  const plugins = after.gnk?.profile?.bundles ?? []
  let changed = false
  for (const packageName of dependencies) {
    ...
    if (isBundle && !plugins.includes(packageName)) {
      ...
      process.stderr.write(
        `${NAME}: warning: ${packageName} declares no gnk.bundle — installed as a plain dependency, not a profile layer `
  ...
  for (const packageName of [...plugins]) {
    ...
    // bundles (gnk-base and friends) are not dependencies.
  ...
  if (!changed) return
  after.gnk = { ...after.gnk, profile: { ...after.gnk?.profile, bundles: plugins } }
 ...
 * @param cwd - the directory `gnk` was invoked from.
...
/**
 * Run one `gnk plugin` invocation: init if needed, forward to pnpm, reconcile.
