/**
 * Commander adapter for the `gnk` command line.
 *
 ...
 * `@greeneek/gnk-cmdline`). Launcher flags therefore come first: the first
 * token this parser does not recognize starts the inner arguments, so
 * `gnk --profile tui --resume abc` boots the tui profile with `--resume abc`,
 * and `gnk --profile web -h` prints the web app's help, not this one's.
 *
 * `web` is a hardcoded alias for `--profile web`; `plugin` manages a profile's
 * plugin dependencies by forwarding to pnpm.
 * @module @greeneek/gnk/args
 */
...
/** The resolved `gnk` invocation. Help, version, and errors exit inside {@link parseGnkArgs}. */
export type GnkInvocation = ProfileInvocation | DumpConfigInvocation | PluginInvocation
...
const HELP_EXAMPLES = `
Examples:
  gnk --profile web                          boot the web profile (same as: gnk web)
  gnk --profile headless "run the tests"     answer one task, print the result, and exit
  gnk --profile tui --patch ./extra.yml      boot a custom profile with one extra overlay
  gnk --profile tui --resume <session>       arguments after the launcher flags reach the app
  gnk --profile web --help                   the web app's own flags and help
  gnk plugin --profile tui add <package>     install a plugin into the tui profile
`
 ...
 */
function resolveBoot(program: Command, profile: string, options: BootOptions, args: string[]): GnkInvocation {
 ...
 * @returns the resolved invocation.
 */
export function parseGnkArgs(argv: readonly string[], version: string): GnkInvocation {
  let resolved: GnkInvocation | undefined
  ...
  program
    ...
    .description('gnk: boot a Greeneek Harness profile — an ordered stack of plugin-bundle patch layers under your own overrides.')
    .addHelpText('after', HELP_EXAMPLES)
    .exitOverride()
    // The launcher's flags come first and end at the first token it does not
    // know; everything from there on belongs to the booted app, including
    // its -h. `gnk -h` with no profile still prints this help, below.
    .helpOption(false)
    .allowUnknownOption()
    .passThroughOptions()
    .enablePositionalOptions()
    .argument('[args...]', 'arguments for the booted profile\'s app (see: gnk --profile <name> --help)')
    .option('--profile <name>', 'the profile under $GNK_HOME/profiles to boot')
    .option('--patch <path>', 'extra patch-list overlay applied after the profile layer (repeatable)', collect)
    .option('--dump-config', 'print the composed profile tree and exit')
    .option('--dump-default-config', 'print the profile tree without its user layer or --patch overlays and exit')
    .action((args: string[], options: BootOptions & { profile?: string }) => {
      // With the app owning -h, the launcher's own help is what a bare
      // `gnk -h` (no profile to hand it to) must print.
  ...
  web
    ...
    .passThroughOptions()
    .enablePositionalOptions()
    .argument('[args...]', 'arguments for the web app (see: gnk web --help)')
    .option('--patch <path>', 'extra patch-list overlay applied after the profile layer (repeatable)', collect)
  ...
  if (resolved === undefined) throw new Error('gnk: no invocation resolved')
  return resolved
}
