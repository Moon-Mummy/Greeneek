...
describe.skipIf(!process.env.GREENEEK_API_KEY)('coding task: fix a failing test via bash', () => {
  it('repairs add.js so node add.test.js passes', async () => {
    workdir = await mkdtemp(join(tmpdir(), 'gnk-coding-task-'))
    ...
    const agent = await ctx.agentLoop.create(SessionId('e2e-task'), { provider: 'greeneek-official', model: 'greeneek-v4-flash' })
