...
describe.skipIf(!process.env.GREENEEK_API_KEY)('todo_write: real model records a plan', () => {
  it('appends a todo/write event with the model-produced task list', async () => {
    workdir = await mkdtemp(join(tmpdir(), 'gnk-todo-write-e2e-'))
    ctx = await codingHarness(workdir, { persona: TODO_SYSTEM_PROMPT })
    const agent = await ctx.agentLoop.create(SessionId('e2e-todo'), { provider: 'greeneek-official', model: 'greeneek-v4-flash' })
