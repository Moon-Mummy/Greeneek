/** Generic keyless ACP v1 automation-control conformance over the real gnk profile. */
...
describe('standard ACP v1 control surface', () => {
  it('selects, mounts MCP, closes, restarts, resumes, and cancels through the SDK only', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'gnk-acp-control-'))
    const persistenceRoot = join(cwd, '.sessions')
    const env = { GNK_CONFORMANCE_PERSISTENCE_ROOT: persistenceRoot, GNK_TELEMETRY_DISABLED: '1' }
