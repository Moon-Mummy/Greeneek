...
import { parseGnkArgs } from '../src/args.ts'

const parse = (argv: string[]) => parseGnkArgs(argv, '1.2.3')
...
describe('parseGnkArgs', () => {
  ...
  it('routes the plugin pnpm forwarder', () => {
    ...
    expect(parse(['plugin', '--profile', 'tui', 'why', '@greeneek/cordis']))
      .toEqual({ mode: 'plugin', profile: 'tui', args: ['why', '@greeneek/cordis'] })
