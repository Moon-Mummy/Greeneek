...
class GreeneekHarnessConfig:
    ...
    GREENEEK_API_KEY and GREENEEK_BASE_URL settings keep working. Use ``env`` to
    intentionally override or inject variables for a subprocess.
    """

    provider: str = "greeneek-official"
    model: str = "greeneek-v4-flash"
...
class GreeneekHarness:
    """Reusable synchronous SDK for running Greeneek Harness agent turns.
    ...
    def __init__(
        self,
        config: GreeneekHarnessConfig | None = None,
        *,
        _launch_args: tuple[str, ...] | None = None,
        **kwargs: object,
    ) -> None:
        if config is not None and kwargs:
            raise TypeError("pass either GreeneekHarnessConfig or keyword options, not both")
        self.config = config or GreeneekHarnessConfig(**kwargs)
        ...
        if self.config.base_url is not None:
            env["GREENEEK_BASE_URL"] = self.config.base_url
        if self.config.api_key is not None:
            env["GREENEEK_API_KEY"] = self.config.api_key

        self._client = HarnessClient(
            HarnessConfig(
                gnk_bin=self.config.gnk_bin,
                profile=self.config.profile,
                patches=self.config.patches,
                gnk_home=self.config.gnk_home,
                cwd=runtime_cwd,
                env=env,
    ...
    def __enter__(self) -> "GreeneekHarness":
        self.start()
        return self
...
class Session:
    def __init__(self, harness: GreeneekHarness, session_id: str) -> None:
