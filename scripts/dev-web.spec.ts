...
it('samples one local environment at startup without validating watcher outputs', async () => {
  const root = await mkdtemp(join(tmpdir(), 'gnk-dev-web-environment-'))
  try {
    ...
    writeClientBuildRecord(root, {
      ...
      GNK_CLIENT_COMMIT_HASH: 'fffffff',
      GNK_CLIENT_TITLE: 'Greeneek Harness',
      GNK_CLIENT_VERSION: '1.2.2',
    })
    ...
    expect(devWebBuildEnvironment(root, {
      ...
      GNK_CLIENT_COMMIT_HASH: 'abc1234',
      GNK_CLIENT_EXTRA: 'launch-value',
    })).toEqual({
      PATH: '/bin',
      GNK_CLIENT_COMMIT_HASH: 'abc1234',
      GNK_CLIENT_EXTRA: 'launch-value',
...
it('discovers gnk.client packages with sibling roles', async () => {
  const root = await mkdtemp(join(tmpdir(), 'gnk-dev-web-discovery-'))
  try {
...
it('discovers client-preset packages the shell links, excluding loader-delivered and test infrastructure', async () => {
  const root = await mkdtemp(join(tmpdir(), 'gnk-dev-web-library-'))
  try {
    ...
    await write('packages/client/delivered', { gnk: { client: { platform: 'web' } } }, clientPreset)
...
it('rebuilds a client-plugin bundle after its source changes', async () => {
  const root = await mkdtemp(join(tmpdir(), 'gnk-dev-web-watch-'))
  let bundles: TsdownBundle[] = []
  try {
    await symlink(join(import.meta.dirname, '..', 'node_modules'), join(root, 'node_modules'), 'dir')
    await writeFile(join(root, 'package.json'), JSON.stringify({ name: '@gnk-test/dev-web-watch', private: true, type: 'module' }))
