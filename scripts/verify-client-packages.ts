...
export interface ClientDeclaration {
  ...
  /** Exact runtime specifiers used to validate `gnk.client.external` declarations. */
 ...
 * @returns Declarations and malformed gnk.client fields.
 */
...
/**
 * Repair malformed or redundant `gnk.client` declaration entries.
 * @param root - Absolute repository root.
...
export function fixClientPackageManifests(root: string, facts: ClientPackageFacts): string[] {
  ...
  for (const declaration of facts.declarations.filter(entry => entry.dynamic)) {
    ...
    const gnk = isRecord(target.manifest.gnk) ? target.manifest.gnk : undefined
    const client = isRecord(gnk?.client) ? gnk.client : undefined
...
function collectModeViolations(facts: ClientPackageFacts): string[] {
  ...
  for (const pkg of facts.packages) {
    if (pkg.dynamic && pkg.staticLinked) {
      violations.push(
        pkg.manifest + ': ' + pkg.name + ' declares gnk.client and uses the staticLinked preset;'
      ...
      violations.push(
        ...
        + ' declare gnk.client or use the staticLinked preset',
      )
    }
  }
  ...
  for (const pkg of facts.declarations.filter(entry => entry.dynamic)) {
    for (const field of ['external', 'inject'] as const) {
      ...
      for (const value of pkg[field]) {
        if (value === '') violations.push(pkg.manifest + ': gnk.client.' + field + ' contains an empty value')
        else if (seen.has(value)) {
          violations.push(pkg.manifest + ': gnk.client.' + field + ' lists ' + JSON.stringify(value) + ' twice')
        }
        seen.add(value)
      }
    }
    ...
    for (const specifier of new Set(pkg.external)) {
      ...
      if (baseline.has(specifier)) {
        violations.push(
          pkg.manifest + ': gnk.client.external repeats baseline module ' + JSON.stringify(specifier)
      ...
      if (supplier === pkg.name) {
        violations.push(pkg.manifest + ': gnk.client.external names its own row ' + JSON.stringify(specifier))
        ...
        if (pkg.runtimeSourceSpecifiers[specifier] === undefined) {
          violations.push(
            pkg.manifest + ': gnk.client.external ' + JSON.stringify(specifier)
        ...
        violations.push(
          pkg.manifest + ': gnk.client.external ' + JSON.stringify(specifier) + ' has no supplier;'
          + (byName.has(owner)
            ? ' workspace package ' + owner
              + ' declares no dynamic gnk.client row and the shell does not seed this specifier'
...
function formatCycle(
  ...
  return manifest + ': synchronous gnk.client.external cycle: ' + chain + ' ' + (entry?.from ?? '')
}
...
function readDeclaration(
  ...
  const gnk = isRecord(manifest.gnk) ? manifest.gnk : undefined
  const rawClient = gnk?.client
  ...
  if (!isRecord(rawClient)) {
    malformed.push(manifestPath + ': ' + manifest.name + ' gnk.client must be an object')
    return {
      name: manifest.name, manifest: manifestPath, dynamic: false, external: [], inject: [],
      runtimeSourceUses: {}, runtimeSourceSpecifiers: {},
    }
...
function stringArray(
  ...
  if (!Array.isArray(value) || value.some(entry => typeof entry !== 'string')) {
    malformed.push(manifestPath + ': ' + packageName + ' gnk.client.' + field + ' must be a string array')
    return []
  }
