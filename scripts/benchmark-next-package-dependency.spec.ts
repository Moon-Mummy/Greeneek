...
describe('next package benchmark graph', () => {
  it('applies a source-derived candidate without changing the filesystem', () => {
    ...
    const facts: PackageDependencyFacts = {
      ...
      manifest,
      workspaceNames: new Set(['@greeneek/cordis', '@f/probe', '@f/runtime', '@f/types']),
  ...
  it('finds reachable unconfigured packages with non-Cordis peers', () => {
    const index = new Map([
      ['@greeneek/gnk', new Map([['1.0.0', {
        name: '@greeneek/gnk', version: '1.0.0', dependencies: { '@f/a': '^1.0.0', '@f/b': '^1.0.0' },
      }]])],
      ['@f/a', new Map([['1.0.0', {
        name: '@f/a', version: '1.0.0', peerDependencies: { '@f/runtime': '^1.0.0' },
      }]])],
      ['@f/b', new Map([['1.0.0', {
        name: '@f/b', version: '1.0.0', peerDependencies: { '@greeneek/cordis': '^4.0.0' },
      }]])],
    ...
    expect(discoverBenchmarkCandidates(
      index,
      new Map([['@greeneek/gnk', '1.0.0'], ['@f/a', '1.0.0'], ['@f/b', '1.0.0']]),
      release,
      new Set(),
    )).toEqual(['@f/a'])
  })
