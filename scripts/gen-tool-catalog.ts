...
import type { ToolSchema } from '@greeneek/gnk-llm'
...
import SqliteSessionQueryEngine from '@greeneek/gnk-session-query-sqlite'
import GoalService from '@greeneek/gnk-goal'
import SystemPrompt from '@greeneek/gnk-system-prompt'
import ToolRuntime, { type Config as ToolsConfig } from '@greeneek/gnk-tools'
import LocalBashExecutor from '@greeneek/gnk-bash-local'
import * as BashEnvPlugin from '@greeneek/gnk-shell-env'
import { PwshLocalExecutor } from '@greeneek/gnk-pwsh-local'
...
import type { SubagentProvider } from '@greeneek/gnk-subagent'
import * as ToolSubagentControl from '@greeneek/gnk-tool-subagent-control'
import * as ToolSubagentListAgents from '@greeneek/gnk-tool-subagent-control/list-agents'
import SkillRegistry from '@greeneek/gnk-skill'
import * as SkillFileSystem from '@greeneek/gnk-skill-filesystem'
import LocalJobRegistry from '@greeneek/gnk-jobs-local'
import * as ToolAskUser from '@greeneek/gnk-tool-ask-user'
import * as ToolBash from '@greeneek/gnk-tool-bash'
import * as ToolPwsh from '@greeneek/gnk-tool-pwsh'
...
import CordisHostRunner from '@greeneek/gnk-cordis-host-runner'
import * as ToolCordis from '@greeneek/gnk-tool-cordis'
import * as ToolFs from '@greeneek/gnk-tool-fs'
import * as ToolFsSearch from '@greeneek/gnk-tool-fs-search'
import * as ToolStrReplaceEditor from '@greeneek/gnk-tool-str-replace-editor'
import TerminalSessionService from '@greeneek/gnk-terminal'
import * as ToolPty from '@greeneek/gnk-tool-terminal'
import * as ToolGoal from '@greeneek/gnk-tool-goal'
import * as ToolSchedule from '@greeneek/gnk-schedule'
import Lsp from '@greeneek/gnk-lsp'
import * as ToolLsp from '@greeneek/gnk-tool-lsp'
import * as ToolSkill from '@greeneek/gnk-tool-skill'
import * as ToolSessionQuery from '@greeneek/gnk-tool-session-query'
import * as ToolTasks from '@greeneek/gnk-tool-jobs'
import type TeamService from '@greeneek/gnk-experimental-agent-team'
import * as ToolTeam from '@greeneek/gnk-experimental-tool-agent-team'
...
import VmWorkflowEngine from '@greeneek/gnk-workflow-worker-thread'
import * as ToolRalph from '@greeneek/gnk-tool-ralph'
import * as ToolWorkflow from '@greeneek/gnk-tool-workflow'
...
const TOOL_PACKAGES: ToolPackage[] = [
  {
    pkg: '@greeneek/gnk-tool-ask-user',
    dir: 'tool-ask-user',
  ...
  },
  {
    ...
    note:
      'The bash tool is the model-facing consumer of the bash executor seam. A `run_in_background` run registers with the generic `ctx.jobs` runtime and is collected/stopped through the `job_*` tools from `@greeneek/gnk-tool-jobs`; the `enableRunInBackground` config (default true) removes the parameter entirely when disabled.',
  },
  {
    pkg: '@greeneek/gnk-tool-pwsh',
    dir: 'tool-pwsh',
    ...
    note:
      'The pwsh tool is the PowerShell-dialect consumer of the bash executor seam for Windows compositions (a PowerShell executor such as `@greeneek/gnk-pwsh-local` backs `ctx.shell`); it mirrors the bash tool call-for-call minus sandbox controls — `run_in_background` runs register with the generic `ctx.jobs` runtime and are collected/stopped through the `job_*` tools, and the managed `GNK_*` environment comes from `@greeneek/gnk-shell-env`. Each call runs in a fresh process (no persistent PTY session), with native `C:\\...` paths and `$env:NAME` variables.',
  },
  {
    pkg: '@greeneek/gnk-tool-cordis',
    dir: 'tool-cordis',
    ...
    note:
      'Not in any shipped tree (a deliberate opt-in — dynamic package code reaches the real runtime, see .agents/notes/implemented/feature/2026-07-08-self-referential-cordis-toolset.md). The toolset injects `ctx.dynamicCordisRunner` from `@greeneek/gnk-cordis-host-runner`, which owns the definition registry and the vm sandbox; a composition missing it never activates the tools. A running package may register ADDITIONAL model-visible tools until it is stopped, undefined, or GNK restarts; a full changed request header logs those tool-set changes.',
  },
  {
    pkg: '@greeneek/gnk-tool-bash-persistent',
  ...
  {
    pkg: '@greeneek/gnk-tool-pwsh-persistent',
  ...
  {
    pkg: '@greeneek/gnk-tool-str-replace-editor',
  ...
  },
  {
    ...
    note:
      'The read-before-write/edit policy is added by `@greeneek/gnk-fs-observation-policy` (an `fs/*` event-gate plugin, no schema change); a deployment that loads these tools is expected to also load it. The image tool is not registered without `ctx.attachments`; its schema is route-independent, and execution refuses unless the exact routed model declares image input.',
  },
  {
    pkg: '@greeneek/gnk-tool-fs-search',
    dir: 'tool-fs-search',
  ...
  {
    pkg: '@greeneek/gnk-tool-terminal',
    dir: 'tool-terminal',
  ...
  },
  {
    ...
    note:
      'The lsp tool keeps provider selection and language-server subprocesses behind ctx.lsp, so its model-visible schema stays stable across providers. Requires a registered provider (e.g. `@greeneek/gnk-lsp-stdio`) at runtime; without one, a query returns the structured `LSP_UNAVAILABLE` error rather than changing the schema.',
  },
  {
    pkg: '@greeneek/gnk-tool-ralph',
    dir: 'tool-ralph',
  ...
  },
  {
    ...
    async mount(ctx) {
      ...
      await ctx.plugin(SkillFileSystem, {
        gnkHome: resolve(root, '.tmp/tool-catalog/.gnk'),
  ...
  {
    pkg: '@greeneek/gnk-tool-session-query',
  ...
  {
    pkg: '@greeneek/gnk-tool-subagent',
    dir: 'tool-subagent',
    source: {
  ...
  {
    pkg: '@greeneek/gnk-tool-subagent-control',
  ...
  {
    pkg: '@greeneek/gnk-experimental-tool-agent-team',
    dir: 'tool-agent-team',
    ...
    scope: ctx => catalogChildScopes.get(ctx) as Agent,
    note:
      'All nine tools are scoped to implicit Team Leads and durable teammates. The shipped gnk-base bundle keeps the package disabled; the documented Agent Teams profile patch enables it while disabling the legacy continuable-child control names.',
  },
  {
    pkg: '@greeneek/gnk-tool-todo',
    dir: 'tool-todo',
  ...
  {
    pkg: '@greeneek/gnk-tool-workflow',
    dir: 'tool-workflow',
