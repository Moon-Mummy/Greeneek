...
function facts(manifest: PackageDependencyManifest): PackageDependencyFacts {
  return {
    ...
    workspaceNames: new Set([
      CORDIS,
      '@greeneek/gnk-runtime',
      '@greeneek/gnk-types',
      '@greeneek/gnk-stale',
      '@greeneek/schemastery',
    ]),
    allSourceUses: new Map([
      ['@greeneek/gnk-runtime', ['packages/core/probe/src/index.ts']],
      ['@greeneek/gnk-types', ['packages/core/probe/src/types.ts']],
    ]),
    hostRuntimeSourceUses: new Map([
      ['@greeneek/gnk-runtime', ['packages/core/probe/src/index.ts']],
    ]),
    hostRuntimeExportUses: [{
      packageName: '@greeneek/gnk-runtime',
      specifier: '@greeneek/gnk-runtime',
      exportName: 'runtimeValue',
      sourcePath: 'packages/core/probe/src/index.ts',
      line: 1,
      column: 10,
      sourceLine: "import { runtimeValue } from '@greeneek/gnk-runtime'",
    }],
...
describe('package dependency scope', () => {
  it('keeps the measured Host relay roster explicit', () => {
    ...
    expect(PACKAGE_DEPENDENCY_POLICY.configurationOnlyDevDependencies).toEqual({
      '@greeneek/gnk-client-locale': ['@greeneek/gnk-api-remotes'],
      '@greeneek/gnk-client-ui-conversation': [
        '@greeneek/gnk-api-remotes',
        '@greeneek/gnk-client-ui-workspace',
      ],
      '@greeneek/gnk-client-ui-model-selection': ['@greeneek/gnk-client-ui-input-trigger'],
      '@greeneek/gnk-client-ui-sidebar': ['@greeneek/gnk-client-ui-workspace'],
      '@greeneek/gnk-client-ui-subagent': ['@greeneek/gnk-client-ui-input-trigger'],
      '@greeneek/gnk-client-ui-theme': ['@greeneek/gnk-api-remotes'],
      '@greeneek/gnk-client-ui-tool': ['@greeneek/gnk-api-remotes'],
    })
    expect(PACKAGE_DEPENDENCY_POLICY.duplicateSafePackages).toEqual([
      '@greeneek/gnk-brand',
      '@greeneek/gnk-typert-protocol',
      '@greeneek/gnk-util-crypto',
      '@greeneek/gnk-util-values',
    ])
    expect(PACKAGE_DEPENDENCY_POLICY.safeHostDependencyExports['@greeneek/gnk-deque']).toEqual(['Deque'])
    expect(PACKAGE_DEPENDENCY_POLICY.safeHostDependencyExports['@greeneek/schemastery']).toEqual(['default'])
    expect(PACKAGE_DEPENDENCY_POLICY.safeHostDependencyExports['@greeneek/gnk-session/types']).toBeUndefined()
    expect(PACKAGE_DEPENDENCY_POLICY.safeHostDependencyExports['@greeneek/gnk-typert-protocol']).toBeUndefined()
    expect(PACKAGE_DEPENDENCY_POLICY.peerRequiredHostExports['@greeneek/gnk-scope']).toEqual([
      'carrierKeyOf', 'scopeOf', 'scopeTarget',
    ])
    expect(PACKAGE_DEPENDENCY_POLICY.peerRequiredHostExports['@greeneek/gnk-typert-protocol']).toBeUndefined()
  })

  it('discovers the Client directory, gnk.client declarations, and configured Host packages', () => {
    const packages = [
      pkg('@f/static', 'packages/client/static/package.json'),
      pkg('@f/dynamic-client', 'packages/client/dynamic/package.json', { gnk: { client: {} } }),
      pkg('@f/dual', 'packages/api/dual/package.json', { gnk: { client: {} } }),
      ...
      pkg('@f/excluded', 'packages/api/excluded/package.json', { gnk: { client: {} } }),
  ...
  it('rejects stale, redundant, overlapping, and unknown configuration', () => {
    ...
    expect(found.violations).toEqual(expect.arrayContaining([
      ...
      expect.stringContaining('clientFaceExclude names @f/host, which declares no gnk.client entry'),
...
describe('face-aware source classification', () => {
  it('fails when a managed Host package has no Host entry', () => {
    const root = mkdtempSync(join(tmpdir(), 'gnk-package-missing-host-'))
    roots.push(root)
  ...
  it('counts Host values as dependencies and Client values as development inputs', () => {
    const root = mkdtempSync(join(tmpdir(), 'gnk-package-faces-'))
    roots.push(root)
    const subject = pkg('@f/dual', 'packages/g/dual/package.json', {
      gnk: { client: { inject: ['@f/injected'] } },
    })
    const files = {
...
describe('dependency sections', () => {
  it('does not leak repository configuration into captured dependency facts', () => {
    const manifest: PackageDependencyManifest = {
      name: '@greeneek/gnk-client-locale',
      dependencies: { '@greeneek/gnk-runtime': 'workspace:^' },
      devDependencies: { [CORDIS]: 'workspace:^', '@greeneek/gnk-types': 'workspace:^' },
      peerDependencies: { [CORDIS]: 'workspace:^' },
    }
    const base = facts(manifest)
    const subject: PackageDependencyFacts = {
      ...base,
      workspaceNames: new Set([...base.workspaceNames, '@greeneek/gnk-api-remotes']),
    }
  ...
  it('requires non-workspace Host runtime imports in dependencies', () => {
    const manifest: PackageDependencyManifest = {
      name: '@greeneek/gnk-probe',
      dependencies: { '@greeneek/gnk-runtime': 'workspace:^' },
      devDependencies: { [CORDIS]: 'workspace:^', '@greeneek/gnk-types': 'workspace:^', external: '^1.0.0' },
      peerDependencies: { [CORDIS]: 'workspace:^' },
    }
    const subject: PackageDependencyFacts = {
      ...facts(manifest),
  ...
  it('accepts Host dependencies, development-only inputs, and shared Cordis', () => {
    const manifest: PackageDependencyManifest = {
      name: '@greeneek/gnk-probe',
      dependencies: {
        '@greeneek/gnk-runtime': 'workspace:^',
        '@greeneek/schemastery': 'workspace:^',
        external: '^1.0.0',
      },
      devDependencies: {
        '@greeneek/gnk-types': 'workspace:^',
        [CORDIS]: 'workspace:^',
      },
      peerDependencies: { [CORDIS]: 'workspace:^' },
    }
  ...
  it('lists managed Host runtime dependencies for fix review', () => {
    const subject = facts({ name: '@greeneek/gnk-probe' })
    expect(formatManagedRuntimeDependencies({
      ...
      '  @greeneek/gnk-probe -> @greeneek/gnk-runtime: @greeneek/gnk-runtime#runtimeValue',
    ])
  })
  ...
  it('reports an unapproved Host runtime export without rewriting its dependency section', () => {
    ...
    expect(safetyViolations).toEqual([
      'packages/core/probe/src/index.ts:1:10: @greeneek/gnk-runtime#runtimeValue is not classified as '
      + 'safe or peer-required — import { runtimeValue } from \'@greeneek/gnk-runtime\'',
    ])
    expect(fixPackageDependencies('/unused', state)).toEqual([])
    expect(manifest.dependencies).toEqual({ '@greeneek/gnk-runtime': 'workspace:^' })
  })
  ...
  it('keeps an edge as a peer when one imported export requires shared identity', () => {
    ...
    const subject: PackageDependencyFacts = {
      ...facts(manifest),
      peerRequiredHostDependencies: new Set(['@greeneek/gnk-runtime']),
    }
    expect(collectHostDependencyExportPolicyViolations(
      [subject],
      subject.workspaceNames,
      {
        safeHostDependencyExports: {},
        peerRequiredHostExports: {
          '@greeneek/gnk-runtime': ['runtimeValue'],
        },
      },
    )).toEqual([])
  ...
  it('reports wrong sections, workspace ranges, and stale peer metadata', () => {
    const manifest: PackageDependencyManifest = {
      name: '@greeneek/gnk-probe',
      dependencies: { '@greeneek/gnk-types': 'workspace:*' },
      devDependencies: { [CORDIS]: 'workspace:^', '@greeneek/gnk-runtime': 'workspace:^' },
      peerDependencies: { [CORDIS]: 'workspace:*', '@greeneek/gnk-runtime': 'workspace:^' },
      peerDependenciesMeta: { '@greeneek/gnk-missing': { optional: true } },
    }
    const state = {
      facts: [facts(manifest)], packages: [], policyViolations: [], workspaceNames: facts(manifest).workspaceNames,
    }
    ...
    expect(violations).toEqual(expect.arrayContaining([
      expect.stringContaining('@greeneek/gnk-runtime'),
      expect.stringContaining('@greeneek/gnk-types'),
      expect.stringContaining(`${CORDIS} must be matching peerDependencies + devDependencies`),
      expect.stringContaining('dependencies.@greeneek/gnk-types must use workspace:^'),
      expect.stringContaining('peerDependenciesMeta.@greeneek/gnk-missing has no matching'),
    ]))
  })

  it('repairs owned relationships without changing unrelated dependencies', () => {
    const root = mkdtempSync(join(tmpdir(), 'gnk-package-dependencies-'))
    roots.push(root)
    const manifestPath = 'package.json'
    const manifest: PackageDependencyManifest = {
      name: '@greeneek/gnk-probe',
      dependencies: { '@greeneek/schemastery': 'workspace:*', external: '^1.0.0' },
      devDependencies: { [CORDIS]: 'workspace:^', '@greeneek/gnk-runtime': 'workspace:^' },
      peerDependencies: {
        [CORDIS]: 'workspace:^',
        '@greeneek/gnk-runtime': 'workspace:^',
        '@greeneek/gnk-stale': 'workspace:^',
      },
      peerDependenciesMeta: { '@greeneek/gnk-stale': { optional: true } },
    }
