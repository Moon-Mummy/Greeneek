 ...
 * owns: `gnk` and `vendor`. Each family carries its own version baseline, tag
...
const WORKSPACE_ROOT_PACKAGE = '@greeneek/gnk-root'
...
export abstract class ReleaseFamily {
  ...
  members(root: string): ReleaseMember[] {
    ...
    for (const manifestPath of manifestPaths) {
      ...
      if (!name.startsWith('@greeneek/')) throw new Error(`${normalized} must name an @greeneek package`)
...
class GnkFamily extends ReleaseFamily {
  ...
  verifyVersions(members: readonly ReleaseMember[]): void {
    ...
    if (versions.size !== 1) {
      ...
      throw new Error(`gnk release members must share one version:\n${detail}`)
    }
  }
  ...
  readonly installedEntry = { packageName: '@greeneek/gnk', binPath: 'lib/bin.js' }
}
...
class VendorFamily extends ReleaseFamily {
  ...
  tagPrefixFor(member: ReleaseMember): string {
    return `${this.tagPrefix}${member.name.replace('@greeneek/', '')}-v`
  }
...
function releaseFamilies(): readonly ReleaseFamily[] {
  return [new GnkFamily(), new VendorFamily()]
}
