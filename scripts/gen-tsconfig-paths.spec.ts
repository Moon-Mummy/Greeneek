...
describe('generated tsconfig package aliases', () => {
  it('maps each package to its own source directory', () => {
    ...
    const session = aliases.find(alias => alias.specifier === '@greeneek/gnk-session')
    expect(session).toEqual({
      specifier: '@greeneek/gnk-session',
    ...
    expect(aliases.some(alias => alias.specifier === '@greeneek/gnk-typert-protocol')).toBe(false)
  })

  it('yields to a hand-written alias and closes without a trailing comma', () => {
    const aliases = [
      { specifier: '@greeneek/gnk-a', source: './packages/g/a/src', hasInvariant: true },
      { specifier: '@greeneek/gnk-b', source: './packages/g/b/src', hasInvariant: false },
    ]
    const body = renderAliases(aliases, new Set(['@greeneek/gnk-a']))

    // The hand-written bare alias is skipped; its /invariant sibling is not.
    expect(body).toBe([
      '      "@greeneek/gnk-a/invariant": ["./packages/g/a/src/invariant.ts"]',
      '      "@greeneek/gnk-b": ["./packages/g/b/src"]',
    ].join(',\n'))
  ...
  it('names a package that no alias covers', () => {
    ...
    expect(uncoveredPackages(
      ['@greeneek/gnk-a', '@greeneek/gnk-b'],
      new Set(['@greeneek/gnk-a', '@greeneek/gnk-a/invariant']),
    )).toEqual(['@greeneek/gnk-b'])

    expect(uncoveredPackages(['@greeneek/gnk-a'], new Set(['@greeneek/gnk-a']))).toEqual([])
  })
  ...
  it('covers every workspace package in the committed config', () => {
    ...
    expect(names).toContain('@greeneek/gnk-typert-protocol')
  ...
  it('leaves no wildcard that probes every package group', () => {
    ...
    expect(config).not.toContain('"@greeneek/gnk-*":')
    expect(config).not.toContain('"@greeneek/gnk-*/invariant":')
  })
})
