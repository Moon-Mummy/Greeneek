...
function fixture(options: {
  ...
  const root = mkdtempSync(join(tmpdir(), 'gnk-package-invariants-'))
  roots.push(root)
  const packageDirectory = options.packageDirectory ?? 'packages/core/probe'
  const dir = join(root, packageDirectory)
  mkdirSync(join(dir, 'src'), { recursive: true })
  const packageName = options.packageName ?? '@greeneek/gnk-probe'
  ...
  const gnk = options.clientDeclaration === true ? { client: {} } : undefined
  ...
  const manifest = {
    ...
    peerDependencies: !invariantDependency || developmentOnlyInvariant ? {} : {
      '@greeneek/gnk-invariants': 'workspace:^',
    },
...
describe('package invariant gate', () => {
  ...
  it('accepts development-only invariants for configured Host dependencies', () => {
    expect(collectPackageInvariantViolations(fixture({ packageName: '@greeneek/gnk-llm' }))).toEqual([])
  })

  it('accepts development-only invariants for client packages', () => {
    expect(collectPackageInvariantViolations(fixture({
      packageName: '@greeneek/gnk-client-probe',
      packageDirectory: 'packages/client/probe',
    }))).toEqual([])
  })

  it('accepts development-only invariants for packages with a gnk.client entry', () => {
  ...
  it('keeps invariant peers for experimental packages with a gnk.client entry', () => {
    expect(collectPackageInvariantViolations(fixture({
...
export const apply = (ctx: { invariants: { register(name: string, install: typeof install): () => void } }) => {
  ctx.invariants.register('@greeneek/gnk-foreign', install)
...
`
  ...
  it('rejects generated markers and reporter-free executable installers', () => {
    const generated = fixture({
      source: `/** @generated */\n${handwrittenInvariant('@greeneek/gnk-probe')}`,
    })
...
export const apply = (ctx: { invariants: { register(name: string, install: typeof install): () => void } }) =>
  Promise.resolve(ctx.invariants.register('@greeneek/gnk-probe', install))
`,
    })
...
export const apply = (ctx: { invariants: { register(name: string, install: () => void): () => void } }) =>
  ctx.invariants.register('@greeneek/gnk-probe', () => {})
`,
    })
  ...
  it.each([
    ...
    const source = `${handwrittenInvariant('@greeneek/gnk-probe')}\n${defaultExport}\n`
...
export const name = 'probe-invariant'
export const inject = ['invariants']
const PACKAGE_NAME = '@greeneek/gnk-probe'
const install = () => {}
export const apply = (ctx: { invariants: { register(name: string, install: () => void): () => void } }) =>
