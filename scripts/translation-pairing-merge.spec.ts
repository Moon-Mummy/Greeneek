...
function createFixture(attributes = true): Fixture {
  const root = mkdtempSync(join(tmpdir(), 'gnk-translation-pairing-merge-'))
  fixtures.push(root)
  ...
  if (attributes) write(root, '.gitattributes', '*.i18n.yaml merge=gnk-translation-pairing\n')
  return fixture
}
...
// The value matches GNK_COVERAGE_TEST_TIMEOUT_MS, which the Windows coverage
