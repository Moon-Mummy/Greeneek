/** Verify npm's physical package placement for two incompatible GNK releases. */
...
const GNK_PACKAGE = '@greeneek/gnk'
const CORDIS_PACKAGE = '@greeneek/cordis'
const NESTED_GNK_ALIAS = 'gnk-previous'
const NESTED_GNK_PATH = `node_modules/${NESTED_GNK_ALIAS}`
...
export const SYNTHETIC_GNK_VERSIONS = ['0.1.0', '0.2.0'] as const
...
export interface GnkInstallLayoutSummary {
  readonly gnkPackagesPerVersion: number
  readonly checkedGnkEdges: number
}

function isGnkPackage(name: string): boolean {
  return name === GNK_PACKAGE || name.startsWith(`${GNK_PACKAGE}-`)
}
...
function cloneForVersion(manifest: object, version: string): MutableRegistryManifest {
  ...
  for (const field of DEPENDENCY_FIELDS) {
    ...
    for (const name of Object.keys(dependencies)) {
      if (isGnkPackage(name)) dependencies[name] = `^${version}`
    }
  }
  return cloned
}
...
/**
 * Replace the working release with two incompatible, internally consistent GNK releases.
 ...
 * @returns Registry metadata containing both synthetic GNK releases and unchanged external packages.
 */
export function buildDualGnkRegistry(index: RegistryIndex, sourceVersion: string): RegistryIndex {
  const output = new Map(index)
  let gnkPackages = 0
  for (const [name, versions] of index) {
    if (!isGnkPackage(name)) {
    ...
    gnkPackages++
    output.set(name, new Map(SYNTHETIC_GNK_VERSIONS.map(version => [
      version,
      cloneForVersion(source, version),
    ])))
  }
  if (gnkPackages === 0) throw new Error('registry contains no GNK packages')
  return output
}
...
/**
 * Assert that npm isolates both GNK releases while sharing the Cordis runtime.
 * @param packageLock - Metadata-only package lock produced by npm.
 * @returns Counts for the verified GNK packages and dependency edges.
 */
export function assertDualGnkInstallLayout(packageLock: NpmPackageLock): GnkInstallLayoutSummary {
  const [nestedVersion, rootVersion] = SYNTHETIC_GNK_VERSIONS
  const errors: string[] = []
  ...
  let checkedGnkEdges = 0

  for (const [path, manifest] of installed) {
    const name = packageNameAtPath(path, manifest)
    if (name === undefined || !isGnkPackage(name)) continue
    const version = manifest.version
    if (version !== nestedVersion && version !== rootVersion) {
      errors.push(`${path}: expected GNK version ${nestedVersion} or ${rootVersion}, got ${String(version)}`)
      continue
    }
    ...
    const expectedPath = version === rootVersion
      ...
      : name === GNK_PACKAGE
        ? NESTED_GNK_PATH
        : `${NESTED_GNK_PATH}/node_modules/${name}`
    ...
    for (const field of DEPENDENCY_FIELDS) {
      for (const dependency of Object.keys(manifest[field] ?? {})) {
        if (!isGnkPackage(dependency)) continue
  ...
  if (!nestedNames.has(GNK_PACKAGE)) errors.push(`${NESTED_GNK_PATH}: missing ${GNK_PACKAGE}@${nestedVersion}`)
  if (!rootNames.has(GNK_PACKAGE)) errors.push(`node_modules/${GNK_PACKAGE}: missing ${GNK_PACKAGE}@${rootVersion}`)
  ...
  return { gnkPackagesPerVersion: rootNames.size, checkedGnkEdges }
}
...
async function main(): Promise<void> {
  ...
  const index = buildDualGnkRegistry(buildRegistryIndex(root), workspaceVersion(root))
  const [nestedVersion, rootVersion] = SYNTHETIC_GNK_VERSIONS
  const result = await resolveNpmPackageLock(index, {
    [GNK_PACKAGE]: rootVersion,
    [NESTED_GNK_ALIAS]: `npm:${GNK_PACKAGE}@${nestedVersion}`,
  }, TIMEOUT_MS)
  if (result.archiveRequests !== 0) throw new Error(`npm requested ${String(result.archiveRequests)} package archive(s)`)
  const summary = assertDualGnkInstallLayout(result.packageLock)
  console.log(
    `verify-npm-install-layout: ${String(summary.gnkPackagesPerVersion)} GNK package(s) per release and `
    + `${String(summary.checkedGnkEdges)} internal edge(s) verified in ${(result.durationMs / 1000).toFixed(2)} s; `
