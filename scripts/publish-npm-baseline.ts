...
const RELEASE_ENTRY_PACKAGE = '@greeneek/gnk'
...
while time.monotonic() < deadline:
    ...
    if not termination_sent and b"gnk web: http://127.0.0.1:" in snapshot:
        ready_seen = True
...
if not ready_seen:
    sys.stderr.write("installed gnk web did not reach its ready URL\n")
    sys.exit(124)
actual_exit = os.waitstatus_to_exitcode(status)
if actual_exit != 0:
    sys.stderr.write(f"installed gnk web exited {actual_exit}, expected 0\n")
    sys.exit(125)
`
...
class DetachedWorktree {
  ...
  static create(repositoryRoot: string, commit: string, runner: CommandRunner): DetachedWorktree {
    const temporaryRoot = mkdtempSync(join(tmpdir(), 'gnk-npm-baseline-'))
  ...
  static discover(root: string): WorkspacePackageSet {
    ...
    for (const manifestPath of manifestPaths) {
      ...
      if (!name.startsWith('@greeneek/')) {
        throw new Error(`${manifestPath} must name an @greeneek package`)
      }
      if (name === '@greeneek/gnk-root') {
...
/** Installs one complete bundle outside the workspace and probes the shipped gnk entry. */
class InstalledBundleSmoke {
  ...
  run(): void {
    const consumerRoot = mkdtempSync(join(tmpdir(), 'gnk-npm-consumer-'))
    try {
      ...
      writeFileSync(resolve(consumerRoot, 'package.json'), `${JSON.stringify({
        name: 'gnk-npm-baseline-consumer',
        version: '0.0.0',
        private: true,
        dependencies,
      }, null, 2)}\n`)
      ...
      const bin = resolve(consumerRoot, 'node_modules/@greeneek/gnk/lib/bin.js')
      assertPathWithin(consumerRoot, bin, 'installed gnk bin')
      ...
      if (version !== this.bundle.manifest.version) {
        throw new Error(
          `installed gnk --version returned ${JSON.stringify(version)}; `
          + `expected ${this.bundle.manifest.version}`,
        )
      }
      this.probeWeb(bin, consumerRoot, environment)
      console.log('publish-npm-baseline: installed gnk entry and Web startup probes passed')
    } finally {
  ...
  private probeWeb(bin: string, consumerRoot: string, environment: NodeJS.ProcessEnv): void {
    if (process.platform === 'win32') {
      throw new Error('installed gnk Web probe requires a POSIX host with python3')
    }
    const result = this.runner.result(
      'python3',
    ...
    if (result.status !== 0) {
      throw commandFailure('python3', ['installed-gnk-web-probe'], result)
    }
  }
}
...
function parsePackedPackage(value: unknown, index: number): PackedPackage {
  ...
  if (origin === 'harness' && (!name.startsWith('@greeneek/') || name === '@greeneek/gnk-root')) {
...
function installedArtifactEnvironment(consumerRoot: string): NodeJS.ProcessEnv {
  ...
  environment.GNK_HOME = resolve(consumerRoot, '.gnk')
  environment.GNK_AGENTS_HOME = resolve(consumerRoot, '.agents')
  environment.GNK_TELEMETRY_DISABLED = '1'
  environment.GREENEEK_API_KEY = 'keyless-installed-web-no-call'
