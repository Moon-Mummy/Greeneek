...
const SESSION_PACKAGE = '@greeneek/gnk-session'

/** The type-only module that plugin declaration merges augment. */
const SESSION_TYPES_MODULE = '@greeneek/gnk-session/types'
 ...
 * top-level declaration (in `@greeneek/gnk-session`) and any declaration
 * merge inside a `declare module '@greeneek/gnk-session/types'` block. Both forms
...
export function renderKnownEventTypes(events: AnnotatedLogEventEntry[]): string {
  ...
  return [
    ...
    ' * @module @greeneek/gnk-session/known-event-types',
    ' */',
    '',
    '/**',
