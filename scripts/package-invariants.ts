...
function checkManifest(
  ...
  if (owner.packageName === '@greeneek/gnk-invariants') return
  ...
  const peerRange = manifest.peerDependencies?.['@greeneek/gnk-invariants']
  if (developmentOnlyInvariant ? peerRange !== undefined : peerRange !== expectedRange) {
    addViolation(violations, owner.manifestPath, developmentOnlyInvariant
      ? '@greeneek/gnk-invariants must not be a peerDependency under this package dependency policy'
      : '@greeneek/gnk-invariants must be a workspace:^ peerDependency')
  }
  if (manifest.devDependencies?.['@greeneek/gnk-invariants'] !== expectedRange) {
    addViolation(
      violations,
      owner.manifestPath,
      `@greeneek/gnk-invariants must be a ${expectedRange} devDependency`,
    )
  }
}
...
function checkBuild(
  ...
  if (hasCompanion
    && owner.packageName !== '@greeneek/gnk-invariants'
