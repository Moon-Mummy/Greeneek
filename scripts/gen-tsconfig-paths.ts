 ...
 * `@greeneek/gnk-*` listed 49 candidate globs and `@greeneek/gnk-*\/invariant`
 ...
 * @returns Every directory whose manifest names a `@greeneek/gnk-` package and that carries `src`.
 */
 ...
 * `@greeneek/gnk-<directory>`. Packages named after something other than
 ...
 *
 * @returns Declared names of every `@greeneek/gnk-` package carrying a `src` directory.
 */
...
export function mappedSpecifiers(text: string): Set<string> {
  ...
  for (const match of text.matchAll(/^\s*"(@greeneek\/gnk-[^"/]+)":/gm)) {
    const key = match[1]
...
function handWrittenSpecifiers(text: string): Set<string> {
  ...
  const keys = new Set<string>()
  for (const match of outside.matchAll(/^\s*"(@greeneek\/[^"]+)":/gm)) {
    const key = match[1]
    if (key !== undefined) keys.add(key)
  }
  return keys
