/**
 * Rescope the vendored Cordis packages into the `@greeneek` scope, and undo
 ...
 * `@greeneek/gnk-tool-cordis`, and `cordiverse/cordis`, and makes the
...
const RENAMES: readonly Rename[] = [
  { directory: 'cordis', upstream: 'cordis', scoped: '@greeneek/cordis' },
  { directory: 'cosmokit', upstream: 'cosmokit', scoped: '@greeneek/cosmokit' },
  { directory: 'schemastery', upstream: 'schemastery', scoped: '@greeneek/schemastery' },
  { directory: 'loader', upstream: '@cordisjs/plugin-loader', scoped: '@greeneek/cordis-plugin-loader' },
  { directory: 'include', upstream: '@cordisjs/plugin-include', scoped: '@greeneek/cordis-plugin-include' },
  { directory: 'group', upstream: '@cordisjs/plugin-group', scoped: '@greeneek/cordis-plugin-group' },
  { directory: 'timer', upstream: '@cordisjs/plugin-timer', scoped: '@greeneek/cordis-plugin-timer' },
  { directory: 'hmr', upstream: '@cordisjs/plugin-hmr', scoped: '@greeneek/cordis-plugin-hmr' },
  { directory: 'logger-console', upstream: '@cordisjs/plugin-logger-console', scoped: '@greeneek/cordis-plugin-logger-console' },
]
...
const POSTCONDITIONS: readonly PostCondition[] = [
  { file: 'vendor/cordis/package.json', text: '"name": "@greeneek/cordis"', count: 1 },
  { file: 'vendor/hmr/package.json', text: '"name": "@greeneek/cordis-plugin-hmr"', count: 1 },
  { file: 'scripts/cordis-walk.ts', text: '@greeneek\\/cordis', count: 1 },
  { file: 'scripts/cordis-walk.ts', text: '!== \'@greeneek/cordis\'', count: 1 },
  { file: 'scripts/gen-scoped-events.ts', text: '=== \'@greeneek/cordis\'', count: 1 },
  { file: 'packages/typert/generator/src/analyzer.ts', text: '!== \'@greeneek/cordis\'', count: 2 },
  { file: 'scripts/check-workspace-constraints.ts', text: '?.[\'@greeneek/cordis\']', count: 2 },
  { file: 'packages/boot/app-boot/tsdown.config.ts', text: '[\'@greeneek/cordis-plugin-include\']', count: 1 },
  { file: 'tsconfig.base.json', text: '"@greeneek/cordis-plugin-loader": ["./vendor/loader/src"]', count: 1 },
  // The vendored README owns this required entry; reject its deletion or duplication.
  { file: 'vendor/README.md', text: '17. **`@greeneek` rescope**', count: 1 },
...
const EXACT_EDITS: readonly ExactEdit[] = [
  {
    ...
    replace: 'const MERGE_HEAD = /declare module [\'"](?:@greeneek\\/cordis|\\.\\/context\\.ts)[\'"]/',
    expect: 1,
  },
  {
    ...
    replace: `    const peer = manifest.peerDependencies?.['@greeneek/cordis']
...
`,
  ...
  },
  {
    ...
    find: '      if (!isVendored && !name.startsWith(\'@greeneek/\')) {',
  ...
  },
  {
    ...
    replace: 'whose lazy `require(\'@greeneek/cosmokit\')` can race',
    expect: 1,
  },
  {
  ...
  },
  {
    ...
    replace: 'vendored packages are rescoped ([mapping](docs/rescope.md)) and `private: true`. `@greeneek/cordis` is a peerDependency (+ dev) of every harness package.',
    expect: 1,
  },
  {
    // The client purity gate reads `@greeneek/` as "another plugin package".
    ...
    replace: `/**
 * Vendored framework libraries: rescoped into @greeneek, so the gate below
 ...
 */
const VENDORED_LIBRARY = /^@greeneek\\/(cosmokit|schemastery)(\\/|$)/
...
/** Generated descriptor/codec contribution with no shared runtime identity. */`,
  ...
  },
  {
    ...
    replace: "import { Context } from '@greeneek/cordis'",
    expect: 1,
  },
  {
  ...
  },
  {
    ...
    replace: 'The Cordis framework and its foundation libraries are source-vendored into this repository rather than consumed from npm, and republished under the \\`@greeneek\\` scope. All are MIT-licensed',
    expect: 1,
  },
  {
...
\${vendored.map(row => \`| \\\`\${row.npmName}\\\` | \\\`\${row.upstreamName}\\\` | [\${row.upstream.replace('https://', '')}](\${row.upstream}) | MIT |\`).join('\\n')}`,
  ...
  },
  {
    ...
    replace: 'parseVendoredRows(\'| `cordis/` | `@greeneek/cordis` | cordis | 4.0.0 | https://example.com | `abc123` |\\n\')',
    expect: 1,
  },
  {
    ...
    writeFileSync(join(consumerDir, 'package.json'), JSON.stringify({ name: 'gnk-packed-consumer', private: true, type: 'module' }))
