...
describe('worktree-local Lefthook installer', { timeout: 90_000 }, () => {
  ...
  it('never replaces a custom worktree pairing merge driver', async () => {
    ...
    git(fixture, fixture.main, ['config', '--file', commonConfig, 'extensions.worktreeConfig', 'true'])
    git(fixture, fixture.main, [
      'config', '--worktree', 'merge.gnk-translation-pairing.driver', 'custom-driver %A',
    ])

    const result = await runInstaller(fixture, fixture.main)

    expect(result.status).toBe(1)
    expect(result.stderr).toContain('refusing to replace worktree merge.gnk-translation-pairing.driver')
    expect(git(fixture, fixture.main, [
      'config', '--worktree', '--get', 'merge.gnk-translation-pairing.driver',
  ...
  it('never masks an inherited custom pairing merge driver', async () => {
    const fixture = createFixture()
    git(fixture, fixture.main, [
      'config', '--local', 'merge.gnk-translation-pairing.driver', 'inherited-driver %A',
    ])

    const result = await runInstaller(fixture, fixture.main)

    expect(result.status).toBe(1)
    expect(result.stderr).toContain('refusing to mask inherited merge.gnk-translation-pairing.driver')
    expect(git(fixture, fixture.main, [
      'config', '--local', '--get', 'merge.gnk-translation-pairing.driver',
  ...
  it('reports installation and hook-path rollback failures together', async () => {
    ...
    expect(result.stderr).toContain('git config --worktree --unset-all merge.gnk-translation-pairing.driver failed')
  })
