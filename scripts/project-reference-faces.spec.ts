...
function workspaceFixture(options: {
  ...
  const root = mkdtempSync(join(tmpdir(), 'gnk-project-reference-faces-'))
  roots.push(root)
  ...
  writeJson(join(shared, 'package.json'), { name: '@greeneek/gnk-shared' })
  writeJson(join(shared, 'tsconfig.json'), {
    extends: '../../../tsconfig.base.json',
    references: [],
  })
  writeJson(join(split, 'package.json'), { name: '@greeneek/gnk-split' })
...
describe('Project Reference compiler faces', () => {
  ...
  it('uses the referencing project face throughout the reachable graph', () => {
    ...
    writeJson(join(hostConsumer, 'package.json'), { name: '@greeneek/gnk-host-consumer' })
    ...
    writeJson(join(clientConsumer, 'package.json'), { name: '@greeneek/gnk-client-consumer' })
