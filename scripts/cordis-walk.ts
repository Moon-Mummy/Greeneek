...
const MERGE_HEAD = /declare module ['"](?:@greeneek\/cordis|\.\/context\.ts)['"]/
 ...
 * BLOCK — a file may legally hold several `declare module '@greeneek/cordis'` blocks
...
/** Every cordis module-merge body in `sf`: `declare module '@greeneek/cordis'` (harness
...
function cordisModuleBodies(sf: ts.SourceFile): ts.ModuleBlock[] {
  ...
  for (const stmt of sf.statements) {
    ...
    if (stmt.name.text !== '@greeneek/cordis' && stmt.name.text !== './context.ts') continue
...
/**
 * Every `key: Type` property a `declare module '@greeneek/cordis'` Context merge
...
/**
 * Every event name a `declare module '@greeneek/cordis'` Events merge declares in one
