...
const REQUESTING_PACKAGE = '@greeneek/gnk-client-ui-conversation'
...
describe('client bundle build faces', () => {
  it('watches source in development and consumes emitted JavaScript in the Client build', () => {
    const bundle = clientBundle('@greeneek/gnk-client-test', ['lib/types/index.js'])
    const development = bundle({ env: {} }).find(config => config.platform === 'browser')
    const artifact = bundle({ env: { GNK_BUILD_FACE: 'client' } })
...
function purityResolveId(id = REQUESTING_PACKAGE): ResolveId {
  ...
  const gate = plugins.find(p => p.name === 'gnk-client-bundle-purity')
...
function cssModulePlugin(): CssModulePlugin {
  ...
  const plugin = plugins.find(candidate => candidate.name === 'gnk-css-modules-inline')
...
function sourceMapPlugin(): SourceMapPlugin {
  ...
  const plugin = plugins.find(candidate => candidate.name === 'gnk-tsc-sourcemap')
...
describe('client bundle purity gate', () => {
  ...
  it('leaves default externals and non-scoped specifiers alone', () => {
    expect(resolveId('@greeneek/gnk-client-store')).toBeNull()
    expect(resolveId('@greeneek/gnk-client-ui-slots')).toBeNull()
    expect(resolveId('@greeneek/gnk-client-ui-primitives')).toBeNull()
    expect(resolveId('react')).toBeNull()
    expect(resolveId('zod')).toBeNull()
  })

  it('rejects the retired web-react platform package', () => {
    expect(() => resolveId('@greeneek/gnk-client-web-react')).toThrow(/purity/)
    expect(() => resolveId('@greeneek/gnk-client-web-react/store')).toThrow(/purity/)
  })

  it('lets inline-safe libraries inline', () => {
    expect(resolveId('@greeneek/gnk-session/surface')).toBeNull()
    expect(resolveId('@greeneek/gnk-brand')).toBeNull()
    expect(resolveId('@greeneek/gnk-deque')).toBeNull()
    expect(resolveId('@greeneek/gnk-util-values')).toBeNull()
    expect(resolveId('@greeneek/gnk-token-meter/client')).toBeNull()
    expect(() => resolveId('@greeneek/gnk-token-meter')).toThrow(/purity/)
    expect(() => resolveId('@greeneek/gnk-token-meter/client/internal')).toThrow(/purity/)
  })

  it('lets exact generated Remote contributions inline without admitting their package implementation', () => {
    expect(resolveId('@greeneek/gnk-goal/remote')).toBeNull()
    expect(() => resolveId('@greeneek/gnk-goal')).toThrow(/purity/)
    expect(() => resolveId('@greeneek/gnk-goal/client')).toThrow(/purity/)
    expect(() => resolveId('@greeneek/gnk-goal/remote/nested')).toThrow(/purity/)
  })

  it('throws on any other @greeneek leak', () => {
    expect(() => resolveId('@greeneek/gnk-agent')).toThrow(/purity/)
    expect(() => resolveId('@greeneek/gnk-client-web')).toThrow(/purity/)
  })

  it('throws on cross-plugin value imports — bare plugin names and /client subpaths alike', () => {
    expect(() => resolveId('@greeneek/gnk-client-connection')).toThrow(/purity/)
    expect(() => resolveId('@greeneek/gnk-client-ui-session')).toThrow(/purity/)
    expect(() => resolveId('@greeneek/gnk-client-ui-layout/client')).toThrow(/purity/)
  })

  it('admits package-specific requests only for the declaring bundle', () => {
    const requesting = purityResolveId('@greeneek/gnk-api-session-controller')
    expect(requesting('@greeneek/gnk-api-gateway/client')).toBeNull()
    expect(() => resolveId('@greeneek/gnk-api-gateway/client')).toThrow(/purity/)
  })
  ...
  it('externalizes the baseline independently of each package manifest', () => {
    ...
    const plain = clientConfigs('@greeneek/gnk-client-connection')[0]?.deps as {
    ...
    expect(plain.neverBundle('@greeneek/gnk-client-store')).toBe(true)
  })
})

describe('client bundle module requests', () => {
  it('requests what the declaration lists', () => {
    const requests = requestedExternals('@greeneek/gnk-client-fixture', {
      external: ['react', 'react/jsx-runtime', '@greeneek/gnk-client-ui-slots'],
    })

    expect([...requests].sort()).toEqual([
      '@greeneek/gnk-client-ui-slots', 'react', 'react/jsx-runtime',
    ])
  })

  it('requests nothing when the declaration is absent', () => {
    expect(requestedExternals('@greeneek/gnk-client-fixture', {}).size).toBe(0)
  })

  it('rejects a malformed declaration instead of reading past it', () => {
    expect(() => requestedExternals('@greeneek/gnk-client-fixture', { external: 'react' }))
      .toThrow(/gnk\.client\.external must be a string array/)
  })
})
...
describe('client bundle debug artifacts', () => {
  ...
  it('chains emitted tsc maps when the production Client build consumes lib/types', async () => {
    const root = mkdtempSync(join(tmpdir(), 'gnk-client-sourcemap-'))
    try {
  ...
  it('maps first-party sources to their repository package paths', () => {
    const configs = clientConfigs('@greeneek/gnk-client-ui-goal')
    ...
    const resolved = new URL(source, 'https://gnk.test/plugins/@greeneek/gnk-client-ui-goal/client.js.map')
  ...
  it('maps dual-face host sources to the host package group', () => {
    const configs = clientConfigs('@greeneek/gnk-host-directory-picker-native')
    const outputOptions = configs[0]?.outputOptions
    if (typeof outputOptions !== 'object' || outputOptions === null) throw new Error('client output options missing')
    const transform = outputOptions.sourcemapPathTransform
    if (transform === undefined) throw new Error('client sourcemap path transform missing')
  ...
  it('maps inlined workspace sources to packages and leaves dependencies outside it unchanged', () => {
    const configs = clientConfigs('@greeneek/gnk-client-connection')
    const outputOptions = configs[0]?.outputOptions
    if (typeof outputOptions !== 'object' || outputOptions === null) throw new Error('client output options missing')
    const transform = outputOptions.sourcemapPathTransform
    if (transform === undefined) throw new Error('client sourcemap path transform missing')
    ...
    const resolved = new URL(workspaceSource, 'https://gnk.test/plugins/@greeneek/gnk-client-connection/client.js.map')
