 ...
 * consumes the briefing is `.agents/skills/gnk-translate-docs/SKILL.md`.
 */
...
function maskCodeSpans(markdown: string, spans: MarkdownSpan[]): string {
  return replaceSpanTexts(markdown, spans, new Map(spans.map(span => [span.index, `GNK_TRANSLATION_CODE_${span.index}\n`])))
}
